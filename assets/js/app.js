...
function renderCategoryFilter(){
  ...
  if(categoryList) categoryList.innerHTML = cats.filter(cat => cat !== 'Todas').map(cat => `<option value="${safeText(cat)}"></option>`).join('');
}
...
function renderProducts(){
  ...
  document.body.classList.toggle('client-mode', state.catalogMode === 'client');
...
function productCard(p){
  ...
  return `<article class="product-card" data-product-id="${safeText(p.id)}">
    ...
    <div class="product-info">
      ...
      ${promoHtml(p)}
      <div class="product-actions"><button type="button" class="quote-btn" data-add-cart ${out ? 'disabled' : ''}>🛒 Añadir al carrito</button><button type="button" class="details-btn" data-open-details>Detalle</button><button type="button" class="edit-btn" data-edit-product>Editar</button></div>
    </div>
  </article>`;
}
  ...
  if(!state.quote.items.length){
    ...
    itemsBox.innerHTML = state.quote.items.map(item => {
      ...
      return `<div class="quote-item" data-quote-product-id="${safeText(item.productId)}">
        ...
        <div class="qty-control"><button type="button" data-qty-minus aria-label="Restar">−</button><input data-qty-input type="number" min="1" max="${maxStock}" value="${item.qty}"><button type="button" data-qty-plus aria-label="Sumar">+</button></div>
        <button type="button" class="remove-btn" data-remove-item aria-label="Quitar producto">✕</button>
      </div>`;
    }).join('');
  }
...
function renderSales(){
  ...
  box.innerHTML = state.sales.map(s => `<div class="sale-card"><div class="sale-main"><strong>${safeText(s.id)}</strong><small>${safeText(s.cliente?.nombre || 'Cliente')}</small><span>${safeText(s.estado || 'activa')}</span></div><b>${money(s.total || 0)}</b>${s.estado !== 'cancelada' ? `<button class="danger-ghost" data-cancel-sale="${safeText(s.id)}">Cancelar</button>` : ''}</div>`).join('');
...
function bindEvents(){
  ...
  $('#categoryFilter').addEventListener('change', e => { state.category = e.target.value; renderCategoryFilter(); renderProducts(); closeDrawer(); });
