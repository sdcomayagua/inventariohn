...
function renderProducts(){
  ...
  $('#btnClientMode').textContent = state.catalogMode === 'client' ? '👁 Cliente activo' : '👁 Cliente';
  ...
  if(!state.quote.items.length){
    ...
    itemsBox.innerHTML = state.quote.items.map(item => {
      ...
      return `<div class="quote-item" data-quote-product-id="${safeText(item.productId)}">
        ...
        <div class="quote-item-info"><h4>${safeText(item.nombre)}</h4><small>${safeText(item.codigo)} · ${money(unit)} prom. c/u · Total ${money(total)} · Stock ${p.stock ?? item.stock}</small></div>
        <div class="qty-control"><button type="button" data-qty-minus>-</button><input data-qty-input type="number" min="1" max="${p.stock ?? item.stock}" value="${item.qty}"><button type="button" data-qty-plus>+</button></div>
...
function renderInvoice(){
  ...
  $('#invoiceClientBox').innerHTML = c.nombre ? `<b>${safeText(c.nombre)}</b><br>${safeText(c.telefono || '')}<br>${safeText([c.municipio,c.departamento].filter(Boolean).join(', '))}<br>${safeText(c.direccion || '')}` : 'Cliente pendiente de agregar.';
...
function bindEvents(){
  ...
  ['customerName','customerPhone','customerDept','customerCity','customerAddress','shippingCost','shippingCompany'].forEach(id => $('#'+id).addEventListener('input', renderQuote));
