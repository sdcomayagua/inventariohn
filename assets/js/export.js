...
function assertLibraries(){
  ...
  if(!window.jspdf?.jsPDF) throw new Error('jsPDF no cargó todavía. Intente de nuevo.');
}
...
export async function downloadInvoiceImage(element, quoteId){
  ...
  canvas.toBlob(blob => {
    if(!blob) return showToast('No se pudo generar la imagen.');
    downloadBlob(blob, `${quoteId || 'cotizacion'}-SDC.png`);
...
export async function downloadInvoicePdf(element, quoteId){
  ...
  pdf.addImage(imgData, 'PNG', x, margin, renderW, renderH, undefined, 'FAST');
  pdf.save(`${quoteId || 'cotizacion'}-SDC.pdf`);
