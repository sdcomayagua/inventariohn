(function () {
  ...
  function defaultState() {
    return {
      version: 90,
      unlocked: false,
      products: clone(window.SDC_DEFAULT_PRODUCTS || []),
      sales: [],
      quotes: [],
      lastReceipt: null,
      lastQuote: null,
      settings: clone(window.SDC_CONFIG || {}),
    };
  }
  function normalizeProduct(p, i = 0) {
    const categories =
      p.categories || p.category || p.categoria || p.etiquetas || "General";
    const image =
      p.image ||
      p.imagen ||
      p.foto ||
  ...
  function load() {
    return normalizeState(
      safeJSON(localStorage.getItem(KEY), null) || defaultState(),
    );
  }
  function save(state) {
  ...
  function restoreBackup(id) {
    ...
    if (!b) return null;
    save(b.state);
    return normalizeState(b.state);
  }
  function exportData(state) {
    return JSON.stringify(normalizeState(state), null, 2);
  }
  function importData(json) {
    const s = normalizeState(safeJSON(json, null));
    if (!s) throw new Error("Archivo no válido");
    save(s);
    return s;
  }
  window.SDCStore = {
