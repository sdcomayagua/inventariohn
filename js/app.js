(() => {
  ...
  function load(){
    try{
      ...
      out.productGridLayout = ['one','two'].includes(out.productGridLayout) ? out.productGridLayout : 'one';
  ...
  function sectionDashboard(){
    ...
    return `<section class="section ${activateClass('dashboard')}" id="dashboard">
      <div class="hero">
        <span class="hero-badge"><i class="dot"></i> Diseñado para celular</span>
        <h1>SDC POS móvil</h1>
  ...
  function sectionProducts(){
    ...
    return `<section class="section ${activateClass('products')}" id="products">
      ...
      <div class="panel">
        ...
        <div class="product-layout-toggle no-print" role="group" aria-label="Vista del catálogo">
          <button class="${state.productGridLayout === 'one' ? 'active' : ''}" data-product-layout="one" type="button">▣ 1 por fila</button>
          <button class="${state.productGridLayout === 'two' ? 'active' : ''}" data-product-layout="two" type="button">▦ 2 por fila</button>
        </div>
        ...
        <div class="product-grid product-grid-${state.productGridLayout === 'two' ? 'two' : 'one'}">${list.length ? list.map(productCard).join('') : '<div class="empty">No hay productos con ese filtro.</div>'}</div>
      </div>
    </section>`;
  }
  ...
  function receiptPreview(){
    ...
    const ready = orderReadiness();
    const readyText = ready.ok ? 'Pedido listo para confirmar' : `Falta: ${ready.missing.slice(0,3).join(', ')}`;
    ...
    return `<div class="invoice-preview sdc-receipt-v16 sdc-receipt-simple invoice-pro" id="receiptCard">
      ...
      <section class="sdc-simple-products">
        <div class="sdc-simple-section-title"><span>Detalle de productos</span><b>${state.cart.length} ítem${state.cart.length===1?'':'s'}</b></div>
      ...
      <section class="sdc-simple-summary">
      ...
      <footer class="sdc-simple-footer">Gracias por preferir SD COMAYAGUA · Atención profesional por WhatsApp</footer>
    </div>`;
  }
  ...
  function bind(){
    ...
    $$('[data-product-layout]').forEach(b => b.onclick = () => { state.productGridLayout = b.dataset.productLayout === 'two' ? 'two' : 'one'; save(); render(); });
