(function(){
  ...
  const DEFAULT_SYNC_URL = String((window.SDC_CONFIG && window.SDC_CONFIG.syncUrl) || 'https://script.google.com/macros/s/AKfycbxRhSkmnCOPEM-EHeOwYoAkDuGhzCbBRkxWOnpMPWLsKdoEgRenh5WjHTPS4yLUYaUr/exec').trim();
  ...
  function hydrateState(){
    ...
    if(state.settings.syncUrl===undefined) state.settings.syncUrl='';
    if(!String(state.settings.syncUrl||'').trim() && DEFAULT_SYNC_URL) state.settings.syncUrl=DEFAULT_SYNC_URL;
    if(state.settings.syncPin===undefined) state.settings.syncPin='';
    if(state.settings.syncAuto===undefined) state.settings.syncAuto=true;
    if(state.settings.lastCloudStatus===undefined) state.settings.lastCloudStatus='URL de nube precargada';
  ...
  function actionNotice(title,msg,type='ok'){
    const old=document.querySelector('.action-notice'); if(old) old.remove();
    const box=document.createElement('div');
    box.className=`action-notice ${type}`;
    box.innerHTML=`<b>${escapeHtml(title)}</b><span>${escapeHtml(msg)}</span>`;
    document.body.appendChild(box);
    requestAnimationFrame(()=>box.classList.add('show'));
    clearTimeout(actionNotice._t);
    actionNotice._t=setTimeout(()=>{box.classList.remove('show');setTimeout(()=>box.remove(),260)},4200);
  }
  function setLoading(btn,on,text){
    if(!btn) return;
    if(on){btn.dataset.oldText=btn.textContent;btn.disabled=true;btn.classList.add('is-loading');btn.textContent=text||'Procesando...';}
    else{btn.disabled=false;btn.classList.remove('is-loading');if(btn.dataset.oldText) btn.textContent=btn.dataset.oldText;}
  }
  let cloudTimer=0, cloudBusy=false;
  function cloudHasUrl(){return !!(window.SDCCloud && SDCCloud.configured(state));}
  function cloudReady(){return !!(cloudHasUrl() && String(state.settings.syncPin||'').trim());}
  function scheduleCloudAutoSave(){
    if(!cloudReady() || !state.settings.syncAuto || !state.unlocked) return;
    clearTimeout(cloudTimer);
    cloudTimer=setTimeout(()=>cloudBackup('auto', true), 2200);
  }
  function save(){hydrateState(); SDCStore.save(state); scheduleCloudAutoSave();}
  ...
  function cloudTimeLabel(){
    const raw=state.settings.lastCloudSyncAt; if(!raw) return 'Pendiente';
    const d=new Date(raw); if(Number.isNaN(d.getTime())) return 'Pendiente';
    return d.toLocaleString('es-HN',{day:'2-digit',month:'short',hour:'numeric',minute:'2-digit'});
  }
  function cloudStatusHTML(){
    const hasUrl=cloudHasUrl();
    const hasPin=!!String(state.settings.syncPin||'').trim();
    const ok=cloudReady();
    let title='☁ Local';
    let detail='Aún no está conectado a Google Sheets';
    let cls='warn';
    if(hasUrl && !hasPin){title='☁ URL guardada'; detail='Falta guardar el PIN de nube una sola vez'; cls='warn';}
    if(ok){title='☁ Sincronización'; detail=`${state.settings.lastCloudStatus || 'Nube lista'} · ${cloudTimeLabel()}`; cls='ok';}
    return `<div class="cloud-status ${cls}"><b>${escapeHtml(title)}</b><span>${escapeHtml(detail)}</span></div>`;
  }
  function validateCloudConfig(showModal=true){
    hydrateState();
    if(!String(state.settings.syncUrl||'').trim()){ if(showModal){openCloudSetup(); toast('Falta la URL /exec de Apps Script.');} return 'Falta la URL /exec de Apps Script.'; }
    if(!String(state.settings.syncPin||'').trim()){ if(showModal){openCloudSetup('Falta el PIN de nube. Escríbelo una sola vez y toca Guardar y probar conexión.');} return 'Falta el PIN de nube.'; }
    return '';
  }
  async function cloudBackup(label='manual', silent=false){
    const missing=validateCloudConfig(!silent);
    if(missing){ if(!silent) actionNotice('Nube pendiente', missing, 'warn'); return false; }
    if(cloudBusy){ if(!silent) toast('Ya hay una acción de nube en proceso.'); return false; }
    try{
      cloudBusy=true;
      state.settings.lastCloudStatus='Guardando en nube...';
      SDCStore.save(state);
      if(!silent) actionNotice('Guardando respaldo', 'Enviando inventario, ventas, cotizaciones y configuración a Google Sheets.', 'info');
      const result = label==='auto' ? await SDCCloud.push(state,'auto') : await SDCCloud.backup(state,label);
      state.settings.lastCloudSyncAt = result.updatedAt || new Date().toISOString();
      state.settings.lastCloudStatus = label==='auto' ? 'Autoguardado' : 'Respaldo guardado';
      SDCStore.save(state);
      if(!silent){ toast('Respaldo completo guardado en Google Sheets.'); actionNotice('Listo', 'Tu respaldo completo ya quedó guardado en Google Sheets.', 'ok'); render(); }
      return true;
    }catch(err){
      console.error(err);
      state.settings.lastCloudStatus = 'Error de nube';
      SDCStore.save(state);
      if(!silent){ toast('No se pudo guardar en nube. Revisa URL, permisos o PIN.'); actionNotice('No se guardó', err?.message || 'Revisa URL, permisos de Apps Script o PIN.', 'err'); render(); }
      return false;
    }finally{cloudBusy=false;}
  }
  async function syncCloud(opts={}){
    const missing=validateCloudConfig(!opts.silent);
    if(missing){ if(!opts.silent) actionNotice('Sincronización pendiente', missing, 'warn'); return false; }
    if(cloudBusy){ if(!opts.silent) toast('Ya hay una sincronización en proceso.'); return false; }
    try{
      cloudBusy=true;
      state.settings.lastCloudStatus='Sincronizando...';
      SDCStore.save(state);
      if(!opts.silent){ toast('Sincronizando con Google Sheets...'); actionNotice('Sincronizando', 'Leyendo Google Sheets y guardando los cambios de este dispositivo.', 'info'); }
      const remote = await SDCCloud.pull(state);
      if(remote){
        state = SDCCloud.mergeStates(state, remote);
        hydrateState();
        state.unlocked=true;
      }
      await SDCCloud.push(state,'sync');
      state.settings.lastCloudSyncAt = new Date().toISOString();
      state.settings.lastCloudStatus = 'Sincronizado';
      SDCStore.save(state);
      applyAppearance(); render();
      if(!opts.silent){ toast('Listo: celular y computadora quedan sincronizados.'); actionNotice('Sincronización completa', 'Lo que hagas en celular ya puede verse en computadora y queda guardado en Sheets.', 'ok'); }
      return true;
    }catch(err){
      console.error(err);
      state.settings.lastCloudStatus='Error de sincronización';
      SDCStore.save(state);
      if(!opts.silent){ toast('No se pudo sincronizar. Revisa Apps Script, permisos o PIN.'); actionNotice('Error de nube', err?.message || 'Revisa Apps Script, permisos, URL /exec o PIN.', 'err'); render(); }
      return false;
    }finally{cloudBusy=false;}
  }
  function openCloudSetup(initialMessage=''){
    const savedUrl=String(state.settings.syncUrl||DEFAULT_SYNC_URL||'').trim();
    openModal(`<div class="modal-head no-print"><h3>Sincronización en nube</h3><button class="close">×</button></div><div class="modal-body cloud-setup no-print"><div class="card-box cloud-intro"><h4>Google Sheets + Apps Script</h4><p>La URL oficial ya queda guardada en la página. Solo necesitas guardar el PIN una vez en cada dispositivo. Después, cada venta, cotización, producto o respaldo se puede sincronizar con Google Sheets.</p>${cloudStatusHTML()}</div><div id="cloudActionStatus" class="cloud-action-status ${initialMessage?'warn':'neutral'}"><b>${initialMessage?'Acción necesaria':'Estado de nube'}</b><span>${escapeHtml(initialMessage || 'Cuando toques un botón, aquí verás si guardó, sincronizó o falló.')}</span></div><label><span class="label">URL Web App /exec de Apps Script</span><input id="syncUrlInput" class="input" placeholder="https://script.google.com/macros/s/.../exec" value="${escapeHtml(savedUrl)}"><small class="field-help">Ya viene precargada. Solo cambia esta URL si publicas otro Apps Script.</small></label><label><span class="label">PIN de nube</span><input id="syncPinInput" class="input" type="password" inputmode="numeric" placeholder="PIN configurado en Apps Script" value="${escapeHtml(state.settings.syncPin||'')}"><small class="field-help">Este PIN se guarda localmente en este celular/computadora para no pedirlo cada vez.</small></label><label class="switch-row"><span><b>Guardar automáticamente</b><small>Cada cambio importante se respalda en Google Sheets sin tocar otro botón.</small></span><input id="syncAutoInput" type="checkbox" ${state.settings.syncAuto?'checked':''}></label><div class="cloud-private-help"><b>Privado del administrador</b><span>Estas instrucciones no aparecen en la ficha del cliente, captura limpia, foto de producto ni recibo enviado por WhatsApp.</span></div><div class="cloud-steps"><b>Qué hace cada botón</b><ol><li><b>Guardar y probar conexión:</b> guarda la URL/PIN en este dispositivo y verifica que Apps Script responda.</li><li><b>Sincronizar ahora:</b> trae lo que esté en Google Sheets y luego guarda los cambios actuales.</li><li><b>Respaldar todo ahora:</b> crea una copia completa de productos, ventas, cotizaciones, clientes, gastos, catálogos y configuración.</li></ol></div><div class="modal-actions import-actions cloud-actions" style="position:static"><button class="btn full" id="saveCloudConfig">Guardar y probar conexión</button><button class="btn secondary full" id="syncNowCloud">Sincronizar ahora</button><button class="btn ghost full" id="backupNowCloud">Respaldar todo ahora</button></div></div>`,true);
    const status=$('#cloudActionStatus');
    const setStatus=(type,title,msg)=>{ if(!status)return; status.className=`cloud-action-status ${type}`; status.innerHTML=`<b>${escapeHtml(title)}</b><span>${escapeHtml(msg)}</span>`; };
    const applyInputs=()=>{ state.settings.syncUrl=$('#syncUrlInput').value.trim() || DEFAULT_SYNC_URL; state.settings.syncPin=$('#syncPinInput').value.trim(); state.settings.syncAuto=$('#syncAutoInput').checked; save(); };
    const missingInline=()=>{ if(!String(state.settings.syncUrl||'').trim())return 'Falta la URL /exec de Apps Script.'; if(!String(state.settings.syncPin||'').trim())return 'Falta el PIN de nube.'; return ''; };
    $('#saveCloudConfig').onclick=async(e)=>{
      const btn=e.currentTarget; applyInputs(); const missing=missingInline();
      if(missing){ setStatus('warn','No está completo',missing); actionNotice('Nube pendiente',missing,'warn'); return; }
      try{ setLoading(btn,true,'Probando...'); setStatus('loading','Probando conexión','Conectando con Google Sheets por Apps Script...'); const r=await SDCCloud.ping(state); state.settings.lastCloudStatus='Conectado'; state.settings.lastCloudSyncAt=r.updatedAt||new Date().toISOString(); save(); setStatus('ok','Conexión guardada','Listo. La URL y el PIN quedaron guardados en este dispositivo. Ya puedes sincronizar o respaldar.'); toast('Conexión correcta con Google Sheets.'); actionNotice('Conexión correcta','La nube quedó guardada. No tienes que pegar la URL otra vez en este dispositivo.','ok'); render(); }
      catch(err){ console.error(err); state.settings.lastCloudStatus='Error de conexión'; save(); setStatus('err','No conectó',err?.message || 'Revisa URL /exec, permisos de Apps Script y PIN.'); actionNotice('No conectó','Revisa URL /exec, permisos de Apps Script o PIN.','err'); }
      finally{ setLoading(btn,false); }
    };
    $('#syncNowCloud').onclick=async(e)=>{
      const btn=e.currentTarget; applyInputs(); const missing=missingInline();
      if(missing){ setStatus('warn','No está completo',missing); actionNotice('Nube pendiente',missing,'warn'); return; }
      setLoading(btn,true,'Sincronizando...'); setStatus('loading','Sincronizando','Leyendo Google Sheets y guardando los cambios actuales...');
      const ok=await syncCloud({silent:true});
      setStatus(ok?'ok':'err', ok?'Sincronización completa':'No se pudo sincronizar', ok?'Celular y computadora quedan con la misma información.':'Revisa permisos, URL /exec o PIN.');
      actionNotice(ok?'Sincronización completa':'Error de nube', ok?'Datos actualizados en Google Sheets.':'No se pudo sincronizar con Google Sheets.', ok?'ok':'err');
      setLoading(btn,false);
    };
    $('#backupNowCloud').onclick=async(e)=>{
      const btn=e.currentTarget; applyInputs(); const missing=missingInline();
      if(missing){ setStatus('warn','No está completo',missing); actionNotice('Nube pendiente',missing,'warn'); return; }
      setLoading(btn,true,'Respaldando...'); setStatus('loading','Guardando respaldo','Creando respaldo completo en Google Sheets...');
      const ok=await cloudBackup('manual', true);
      setStatus(ok?'ok':'err', ok?'Respaldo guardado':'No se pudo respaldar', ok?'Productos, ventas, cotizaciones, clientes y configuración quedaron respaldados.':'Revisa permisos, URL /exec o PIN.');
      actionNotice(ok?'Respaldo guardado':'Error de respaldo', ok?'Todo quedó guardado en Google Sheets.':'No se pudo guardar el respaldo.', ok?'ok':'err');
      setLoading(btn,false);
    };
  }
  function currentAppearance(){return ['turbo','pro-gamer','pro'].includes(state.settings.appearance)?'turbo':'gamer'}
  function applyAppearance(){const mode=currentAppearance(); document.body.classList.toggle('pro-mode',false); document.body.classList.toggle('turbo-mode',mode==='turbo'); document.body.classList.toggle('gamer-mode',mode==='gamer'); document.body.classList.toggle('capture-clean',!!state.settings.captureClean); document.body.classList.toggle('money-locked',!!state.settings.moneyLocked); document.body.classList.toggle('compact-mode',!!state.settings.compactMode);}
  ...
  function toggleDensity(){state.settings.compactMode=!state.settings.compactMode; save(); applyAppearance(); render(); toast(state.settings.compactMode?'Modo compacto activado.':'Modo amplio activado.'); actionNotice(state.settings.compactMode?'Modo compacto':'Modo amplio', state.settings.compactMode?'Se redujo espacio para vender más rápido desde celular.':'Se dejó más aire visual para revisar con calma.', 'ok');}

  function render(){
    applyAppearance();
    if(!state.unlocked){renderLogin();return}
    app.className='app';
    app.innerHTML = `${topbar()}${hero()}${smartStatusPanel()}${presencePanel()}${quickPanel()}${cardModePanel()}${searchPanel()}${categoryGallery()}${inventoryHTML()}${bottomNav()}`;
    bindMain();
  ...
  function unlock(){ if($('#keyInput').value.trim()===(state.settings.accessKey||'199311')){state.unlocked=true;save();render();toast('Panel desbloqueado.'); if(cloudReady()) setTimeout(()=>syncCloud({silent:true}),450);} else toast('Clave incorrecta.'); }
  function topbar(){
    const turbo=currentAppearance()==='turbo';
    const cloud=cloudReady();
    return `<header class="topbar topbar-v28"><img class="top-logo" src="assets/logo-sdc.png" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1><p><span class="mini-led ${cloud?'ok':'warn'}"></span>${turbo?'PRO Gamer':'Normal'} · ${cloud?'Nube activa':(cloudHasUrl()?'URL guardada':'Local')} · V37</p></div><div class="spacer"></div><button class="btn small ghost cloud-btn" data-action="cloudSetup">☁</button><button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro Gamer'}</button>
      <button class="btn small ghost density-btn" data-action="density">${state.settings.compactMode?'Amplio':'Compacto'}</button><button class="btn small ghost sync-btn" data-action="sync">Sync</button><button class="btn small secondary exit-btn" data-action="lock">Salir</button></header>`
  }
  function hero(){
    const st=stats();
    const todaySales=(state.sales||[]).filter(s=>isTodayISO(s.date)).length;
    const pendingQuotes=(state.quotes||[]).filter(q=>!/(vendido|cancelado)/i.test(q.status||'')).length;
    return `<section class="hero hero-v27 hero-v28 hero-v31" id="inicio">
      <img class="hero-watermark-logo no-print" src="assets/logo-sdc.png" alt="SD Comayagua">
      <div class="hero-copy-v27">
        <div class="pill login-pill"><span class="dot"></span> SD Comayagua · Sistema privado</div>
        <h2>CONTROL DE VENTAS</h2><p>Inventario, cotizaciones, ventas, recibos editables, envíos y respaldo automático para trabajar rápido desde celular.</p>
        <div class="ops-ribbon no-print"><span>📱 Listo para celular</span><span>🧾 ${num(todaySales)} ventas hoy</span><span>📌 ${num(pendingQuotes)} cotizaciones activas</span><span>☁ ${cloudReady()?'Nube conectada':'Pendiente nube'}</span></div>
      </div>
      <div class="hero-cloud-card no-print">${cloudStatusHTML()}<button class="btn small ghost" data-action="cloudSetup">Configurar nube</button><button class="btn small" data-action="sync">Sincronizar</button></div>
      <div class="stats stats-v28">
  ...
  function smartStatusPanel(){
    const st=stats();
    const todaySales=(state.sales||[]).filter(s=>isTodayISO(s.date));
    const todayTotal=todaySales.reduce((a,s)=>a+calc(s).total,0);
    const pendingQuotes=(state.quotes||[]).filter(q=>!/(vendido|cancelado)/i.test(q.status||'')).length;
    ...
    const cloudLabel=cloudReady()?`Nube activa · ${cloudTimeLabel()}`:(cloudHasUrl()?'URL guardada · falta PIN':'Modo local');
    return `<section class="smart-strip-v34 no-print" aria-label="Resumen rápido">
      <div class="smart-main-v34"><span>Hoy</span><b>${money(todayTotal)}</b><small>${num(todaySales.length)} ventas registradas</small></div>
      <div><span>Cotizaciones</span><b>${num(pendingQuotes)}</b><small>Pendientes o activas</small></div>
      <div><span>Inventario</span><b>${num(st.stock)}</b><small>${num(low)} bajo stock · ${num(noCost)} sin costo</small></div>
      <div><span>Nube</span><b>${cloudReady()?'Lista':'Revisar'}</b><small>${escapeHtml(cloudLabel)}</small></div>
    </section>`;
  }
  ...
  function presencePanel(){
    ...
    const todayTotal=today.reduce((a,s)=>a+calc(s).total,0);
    const pendingQuotes=(state.quotes||[]).filter(q=>!/(vendido|cancelado)/i.test(q.status||'')).length;
    const low=state.products.filter(p=>Number(p.stock)>0 && Number(p.stock)<=Number(state.settings.lowStockLimit||3)).length;
    const cloud=cloudReady();
    return `<section class="presence-panel no-print">
      <div class="presence-brand-card">
        <img src="assets/logo-sdc.png" alt="SD Comayagua">
        <div>
          <small>Centro de mando privado</small>
          <b>SD COMAYAGUA</b>
          <span>Ventas, inventario, cotizaciones y respaldo en una sola pantalla.</span>
        </div>
      </div>
      <div class="presence-actions">
        <button data-action="quickSale"><i>⚡</i><b>Venta rápida</b><span>${money(todayTotal)} hoy</span></button>
        <button data-action="quote"><i>🧾</i><b>Cotizar</b><span>${num(pendingQuotes)} activas</span></button>
        <button data-action="cloudSetup"><i>☁</i><b>${cloud?'Nube activa':'Conectar nube'}</b><span>${cloud?cloudTimeLabel():'Sheets + Apps Script'}</span></button>
        <button data-action="backup"><i>🛡</i><b>Respaldo total</b><span>${num(st.count)} productos · ${num(low)} alertas</span></button>
      </div>
    </section>`
  }
  ...
  function quickPanel(){
    ...
    return `<div class="module-title no-print"><div><small>MÓDULOS SDC</small><b>Accesos principales</b></div><span>Panel privado</span></div><section class="quick no-print quick-v22 quick-v31">
      <button data-action="catalog"><i>▦</i><b>Catálogo</b><span>Ver productos</span></button>
      <button data-action="sell"><i>🛒</i><b>Vender</b><span>Seleccionar producto</span></button>
      <button data-action="quickSale"><i>⚡</i><b>Venta rápida</b><span>Sin tantos pasos</span></button>
      <button data-action="newProduct"><i>＋</i><b>Producto</b><span>Agregar nuevo</span></button>
      <button data-action="profit"><i>📈</i><b>Ganancias</b><span>Por producto</span></button>
      <button data-action="quotes"><i>🧾</i><b>Cotizaciones</b><span>Guardadas</span></button>
      <button data-action="clients"><i>👤</i><b>Clientes</b><span>Agenda rápida</span></button>
      <button data-action="receipts"><i>▤</i><b>Caja</b><span>Ventas y cierre</span></button>
      <button data-action="expenses"><i>−</i><b>Gastos</b><span>Ganancia neta</span></button>
      <button data-action="backup"><i>🛡</i><b>Respaldo</b><span>Una sola opción</span></button>
      <button data-action="cloudSetup"><i>☁</i><b>Nube</b><span>Sheets / App Script</span></button>
    </section>
  ...
  function mainAction(e){
    ...
    if(a==='sync') return cloudHasUrl()?syncCloud():syncLocal();
    if(a==='cloudSetup') return openCloudSetup();
    if(a==='cloudBackup') return cloudBackup('manual');
    if(a==='theme') return toggleAppearance();
    if(a==='density') return toggleDensity();
...
WhatsApp SD COMAYAGUA: +504 3151-7755`;
  ...
  async function productCardToBlob(){
    ...
    try{
      const canvas=await html2canvas(el,{backgroundColor:'#07111f',scale:3,useCORS:true,imageTimeout:12000});
      return await new Promise(res=>canvas.toBlob(res,'image/png',.98));
    }catch(err){console.error(err); toast('No se pudo generar la imagen. Revisa si alguna foto externa bloquea la descarga.'); return null;}
  }
  ...
  function addDocItem(id,isSale){
    ...
    if(notice){notice.classList.remove('hide'); notice.innerHTML=`<b>✓ Añadido al carrito</b><span>${escapeHtml(p.name)} · cantidad ${found?num(found.qty):'1'}</span>`; clearTimeout(window.__sdcCartNoticeTimer); window.__sdcCartNoticeTimer=setTimeout(()=>notice.classList.add('hide'),2800);}
  ...
  function openBackup(){
    ...
    $$('[data-action]',modalRoot).forEach(b=>b.onclick=()=>mainAction({currentTarget:b}));
  ...
  function toggleCaptureClean(){
    ...
    save(); applyAppearance(); render(); toast(state.settings.captureClean?'Captura limpia activada.':'Captura limpia desactivada.');
  }
  ...
  /* =========================================================
     SDC V32: mejoras visuales para cliente, recibo y captura
     Se deja la misma fuente; solo se sube calidad y claridad.
     ========================================================= */
  function openProductDetails(id){
    const p=productById(id); if(!p)return;
    const imgs=galleryOf(p); const safeImgs=imgs.length?imgs:[productImage(p)];
    let shareQty=clientQty(p.id)||1;
    const stock=Number(p.stock)||0;
    const statusText=stock<=0?'Agotado':stock<=Number(state.settings.lowStockLimit||3)?'Bajo stock':'Disponible';
    const unitsLabel=()=>`${num(shareQty)} ${shareQty===1?'unidad':'unidades'}`;
    const promoUsed=()=>promoTotalForQty(p,shareQty)!==null;
    function cardHTML(){
      const productTotal=productItemsTotal(p,shareQty);
      const normalTotal=productNormalTotalQty(p,shareQty);
      const codTotal=productCodTotalQty(p,shareQty);
      const unitPrice=productTotal/Math.max(1,shareQty);
      const promoMsg=promoLabelForQty(p,shareQty);
      return `<div class="product-share-card product-share-card-v32" id="productShareCard">
        <div class="share-top-v32"><img src="assets/logo-sdc.png" alt="SD"><div><small>SD COMAYAGUA</small><b>${escapeHtml(p.name)}</b><span>WhatsApp +504 3151-7755</span></div><strong class="status ${stock<=0?'sold':stock<=Number(state.settings.lowStockLimit||3)?'low':''}">${statusText}</strong></div>
        <div class="share-media-frame-v32"><img id="detailMainImage" src="${escapeHtml(safeImgs[0])}" alt="${escapeHtml(p.name)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><b class="price-badge share-main-price">${money(p.price)}</b><span class="share-watermark-v32">SD</span></div>
        <div class="share-info-v32"><span class="share-kicker-v34">Ficha lista para cliente</span><h2>${escapeHtml(p.name)}</h2><p class="share-description">${escapeHtml(p.description||'Producto disponible para entrega. Consulta disponibilidad antes de confirmar tu pedido.')}</p></div>
        <div class="share-qty-line share-qty-v32"><span>Cantidad consultada</span><b id="shareQtyText">${unitsLabel()} · ${money(productTotal)}${promoUsed()?' · promo aplicada':''}</b></div>
        ${promoMsg?`<div class="share-offer-v32">🎁 ${escapeHtml(promoMsg)}</div>`:''}
        <div class="public-metrics public-metrics-v32"><div><span>Producto</span><b id="metricProduct">${money(productTotal)}</b><small>${money(unitPrice)} c/u</small></div><div><span>Envío normal</span><b id="metricNormal">${money(normalTotal)}</b><small>Depósito / Tigo Money</small></div><div><span>Al recibir + 6%</span><b id="metricCod">${money(codTotal)}</b><small>Lps.100 + comisión</small></div></div>
        <div class="client-ready-v32"><b>Para confirmar pedido</b><span>Nombre completo</span><span>Departamento y municipio</span><span>Dirección exacta con referencia</span><span>Empresa: Forza, C807 o Cargo Expreso</span></div>
        <div class="process-row process-row-v32"><div class="process-card"><b>Envío Normal</b><span>Producto + Lps. 110. Se paga por depósito, transferencia bancaria o Tigo Money.</span></div><div class="process-card"><b>Pagar al Recibir</b><span>Producto + Lps. 100 de envío + 6% de comisión. Total redondeado en lempiras.</span></div></div>
        <div class="share-service-v34"><span>Atención por WhatsApp</span><span>Envíos a Honduras</span><span>Confirmación antes de despachar</span></div><div class="share-footer share-footer-v32">SD COMAYAGUA · Catálogo y pedidos por WhatsApp</div>
      </div>`;
    }
    openModal(`<div class="modal-head"><h3>Producto</h3><button class="close">×</button></div><div class="modal-body"><div id="shareCardMount">${cardHTML()}</div><div class="client-quote-panel no-print"><div><b>Precio rápido para cliente</b><span>Cambia la cantidad antes de descargar, compartir o enviar por WhatsApp.</span></div><div class="qty-presets"><button data-setqty="1">1</button><button data-setqty="2">2</button><button data-setqty="3">3</button><button data-setqty="5">5</button><button data-setqty="10">10</button><button data-setqty="12">12</button><button data-setqty="20">20</button><button data-setqty="24">24</button></div><div class="qty-control"><button id="detailQtyMinus">−</button><input id="detailQtyInput" type="number" min="1" value="${shareQty}" inputmode="numeric"><button id="detailQtyPlus">+</button></div></div><div class="thumb-row">${safeImgs.map((img,i)=>`<button class="thumb ${i===0?'active':''}" data-product-img="${escapeHtml(img)}"><img src="${escapeHtml(img)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"></button>`).join('')}</div><div class="modal-actions product-actions product-actions-grid no-print"><button class="btn secondary prod-action" id="downloadProductPhoto"><span class="ico">▣</span><b>Imagen</b><small>Descargar</small></button><button class="btn prod-action" id="shareProductPhoto"><span class="ico">↗</span><b>Foto</b><small>Compartir</small></button><button class="btn secondary prod-action" id="waProductText"><span class="ico">✎</span><b>Texto</b><small>WhatsApp</small></button><button class="btn prod-action" data-action="sellProduct" data-id="${escapeHtml(id)}"><span class="ico">🛒</span><b>Vender</b><small>Venta real</small></button><button class="btn secondary prod-action" data-action="quoteProduct" data-id="${escapeHtml(id)}"><span class="ico">🧾</span><b>Cotizar</b><small>Precio total</small></button><button class="btn secondary prod-action" data-action="marketingProduct" data-id="${escapeHtml(id)}"><span class="ico">#</span><b>Textos</b><small>Marketplace</small></button><button class="btn ghost prod-action" data-action="editProduct" data-id="${escapeHtml(id)}"><span class="ico">✦</span><b>Editar</b><small>Producto</small></button></div></div>`);
    function refreshShareCard(){
      const currentImg=$('#detailMainImage',modalRoot)?.src || safeImgs[0];
      $('#shareCardMount',modalRoot).innerHTML=cardHTML();
      const img=$('#detailMainImage',modalRoot); if(img) img.src=currentImg;
      const input=$('#detailQtyInput',modalRoot); if(input) input.value=shareQty;
      setClientQty(p.id,shareQty);
    }
    function setQty(v){shareQty=Math.max(1,Math.min(999,Number(v)||1)); refreshShareCard();}
    $$('[data-setqty]',modalRoot).forEach(b=>b.onclick=()=>setQty(b.dataset.setqty));
    $('#detailQtyMinus').onclick=()=>setQty(shareQty-1);
  ...
  function productWhatsAppText(p,qty=1){
    ...
    const promoNote=promoTotalForQty(p,qty)!==null?`\n🎁 *${promoLabelForQty(p,qty)||'Promoción aplicada por cantidad.'}*`:'';
    const moreQtyNote=String(p.promos||'').trim()?`\n\n🎁 *Tenemos precios por cantidad.* Si desea más unidades, le preparo una cotización con el paquete exacto y un solo envío.`:'';
    return `🛍️ *PRODUCTO DISPONIBLE - SD COMAYAGUA*\n\n📌 *Producto:* ${p.name}\n🔢 *Cantidad consultada:* ${num(qty)} ${qty===1?'unidad':'unidades'}\n💰 *Producto:* ${money(productTotal)}${qty>1?` (${money(unit)} c/u)`:''}${promoNote}\n\n🚚 *Envío Normal:* ${money(productNormalTotalQty(p,qty))}\nProducto + Lps. 110 de envío. Puede pagar por depósito, transferencia bancaria o Tigo Money.\n\n📦 *Pagar al Recibir:* ${money(productCodTotalQty(p,qty))}\nProducto + Lps. 100 de envío + comisión del 6%.\n\n📝 *Descripción:*\n${p.description||'Producto disponible para entrega.'}${moreQtyNote}\n\n✅ *Para confirmar pedido necesito:*\n• Nombre completo\n• Departamento y municipio\n• Dirección exacta con referencia\n• Empresa de envío: Forza, C807 o Cargo Expreso\n\nWhatsApp SD COMAYAGUA: +504 3151-7755`;
  }
  ...
  function docCard(doc,isSale){
    ...
    const code=doc.id||'SDC';
    const date=new Date(doc.date||Date.now()).toLocaleString('es-HN',{day:'2-digit',month:'short',year:'numeric',hour:'numeric',minute:'2-digit'});
    const payTitle=doc.cod?'Pagar al Recibir':'Envío Normal';
    const process=doc.cod?'Producto + Lps. 100 de envío + comisión del 6%. El total ya queda redondeado en lempiras.':'Producto + Lps. 110 de envío. Pago por depósito, transferencia bancaria o Tigo Money.';
    const location=[doc.department,doc.municipality].filter(Boolean).join(' / ')||'No seleccionada';
    const contactReady=!!(doc.client&&doc.phone&&doc.department&&doc.municipality);
    const rows=(doc.items||[]).map((it,i)=>{
      ...
      const p=itemProductRef(it);
      const promo=promoTotalForQty(p,qty)!==null;
      const img=escapeHtml(it.image||productImage(p)||placeholderFor(p));
      return `<div class="doc-item-card-v32"><img src="${img}" alt="${escapeHtml(it.name)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div class="doc-item-main-v32"><div class="doc-prod-line"><span>#${i+1}</span><b>${escapeHtml(it.name)}${promo?`<small class="doc-promo-note">Oferta aplicada</small>`:''}</b></div><div class="doc-prod-metrics"><div><small>Cant.</small><b>${num(qty)}</b></div><div><small>Precio C/U</small><b>${money(unit)}</b></div><div><small>Total</small><b>${money(total)}</b></div></div></div></div>`;
    }).join('')||'<div class="doc-item-empty">Sin productos agregados</div>';
    return `<div class="doc-wrap doc-v32" id="printableDoc">
      <div class="doc-hero-v32"><div class="doc-brand-v32"><img src="assets/logo-sdc.png" alt="Logo"><div><small>${isSale?'Recibo / factura de compra':'Cotización para cliente'}</small><h2>SD COMAYAGUA</h2><p>${date} · ${escapeHtml(code)}</p></div></div><div class="doc-status-v32 ${isSale?'sale':'quote'}"><span>${isSale?'Venta registrada':'Cotización'}</span><b>${money(c.total)}</b></div></div>
      <div class="doc-client-strip-v32"><div><span>Cliente</span><b>${escapeHtml(doc.client||'Cliente')}</b></div><div><span>Teléfono</span><b>${escapeHtml(doc.phone||'No registrado')}</b></div><div><span>Ubicación</span><b>${escapeHtml(location)}</b></div><div><span>Entrega</span><b>${escapeHtml(doc.company||'No seleccionada')}</b></div>${doc.reference?`<div class="wide"><span>Referencia</span><b>${escapeHtml(doc.reference)}</b></div>`:''}</div>
      <div class="doc-pay-v32"><div><small>Proceso de pago</small><b>${payTitle}</b><p>${process}</p></div><span>${contactReady?'Datos básicos completos':'Faltan datos del cliente'}</span></div>
      <div class="doc-items-v32"><div class="doc-items-title-v32"><span>Productos cotizados</span><b>${num((doc.items||[]).length)}</b></div>${rows}</div>
      <div class="doc-mini-summary doc-summary-v32"><div><span>Productos</span><b>${money(c.products)}</b></div><div><span>Envío</span><b>${money(c.shipping)}</b></div><div><span>Comisión</span><b>${money(c.commission)}</b></div><div><span>Descuento</span><b>${money(c.discount)}</b></div><div class="grand"><span>Total a pagar</span><b>${money(c.total)}</b></div></div>
      <div class="doc-next-v34"><b>Siguiente paso</b><p>Confirme el total, la dirección y el método de entrega. Al estar correcto, SD Comayagua prepara el pedido para envío.</p></div><div class="doc-required-v32"><b>Antes de confirmar, revise:</b><span>Producto y cantidad</span><span>Total con envío</span><span>Empresa de entrega</span><span>Dirección con referencia</span></div>
      <p class="doc-note compact-note">${isSale?'Gracias por comprar en SD Comayagua.':'Cotización informativa válida según disponibilidad de inventario y confirmación final.'} WhatsApp: +504 3151-7755</p>
    </div>`;
  }

  async function docToBlob(){
    const el=$('#printableDoc',modalRoot); if(!window.html2canvas){window.print();return null}
    try{const canvas=await html2canvas(el,{backgroundColor:'#edf6fb',scale:3,useCORS:true,imageTimeout:15000,logging:false}); return await new Promise(res=>canvas.toBlob(res,'image/png',.98));}
    catch(err){console.error(err); toast('No se pudo generar la imagen del documento. Revisa si alguna foto externa bloquea la descarga.'); return null;}
  }
  async function productCardToBlob(){
    const el=$('#productShareCard',modalRoot); if(!window.html2canvas){window.print();return null}
    try{const canvas=await html2canvas(el,{backgroundColor:'#04111f',scale:3,useCORS:true,imageTimeout:15000,logging:false}); return await new Promise(res=>canvas.toBlob(res,'image/png',.98));}
    catch(err){console.error(err); toast('No se pudo generar la imagen. Revisa si alguna foto externa bloquea la descarga.'); return null;}
  }


  /* =========================================================
     SDC V37 · Presencia Máxima
     Mejora profesional: tablero ejecutivo, ficha cliente premium,
     recibo más claro y contenido justificado.
     ========================================================= */
  function render(){
    applyAppearance();
    if(!state.unlocked){renderLogin();return}
    app.className='app app-v35';
    app.innerHTML = `${topbar()}${hero()}${smartStatusPanel()}${executivePanelV36()}${businessPulseV38()}${qualityRouteV38()}${quickPanel()}${cardModePanel()}${searchPanel()}${categoryGallery()}${inventoryHTML()}${bottomNav()}`;
    bindMain();
    bindProductCards();
  }
  ...
  function topbar(){
    const turbo=currentAppearance()==='turbo';
    const cloud=cloudReady();
    return `<header class="topbar topbar-v28 topbar-v35">
      <img class="top-logo" src="assets/logo-sdc.png" alt="SD">
      <div class="top-title"><h1>SD COMAYAGUA</h1><p><span class="mini-led ${cloud?'ok':'warn'}"></span>${turbo?'PRO Gamer Elite':'Normal'} · ${state.settings.compactMode?'Compacto':'Amplio'} · ${cloud?'Nube activa':(cloudHasUrl()?'URL guardada':'Local')} · V38</p></div>
      <div class="spacer"></div>
      <button class="btn small ghost cloud-btn" data-action="cloudSetup" aria-label="Nube">☁</button>
      <button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro Gamer'}</button>
      <button class="btn small ghost density-btn" data-action="density">${state.settings.compactMode?'Amplio':'Compacto'}</button>
      <button class="btn small ghost sync-btn" data-action="sync">Sync</button>
      <button class="btn small secondary exit-btn" data-action="lock">Salir</button>
    </header>`;
  }
  ...
  function hero(){
    ...
    const todaySales=(state.sales||[]).filter(s=>isTodayISO(s.date)).length;
    const pendingQuotes=(state.quotes||[]).filter(q=>!/(vendido|cancelado)/i.test(q.status||'')).length;
    return `<section class="hero hero-v27 hero-v28 hero-v31 hero-v35" id="inicio">
      <img class="hero-watermark-logo no-print" src="assets/logo-sdc.png" alt="SD Comayagua">
      <div class="hero-copy-v27 hero-copy-v35">
        <div class="pill login-pill"><span class="dot"></span> Centro de mando privado</div>
        <h2>SDC PRO CENTER</h2>
        <p>Centro privado para vender, cotizar, controlar inventario, guardar facturas y respaldar todo en la nube con presencia profesional.</p>
        <div class="ops-ribbon no-print"><span>📱 Celular primero</span><span>🧾 ${num(todaySales)} ventas hoy</span><span>📌 ${num(pendingQuotes)} cotizaciones</span><span>☁ ${cloudReady()?'Nube conectada':'Configurar nube'}</span></div>
      </div>
      <div class="hero-cloud-card no-print">${cloudStatusHTML()}<button class="btn small ghost" data-action="cloudSetup">Configurar nube</button><button class="btn small" data-action="sync">Sincronizar</button></div>
      <div class="stats stats-v28 stats-v35">
        <div class="stat"><b>${num(st.count)}</b><span>Productos</span></div><div class="stat"><b>${num(st.stock)}</b><span>Stock total</span></div>
        <div class="stat"><b>${moneyPrivate(st.value)}</b><span>Valor venta</span></div><div class="stat"><b>${moneyPrivate(st.invested)}</b><span>Invertido</span></div>
        <div class="stat stat-profit"><b>${moneyPrivate(st.profit)}</b><span>Ganancia estimada</span></div>
      </div>
    </section>`;
  }

  function executivePanelV36(){
    const st=stats();
    const todaySales=(state.sales||[]).filter(s=>isTodayISO(s.date));
    const todayTotal=todaySales.reduce((a,s)=>a+calc(s).total,0);
    const low=state.products.filter(p=>Number(p.stock)>0 && Number(p.stock)<=Number(state.settings.lowStockLimit||3)).length;
    const noCost=state.products.filter(p=>Number(p.cost)<=0).length;
    const pendingQuotes=(state.quotes||[]).filter(q=>!/(vendido|cancelado)/i.test(q.status||'')).length;
    const nextTitle = low ? 'Reponer inventario bajo' : noCost ? 'Completar costos faltantes' : pendingQuotes ? 'Dar seguimiento a cotizaciones' : 'Todo listo para vender';
    const nextText = low ? `${num(low)} producto(s) necesitan reposición.` : noCost ? `${num(noCost)} producto(s) no tienen costo registrado.` : pendingQuotes ? `${num(pendingQuotes)} cotización(es) siguen activas.` : 'Inventario y ventas se ven estables.';
    const action = low ? 'lowStock' : noCost ? 'noCost' : pendingQuotes ? 'quotes' : 'quickSale';
    return `<section class="executive-v35 no-print">
      <div class="executive-card-v35 primary">
        <span class="eyebrow-v35">Siguiente acción</span>
        <h3>${escapeHtml(nextTitle)}</h3>
        <p>${escapeHtml(nextText)}</p>
        <button class="btn small" data-action="${action}">Revisar ahora</button>
      </div>
      <div class="executive-card-v35">
        <span class="eyebrow-v35">Caja de hoy</span>
        <b>${money(todayTotal)}</b>
        <p>${num(todaySales.length)} venta(s) registradas hoy.</p>
      </div>
      <div class="executive-card-v35">
        <span class="eyebrow-v35">Utilidad estimada</span>
        <b>${moneyPrivate(st.profit)}</b>
        <p>${state.settings.moneyLocked?'Protegida para vista privada.':'Según costo y precio guardado.'}</p>
      </div>
      <div class="executive-card-v35 compact-actions-v35">
        <button data-action="quickSale">⚡ Venta</button>
        <button data-action="quote">🧾 Cotizar</button>
        <button data-action="backup">🛡 Respaldo</button>
        <button data-action="cloudSetup">☁ Nube</button>
      </div>
    </section>`;
  }


  function businessPulseV38(){
    const todaySales=(state.sales||[]).filter(s=>isTodayISO(s.date));
    const todayTotal=todaySales.reduce((a,s)=>a+calc(s).total,0);
    const pendingQuotes=(state.quotes||[]).filter(q=>!/(vendido|cancelado)/i.test(q.status||'')).length;
    const low=state.products.filter(p=>Number(p.stock)>0 && Number(p.stock)<=Number(state.settings.lowStockLimit||3)).length;
    const out=state.products.filter(p=>Number(p.stock)<=0).length;
    const cloudLabel=cloudReady()?'Protegido en nube':(cloudHasUrl()?'PIN pendiente':'Solo local');
    const risk=(!cloudReady()?1:0)+(low?1:0)+(out?1:0);
    const health=risk===0?'Excelente':risk===1?'Atención leve':'Revisar hoy';
    return `<section class="pulse-v38 no-print">
      <div class="pulse-head-v38"><div><span>SDC BUSINESS PULSE</span><b>${health}</b></div><small>Vista rápida antes de vender</small></div>
      <div class="pulse-grid-v38">
        <button data-action="receipts"><small>Caja de hoy</small><b>${money(todayTotal)}</b><span>${num(todaySales.length)} venta(s)</span></button>
        <button data-action="quotes"><small>Seguimiento</small><b>${num(pendingQuotes)}</b><span>Cotizaciones activas</span></button>
        <button data-action="lowStock"><small>Inventario</small><b>${num(low)}</b><span>Bajo stock · ${num(out)} agotado(s)</span></button>
        <button data-action="cloudSetup"><small>Seguridad</small><b>${cloudLabel}</b><span>${cloudReady()?'Sync disponible':'Configurar para respaldo'}</span></button>
      </div>
    </section>`;
  }

  function qualityRouteV38(){
    return `<section class="route-v38 no-print">
      <div><i>1</i><b>Producto</b><span>Precio, stock, costo e imagen.</span></div>
      <div><i>2</i><b>Cotización</b><span>Total claro para WhatsApp.</span></div>
      <div><i>3</i><b>Factura</b><span>Venta guardada en caja.</span></div>
      <div><i>4</i><b>Respaldo</b><span>Datos protegidos en Sheets.</span></div>
    </section>`;
  }
  ...
  function quickPanel(){
    ...
    const st=stats();
    const lowMargin=state.products.filter(p=>Number(p.price||0)>0 && (Number(p.price||0)-Number(p.cost||0))>0 && (Number(p.price||0)-Number(p.cost||0))<10).length;
    return `<div class="module-title module-title-v35 no-print"><div><small>ACCESOS SDC</small><b>Operación principal</b></div><span>Privado</span></div>
    <section class="quick no-print quick-v22 quick-v31 quick-v35">
      <button data-action="catalog"><i>▦</i><b>Catálogo</b><span>Ver productos</span></button>
      <button data-action="quickSale"><i>⚡</i><b>Venta rápida</b><span>Registrar en segundos</span></button>
      <button data-action="sell"><i>🛒</i><b>Vender</b><span>Seleccionar producto</span></button>
      <button data-action="quote"><i>🧾</i><b>Cotizar</b><span>Enviar total claro</span></button>
      <button data-action="newProduct"><i>＋</i><b>Producto</b><span>Agregar nuevo</span></button>
      <button data-action="receipts"><i>▤</i><b>Caja</b><span>Ventas y cierre</span></button>
      <button data-action="clients"><i>👤</i><b>Clientes</b><span>Agenda rápida</span></button>
      <button data-action="backup"><i>🛡</i><b>Respaldo</b><span>Guardar todo</span></button>
    </section>
    <section class="alert-row alert-row-v35 no-print">
      <div class="alert-card"><div><b>${low} bajo stock</b><span>Revisa reposición.</span></div><button class="btn small secondary" data-action="lowStock">Ver</button></div>
      <div class="alert-card"><div><b>${nocost} sin costo</b><span>Agrega costo para utilidad real.</span></div><button class="btn small secondary" data-action="noCost">Revisar</button></div>
      <div class="alert-card"><div><b>${lowMargin} ganancia baja</b><span>Menos de Lps. 10 por unidad.</span></div><button class="btn small secondary" data-action="profit">Detalle</button></div>
      <div class="alert-card"><div><b>Ganancia</b><span>${moneyPrivate(st.profit)} estimado.</span></div><button class="btn small secondary" data-action="profit">Detalle</button></div>
      <div class="alert-card"><div><b>${state.settings.moneyLocked?'Ganancias ocultas':'Ganancias visibles'}</b><span>Protege costos y utilidad.</span></div><button class="btn small secondary" data-action="moneyLock">${state.settings.moneyLocked?'Mostrar':'Ocultar'}</button></div>
    </section>`;
  }
  ...
  function productWhatsAppText(p,qty=1){
    ...
    const unit=productTotal/qty;
    const promoNote=promoTotalForQty(p,qty)!==null?`\n🎁 *${promoLabelForQty(p,qty)||'Promoción aplicada por cantidad.'}*`:'';
    const moreQtyNote=String(p.promos||'').trim()?`\n\n🎁 *Precios por cantidad disponibles.* Si desea más unidades, le preparo el paquete exacto con un solo envío.`:'';
    return `🛍️ *SD COMAYAGUA - PRODUCTO DISPONIBLE*\n\n📌 *Producto:* ${p.name}\n🔢 *Cantidad consultada:* ${num(qty)} ${qty===1?'unidad':'unidades'}\n💰 *Producto:* ${money(productTotal)}${qty>1?` (${money(unit)} c/u)`:''}${promoNote}\n\n🚚 *Envío Normal:* ${money(productNormalTotalQty(p,qty))}\nIncluye producto + Lps. 110 de envío. Pago por depósito, transferencia bancaria o Tigo Money.\n\n📦 *Pagar al Recibir:* ${money(productCodTotalQty(p,qty))}\nIncluye producto + Lps. 100 de envío + 6% de comisión.\n\n📝 *Descripción:*\n${p.description||'Producto disponible para entrega.'}${moreQtyNote}\n\n✅ *Para confirmar su pedido necesito:*\n• Nombre completo\n• Departamento y municipio\n• Dirección exacta con referencia\n• Número de celular\n• Empresa de envío: Forza, C807 o Cargo Expreso\n\n📲 *WhatsApp SD COMAYAGUA:* +504 3151-7755`;
  }
  ...
  function openProductDetails(id){
    ...
    function cardHTML(){
      ...
      const unitPrice=productTotal/Math.max(1,shareQty);
      const promoMsg=promoLabelForQty(p,shareQty);
      return `<div class="product-share-card product-share-card-v32 product-share-card-v35" id="productShareCard">
        <div class="share-top-v35"><img src="assets/logo-sdc.png" alt="SD"><div><small>SD COMAYAGUA</small><b>${escapeHtml(p.name)}</b><span>WhatsApp +504 3151-7755</span></div><strong class="status ${stock<=0?'sold':stock<=Number(state.settings.lowStockLimit||3)?'low':''}">${statusText}</strong></div>
        <div class="share-media-frame-v32 share-media-frame-v35"><img id="detailMainImage" crossorigin="anonymous" src="${escapeHtml(safeImgs[0])}" alt="${escapeHtml(p.name)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><b class="price-badge share-main-price">${money(p.price)}</b><span class="share-watermark-v32">SD</span></div>
        <div class="quote-band-v35 quote-band-v36"><span>Cantidad consultada</span><b id="shareQtyText">${unitsLabel()} · ${money(productTotal)}${promoUsed()?' · promo aplicada':''}</b></div>
        <div class="share-info-v32 share-info-v35 share-info-v36"><span class="share-kicker-v34">Ficha lista para cliente</span><h2>${escapeHtml(p.name)}</h2><p class="share-description">${escapeHtml(p.description||'Producto disponible para entrega. Consulta disponibilidad antes de confirmar tu pedido.')}</p></div>
        <div class="client-proof-v36"><div><small>Atención directa</small><b>WhatsApp +504 3151-7755</b></div><div><small>Compra clara</small><b>Precio + envío visibles</b></div><div><small>Confirmación</small><b>Antes de despachar</b></div></div>
        ${promoMsg?`<div class="share-offer-v32 share-offer-v35">🎁 ${escapeHtml(promoMsg)}</div>`:''}
        <div class="public-metrics public-metrics-v32 public-metrics-v35"><div><span>Solo producto</span><b id="metricProduct">${money(productTotal)}</b><small>${money(unitPrice)} c/u</small></div><div><span>Envío normal</span><b id="metricNormal">${money(normalTotal)}</b><small>Producto + Lps.110</small></div><div><span>Al recibir + 6%</span><b id="metricCod">${money(codTotal)}</b><small>Lps.100 + comisión</small></div></div>
        <div class="total-recommend-v36"><span>Total recomendado para confirmar</span><b>${money(normalTotal)}</b><small>Envío Normal: producto + Lps.110. Para Pagar al Recibir usar el total verde de comisión.</small></div>
        <div class="order-steps-v35 order-steps-v36"><b>Para pedir este producto</b><span>1. Confirmar cantidad</span><span>2. Enviar datos completos</span><span>3. Aprobar total con envío</span></div>
        <div class="client-ready-v32 client-ready-v35"><b>Datos necesarios</b><span>Nombre completo</span><span>Departamento y municipio</span><span>Dirección exacta con referencia</span><span>Empresa: Forza, C807 o Cargo Expreso</span></div>
        <div class="process-row process-row-v32 process-row-v35"><div class="process-card"><b>Envío Normal</b><span>Producto + Lps. 110. Pago por depósito, transferencia bancaria o Tigo Money.</span></div><div class="process-card"><b>Pagar al Recibir</b><span>Producto + Lps. 100 de envío + 6% de comisión. Total redondeado en lempiras.</span></div></div>
        <div class="share-footer share-footer-v32 share-footer-v35">SD COMAYAGUA · Pedidos por WhatsApp · +504 3151-7755</div>
      </div>`;
    }
    openModal(`<div class="modal-head"><h3>Producto</h3><button class="close">×</button></div><div class="modal-body"><div id="shareCardMount">${cardHTML()}</div><div class="client-quote-panel client-quote-panel-v35 no-print"><div><b>Precio rápido para cliente</b><span>Cambia la cantidad antes de descargar, compartir o enviar por WhatsApp.</span></div><div class="qty-presets"><button data-setqty="1">1</button><button data-setqty="2">2</button><button data-setqty="3">3</button><button data-setqty="5">5</button><button data-setqty="10">10</button><button data-setqty="12">12</button><button data-setqty="20">20</button><button data-setqty="24">24</button></div><div class="qty-control"><button id="detailQtyMinus">−</button><input id="detailQtyInput" type="number" min="1" value="${shareQty}" inputmode="numeric"><button id="detailQtyPlus">+</button></div></div><div class="thumb-row">${safeImgs.map((img,i)=>`<button class="thumb ${i===0?'active':''}" data-product-img="${escapeHtml(img)}"><img src="${escapeHtml(img)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"></button>`).join('')}</div><div class="modal-actions product-actions product-actions-grid no-print"><button class="btn secondary prod-action" id="downloadProductPhoto"><span class="ico">▣</span><b>Imagen</b><small>Descargar</small></button><button class="btn prod-action" id="shareProductPhoto"><span class="ico">↗</span><b>Foto</b><small>Compartir</small></button><button class="btn secondary prod-action" id="waProductText"><span class="ico">✎</span><b>Texto</b><small>WhatsApp</small></button><button class="btn prod-action" data-action="sellProduct" data-id="${escapeHtml(id)}"><span class="ico">🛒</span><b>Vender</b><small>Venta real</small></button><button class="btn secondary prod-action" data-action="quoteProduct" data-id="${escapeHtml(id)}"><span class="ico">🧾</span><b>Cotizar</b><small>Precio total</small></button><button class="btn secondary prod-action" data-action="marketingProduct" data-id="${escapeHtml(id)}"><span class="ico">#</span><b>Textos</b><small>Marketplace</small></button><button class="btn ghost prod-action" data-action="editProduct" data-id="${escapeHtml(id)}"><span class="ico">✦</span><b>Editar</b><small>Producto</small></button></div></div>`);
    function refreshShareCard(){
      const currentImg=$('#detailMainImage',modalRoot)?.src || safeImgs[0];
      $('#shareCardMount',modalRoot).innerHTML=cardHTML();
      const img=$('#detailMainImage',modalRoot); if(img) img.src=currentImg;
  ...
  function docCard(doc,isSale){
    ...
    const rows=(doc.items||[]).map((it,i)=>{
      ...
      const promo=promoTotalForQty(p,qty)!==null;
      const img=escapeHtml(it.image||productImage(p)||placeholderFor(p));
      return `<div class="doc-item-card-v32 doc-item-card-v35"><img src="${img}" alt="${escapeHtml(it.name)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div class="doc-item-main-v32"><div class="doc-prod-line"><span>#${i+1}</span><b>${escapeHtml(it.name)}${promo?`<small class="doc-promo-note">Oferta aplicada</small>`:''}</b></div><div class="doc-prod-metrics"><div><small>Cant.</small><b>${num(qty)}</b></div><div><small>Precio C/U</small><b>${money(unit)}</b></div><div><small>Total</small><b>${money(total)}</b></div></div></div></div>`;
    }).join('')||'<div class="doc-item-empty">Sin productos agregados</div>';
    return `<div class="doc-wrap doc-v32 doc-v35" id="printableDoc">
      <div class="doc-hero-v32 doc-hero-v35"><div class="doc-brand-v32"><img src="assets/logo-sdc.png" alt="Logo"><div><small>${isSale?'Recibo / factura de compra':'Cotización para cliente'}</small><h2>SD COMAYAGUA</h2><p>${date} · ${escapeHtml(code)}</p></div></div><div class="doc-status-v32 ${isSale?'sale':'quote'}"><span>${isSale?'Venta registrada':'Cotización'}</span><b>${money(c.total)}</b></div></div>
      <div class="doc-client-strip-v32 doc-client-strip-v35"><div><span>Cliente</span><b>${escapeHtml(doc.client||'Cliente')}</b></div><div><span>Teléfono</span><b>${escapeHtml(doc.phone||'No registrado')}</b></div><div><span>Ubicación</span><b>${escapeHtml(location)}</b></div><div><span>Entrega</span><b>${escapeHtml(doc.company||'No seleccionada')}</b></div>${doc.reference?`<div class="wide"><span>Referencia</span><b>${escapeHtml(doc.reference)}</b></div>`:''}</div>
      <div class="doc-pay-v32 doc-pay-v35 doc-pay-v36"><div><small>Proceso de pago</small><b>${payTitle}</b><p>${process}</p></div><span>${contactReady?'Datos básicos completos':'Faltan datos del cliente'}</span></div>
      <div class="doc-confirm-v36"><div><small>Código</small><b>${escapeHtml(code)}</b></div><div><small>Total final</small><b>${money(c.total)}</b></div><div><small>WhatsApp</small><b>+504 3151-7755</b></div></div>
      <div class="doc-items-v32"><div class="doc-items-title-v32"><span>Productos cotizados</span><b>${num((doc.items||[]).length)}</b></div>${rows}</div>
      <div class="doc-mini-summary doc-summary-v32 doc-summary-v35"><div><span>Productos</span><b>${money(c.products)}</b></div><div><span>Envío</span><b>${money(c.shipping)}</b></div><div><span>Comisión</span><b>${money(c.commission)}</b></div><div><span>Descuento</span><b>${money(c.discount)}</b></div><div class="grand"><span>Total a pagar</span><b>${money(c.total)}</b></div></div>
      <div class="doc-route-v35"><div><b>1</b><span>Confirmar datos</span></div><div><b>2</b><span>Confirmar pago o entrega</span></div><div><b>3</b><span>Preparar pedido</span></div></div>
      <div class="doc-next-v34 doc-next-v35"><b>Siguiente paso</b><p>Confirme el total, la dirección y el método de entrega. Si algún dato está incorrecto, avisar antes de despachar el pedido.</p></div>
      <div class="doc-required-v32 doc-required-v35"><b>Antes de confirmar, revise:</b><span>Producto y cantidad</span><span>Total con envío</span><span>Empresa de entrega</span><span>Dirección con referencia</span></div>
      <p class="doc-note compact-note">${isSale?'Gracias por comprar en SD Comayagua.':'Cotización informativa válida según disponibilidad de inventario y confirmación final.'} WhatsApp: +504 3151-7755</p>
    </div>`;
  }
