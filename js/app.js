(function(){
  ...
  function hero(){
    ...
    return `<section class="hero" id="inicio">
      <div class="private-hero-head private-hero-head--no-logo"><div><div class="pill"><span class="dot"></span> Panel privado</div><h2>CONTROL SDC</h2><p>Solo usted entra aquí. El cliente únicamente recibe la foto, cotización o texto que le comparte por WhatsApp.</p></div></div>
      <div class="stats">
  ...
  function cardModePanel(){
    ...
    return `<section class="view-mode-panel no-print"><div class="view-mode-copy"><b>Vista privada / cliente</b><span>${captureActive?'Modo captura activo. Toque SALIR para volver a la vista normal.':(mode==='admin'?'Admin: costos, inversión y ganancia solo para usted.':'Cliente: precio, descripción y totales listos para enviar.')}</span></div><div class="view-mode-buttons"><button class="${mode==='admin'&&!captureActive?'active':''}" data-action="cardAdmin">ADMIN</button><button class="${mode==='client'&&!captureActive?'active':''}" data-action="cardClient">CLIENTE</button><button class="${captureActive?'active capture-live':''}" data-action="captureClean">${captureActive?'SALIR':'CAPTURA'}</button></div></section>`
  }
