...
(function(){
  ...
  function isVerifiableProductWrite(payload={}){
    ...
    return ['upsertproduct','patchproduct','saveproduct','setactive','updatestock','adjuststock','addgalleryimages','setmainimage'].includes(action);
  }
  ...
  async function saveProductToSheets(product, previousId=''){
    ...
    const originalCode=String(previousId || product?.id || '').trim();
    const newCode=String(product?.id || product?.codigo || originalCode).trim();
    const result=await sheetPost({
      action:'upsertProduct',
      codigo:originalCode,
      previousCodigo:originalCode,
      product:{...product, codigo:newCode, previousCodigo:originalCode}
    },{verifyProductCode:newCode});
    return result || true;
  }
  ...
  function openProductEditor(id){
    ...
    function drawImages(){
      ...
      $$('[data-upload-image]',modalRoot).forEach(inp=>inp.onchange=async()=>{const i=+inp.dataset.uploadImage; const file=inp.files&&inp.files[0]; if(!file)return; try{toast('Optimizando imagen para Google Sheets...'); imageRows[i]=await imageFileToDataURL(file, i===0?900:460, i===0?0.72:0.54, i===0?42000:9000); drawImages(); toast(`Imagen ${i+1} lista para guardar.`);}catch(err){console.error(err); toast(err.message||'No se pudo cargar la imagen.');}});
    ...
    function setSaveButtonBusy(btn,busy,label){
      if(!btn) return;
      if(busy){
        btn.dataset.originalText=btn.dataset.originalText || btn.textContent || 'Guardar producto';
        btn.disabled=true;
    ...
    $('#saveProduct').onclick=async()=>{
      ...
      const rawImages=$$('.pImageUrl',modalRoot).map(inp=>inp.value.trim()).filter(Boolean);
      setProductSaveStatus('Optimizando imágenes para Google Sheets...', 'info');
      ...
      try{
        ...
        if(getSheetApiUrl()){
          setSaveButtonBusy(btn,true,'Sincronizando...');
          setProductSaveStatus('Enviando y verificando en Google Sheets...', 'info');
          await saveProductToSheets(np,id||np.id);
          state.settings.lastSheetSync=new Date().toISOString();
          save();
          setProductSaveStatus('Producto guardado y confirmado en Google Sheets.', 'ok');
          closeModal(); render(); toast('✅ Producto guardado y sincronizado con Google Sheets.');
        }else{
          closeModal(); render(); toast('Producto guardado localmente. Configura la URL de Apps Script para sincronizar.');
        }
      }catch(err){
        console.warn('Sheets save failed',err);
        setProductSaveStatus('No se confirmó Google Sheets: '+(err.message||err)+'. El producto quedó guardado en este dispositivo. Toca Guardar otra vez para reintentar.', 'error');
        toast('No se confirmó Google Sheets. Revisa el mensaje del formulario.');
      }finally{
        productSaving=false;
  ...
  // Exponer funciones para que parches externos (sdc-v70, etc.) puedan sincronizar Sheets.
  window.saveProductToSheets = saveProductToSheets;
  window.syncProductsFromSheets = syncProductsFromSheets;
