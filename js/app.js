/* SDC V61 Mobile POS Pro: estructura limpia, móvil primero. */
(function(){
  ...
  function normalizeSheetRemoteProduct(row,i=0){
    const p=SDCStore.normalizeProduct({
      ...
      categories: row.categoria || row.categorias || row.category || row.categories || '',
  ...
  async function sheetPost(payload={}){
    const base=getSheetApiUrl();
    if(!base) throw new Error('No hay URL /exec de Apps Script configurada.');
    const body={sheetId:getSheetId(),productSheet:getProductSheetName(),adminKey:state.settings.accessKey||'',origin:'github-pages-mobile',...payload};
    const raw=JSON.stringify(body);
    const request={method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:raw,redirect:'follow'};

    try{
      const res=await fetch(base,request);
      const txt=await res.text();
      let data;
      try{data=JSON.parse(txt)}catch(e){throw new Error('Apps Script no devolvió JSON válido.')}
      if(!data.ok) throw new Error(data.error||'No se pudo guardar en Google Sheets.');
      return data;
    }catch(err){
      const message=String(err && err.message || err).toLowerCase();
      const looksLikeCors=/failed to fetch|load failed|networkerror|cors|blocked|json válido|json valido/.test(message);
      if(!looksLikeCors) throw err;

      // GitHub Pages + Apps Script a veces bloquea la lectura de la respuesta por CORS,
      // aunque el POST sí llega. Reintentamos en no-cors y marcamos como enviado.
      await fetch(base,{...request,mode:'no-cors',keepalive:raw.length < 55000});
      return {ok:true,opaque:true,warning:'Enviado a Apps Script en modo no-cors. Use Sync para verificar.'};
    }
  }
  async function syncProductsFromSheets(opts={}){
    ...
    try{
      ...
      const products=raw.filter(sheetRowHasProduct).map(normalizeSheetRemoteProduct).filter(isRealProduct);
  ...
  async function saveProductToSheets(product, previousId=''){
    ...
    const currentCode=String(product?.id || previousId || '').trim();
    const previousCode=String(previousId || currentCode).trim();
    if(!currentCode) throw new Error('Producto sin código.');
    const result=await sheetPost({
      action:'upsertProduct',
      codigo:currentCode,
      previousCodigo:previousCode,
      allowClearImages:false,
      applyFormulas:true,
      product:{...product,codigo:currentCode,id:currentCode,previousCodigo:previousCode}
    });
    state.settings.lastSheetSync=new Date().toISOString();
    save();
    return result && result.ok !== false;
  }
  window.SDC_saveProductToSheets=saveProductToSheets;
  window.SDC_syncProductToSheets=saveProductToSheets;
  ...
  function applyAppearance(){
    state.settings.appearance='gamer';
    document.body.classList.remove('pro-mode','pro-white-mode');
  ...
  function topbar(){
    return `<header class="topbar"><img class="top-logo" src="${LOGO_SRC}" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1></div><div class="spacer"></div><button class="btn small ghost sync-status sync-icon-only" data-action="sync" title="Sincronizar con Google Sheets" aria-label="Sincronizar con Google Sheets"><span class="sync-dot"></span><span class="sync-icon">↻</span><span class="sync-label">${lastSyncLabel()}</span></button><button class="btn small secondary logout-btn logout-icon-btn" data-action="lock" title="Salir del panel" aria-label="Salir del panel"><span class="logout-door" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M9 4h7.2c.9 0 1.6.7 1.6 1.6v12.8c0 .9-.7 1.6-1.6 1.6H9"/><path d="M9 3.8 5.4 5.2v13.6L9 20.2V3.8Z"/><path d="M12.2 12h7"/><path d="m17 9.8 2.2 2.2-2.2 2.2"/></svg></span><span class="logout-text">Salir</span></button></header>`
  }
  function hero(){
    const st=stats();
    return `<section class="hero v51-hero hero-v56-clean" id="inicio">
      <div class="v51-hero-shell">
        <div class="private-hero-head private-hero-head--actions-only">
          <div class="v51-hero-actions no-print">
            <button type="button" data-action="sell">Vender</button>
            <button type="button" data-action="quote" class="secondary">Cotizar</button>
            <button type="button" data-action="newProduct" class="secondary hero-new-product">+ Producto</button>
          </div>
        </div>
        <div class="stats">
          <div class="stat"><b>${num(st.count)}</b><span>Productos activos</span></div><div class="stat"><b>${num(st.stock)}</b><span>Unidades en stock</span></div>
          <div class="stat"><b>${moneyPrivate(st.value)}</b><span>Valor de venta</span></div><div class="stat"><b>${moneyPrivate(st.invested)}</b><span>Invertido</span></div>
          <div class="stat"><b>${moneyPrivate(st.profit)}</b><span>Ganancia estimada</span></div>
        </div>
      </div>
    </section>`
  }

  function stats(){const base=activeProducts(); let count=base.length,stock=0,value=0,invested=0; base.forEach(p=>{stock+=+p.stock||0; value+=(+p.stock||0)*(+p.price||0); invested+=(+p.stock||0)*(+p.cost||0)}); return {count,stock,value,invested,profit:value-invested}}
  ...
  function sheetsButtonHTML(){
    ...
    return `<a class="btn small ghost sheets-head-btn" href="${sheetUrl}" target="_blank" rel="noopener" title="Abrir Google Sheets" aria-label="Abrir Google Sheets"><svg class="sheets-icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2" fill="#1a7d4a"/><rect x="7" y="7" width="4" height="2" rx=".5" fill="#fff"/><rect x="13" y="7" width="4" height="2" rx=".5" fill="#fff"/><rect x="7" y="11" width="4" height="2" rx=".5" fill="#fff"/><rect x="13" y="11" width="4" height="2" rx=".5" fill="#fff"/><rect x="7" y="15" width="4" height="2" rx=".5" fill="#fff"/><rect x="13" y="15" width="4" height="2" rx=".5" fill="#fff"/></svg><span>Sheets</span></a>`;
  }
  ...
  function quickPanel(){
    ...
    return `<section class="quick no-print quick-v22 quick-private quick-panel quick-panel-v72">
      <div class="quick-grid quick-grid-v72" aria-label="Acciones rápidas del panel">
        <button class="quick-btn" data-action="cardClient"><small>Vista</small><b>Cliente</b></button>
        <button class="quick-btn" data-action="captureClean"><small>Modo</small><b>Captura</b></button>
        <button class="quick-btn" data-action="catalog"><small>Inicio</small><b>Catálogo</b></button>
        <button class="quick-btn" data-action="quickSale"><small>Venta</small><b>Rápida</b></button>
        <button class="quick-btn" data-action="quotes"><small>Cotización</small><b>Guardadas</b></button>
        <button class="quick-btn" data-action="clients"><small>Agenda</small><b>Clientes</b></button>
        <button class="quick-btn" data-action="receipts"><small>Ventas</small><b>Caja</b></button>
        <button class="quick-btn" data-action="profit"><small>Utilidad</small><b>Ganancia</b></button>
        <button class="quick-btn" data-action="backup"><small>Copia</small><b>Respaldo</b></button>
        <button class="quick-btn" data-action="lowStock"><small>Alerta</small><b>Bajo stock</b></button>
        <button class="quick-btn" data-action="noCost"><small>Revisar</small><b>Sin costo</b></button>
        <button class="quick-btn" data-action="sync"><small>Datos</small><b>Sync</b></button>
      </div>
    </section>
    <section class="alert-row no-print alert-row-v72">
      <div class="alert-grid alert-grid-v72">
        <div class="alert-card alert-card-v72"><i>📦</i><div><b>${low} bajo stock</b><span>Productos que conviene reponer.</span></div><button class="btn small secondary" data-action="lowStock">Ver</button></div>
        <div class="alert-card alert-card-v72"><i>🧾</i><div><b>${nocost} sin costo</b><span>Agrega costo para ganancia real.</span></div><button class="btn small secondary" data-action="noCost">Revisar</button></div>
        <div class="alert-card alert-card-v72"><i>📉</i><div><b>${lowMargin} ganancia baja</b><span>Menos de Lps. 10 por unidad.</span></div><button class="btn small secondary" data-action="lowProfit">Detalle</button></div>
        <div class="alert-card alert-card-v72"><i>💰</i><div><b>${moneyPrivate(st.profit)}</b><span>Ganancia estimada.</span></div><button class="btn small secondary" data-action="profit">Ver</button></div>
        <div class="alert-card alert-card-v72"><i>🔒</i><div><b>${state.settings.moneyLocked?'Ganancias ocultas':'Ganancias visibles'}</b><span>Protege costos y utilidad.</span></div><button class="btn small secondary" data-action="moneyLock">${state.settings.moneyLocked?'Mostrar':'Ocultar'}</button></div>
      </div>
    </section>`
  }
  ...
  function pageFooter(){
    return `<footer class="sdc-page-footer no-print footer-v54"><p>Derechos reservados</p><b>Hecho por: Gabriel Guerrero.</b></footer>`
  }
  ...
  function filteredProducts(){
    ...
    return activeProducts().filter(p=>{
      ...
      if(filter.special==='lowStock') ok = ok && Number(p.stock)>0 && Number(p.stock)<=Number(state.settings.lowStockLimit||3);
      if(filter.special==='noCost') ok = ok && !(Number(p.cost||0)>0);
      if(filter.special==='lowProfit') ok = ok && Number(p.price||0)>0 && (Number(p.price||0)-Number(p.cost||0))<10;
      return ok;
    })
  }
  ...
  function productCard(p){
    const st=status(p);
    const isOut=st.cls==='out' || Number(p.stock||0)<=0;
    ...
    const stockLine=isOut?'Sin unidades disponibles':`En stock: <b>${num(p.stock||0)}</b> unidades`;
    return `<article class="product-card product-card-v49 ${inventoryLayout()==='one'?'wide':'compact'} ${isOut?'is-out is-agotado':''}" data-id="${id}" data-product-id="${id}" data-stock-status="${st.cls}">
      ...
      <div class="v49-card-body">
        ...
        <div class="v49-card-actions v49-card-actions-single v53-card-actions">
          <button type="button" class="v53-view-only" data-action="viewProduct" data-id="${id}">VER</button>
        </div>
      </div>
  ...
  function imageFileToDataURL(file, maxSize=900, quality=0.78){
    return new Promise((resolve,reject)=>{
      ...
      const MAX_DATA_URL_CHARS=45000; // Google Sheets acepta celdas de hasta ~50k caracteres; dejamos margen.
      ...
      reader.onload=()=>{
        ...
        img.onload=()=>{
          ...
          let side=Math.max(420,Number(maxSize)||900);
          let q=Math.min(.86,Math.max(.42,Number(quality)||.78));
          let data='';
          ...
          for(let attempt=0; attempt<12; attempt++){
            ...
            if(data.length<=MAX_DATA_URL_CHARS) break;
            if(q>.48) q=Math.max(.48,q-.10);
            else side=Math.max(360,Math.round(side*.82));
          }

          if(data.length>MAX_DATA_URL_CHARS){
            reject(new Error('La imagen sigue muy pesada para Google Sheets. Use una foto más liviana o un enlace de imagen.'));
            return;
          }
          resolve(data);
        };
  ...
  function openProductEditor(id){
    ...
    function drawImages(){
      $('#imageRows',modalRoot).innerHTML=imageRows.map((url,i)=>`<div class="mini-row image-row image-row-upload"><span class="row-index">Imagen ${i+1}</span><input class="input pImageUrl" value="${escapeHtml(url)}" placeholder="Enlace o imagen subida"><label class="btn small secondary upload-image-btn">Galería<input data-upload-image="${i}" type="file" accept="image/*" hidden></label><button class="btn small ghost" data-delimage="${i}" type="button">×</button></div>`).join('');
      $$('.pImageUrl',modalRoot).forEach((inp,i)=>inp.oninput=()=>{imageRows[i]=inp.value});
      $$('[data-upload-image]',modalRoot).forEach(inp=>inp.onchange=async()=>{const i=+inp.dataset.uploadImage; const file=inp.files&&inp.files[0]; if(!file)return; try{toast('Procesando imagen de galería...'); imageRows[i]=await imageFileToDataURL(file); drawImages(); toast(`Imagen ${i+1} cargada desde galería.`);}catch(err){console.error(err); toast(err.message||'No se pudo cargar la imagen.');}});
    ...
    $('#saveProduct').onclick=async()=>{
      ...
      const originalText=btn?btn.textContent:'Guardar producto';
      if(btn){btn.disabled=true; btn.textContent='Guardando...';}
      const images=$$('.pImageUrl',modalRoot).map(inp=>inp.value.trim()).filter(Boolean);
      const tooLarge=images.find(src=>/^data:image\//i.test(src) && src.length>49000);
      if(tooLarge){
        if(btn){btn.disabled=false; btn.textContent=originalText;}
        toast('La imagen es demasiado pesada para Google Sheets. Vuelva a subirla o use un enlace.');
        return;
      }
      ...
      const np={id:$('#pId').value.trim()||nextCode(),name:$('#pName').value.trim()||'Producto sin nombre',categories:$('#pCats').value.trim(),cost:readMoneyField('#pCost'),price:readMoneyField('#pPrice'),stock:readStockField(),image:images[0]||'',gallery:images.slice(1).join('\n'),promos,description:$('#pDesc').value.trim(),active:true};
      ...
      let remoteOk=false;
      try{remoteOk=await saveProductToSheets(np,id||np.id)}catch(err){console.warn('Sheets save failed',err); toast('Guardado local. Sheets falló: '+(err.message||err));}
      closeModal(); render(); toast(remoteOk?'Producto guardado y enviado a Google Sheets.':'Producto guardado localmente. Revisa conexión con Sheets.');
    };
  ...
  function productDetailHTML(p){
    ...
    const gainUnit=Math.max(0,Number(p.price||0)-Number(p.cost||0));
    const gainTotal=Math.max(0,gainUnit*Number(p.stock||0));
    ...
    return `<div class="modal-head v49-detail-head"><div><span class="sdc-safe-pill detail-pill hn-time-pill" id="hnLiveTime">${nowHNPanel()}</span><h3>${escapeHtml(p.name)}</h3></div><button class="close">×</button></div>
      <div class="modal-body v49-product-detail ${st.cls==='out'?'is-out':''}">
        <div class="v49-detail-hero ${st.cls==='out'?'is-out':''}">
          ...
          <div class="v49-detail-main"><small>${escapeHtml(firstTag(p))} · ${escapeHtml(p.id||'')}</small><b>${money(p.price)}</b><p>${escapeHtml(productDescription(p))}</p></div>
        </div>
        <div class="v49-tabbar" role="tablist">
        ...
        <section class="v49-tab active" data-panel="cliente">
          ...
          <div class="v49-benefits"><b>Beneficios para cliente</b><p>${escapeHtml(productDescription(p))}</p></div>
        </section>
        <section class="v49-tab" data-panel="admin">
          <div class="v49-admin-grid">
            <div><span>Stock</span><b>${num(p.stock)}</b></div>
            ...
            <div class="full"><span>Si vende todo el stock</span><b>${money(sellAll)}</b></div>
          </div>
        </section>
        ...
        <div class="modal-actions v49-detail-actions" style="position:static">
          <button class="btn v53-modal-quote" type="button" id="v49QuoteProduct">Cotizar</button>
          <button class="btn v53-modal-sell" type="button" id="v49SellProduct" ${st.cls==='out'?'disabled title="Producto agotado"':''}>${st.cls==='out'?'Agotado':'Vender'}</button>
  ...
  function bindProductDetails(p){
    ...
    $('#v49SellProduct',modalRoot)?.addEventListener('click',()=>{if(Number(p.stock||0)<=0){toast('Producto agotado. Primero aumenta el stock en Editar.'); return;} closeModal();openSale(p.id);});
  ...
  function renderPicker(isSale){
    ...
    function draw(){
      ...
      const allItems=activeProducts().filter(p=>{
        const okCat=!cat || productTags(p).map(localNorm).includes(catKey);
      ...
      list.innerHTML=(items.map(p=>`<div class="picker-item"><img src="${escapeHtml(productImage(p))}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div><b>${escapeHtml(p.name)}</b><span>${money(p.price)} · Stock ${num(p.stock)} · ${escapeHtml(firstTag(p))}</span></div><button class="btn small add-pick-btn" type="button" data-additem="${escapeHtml(p.id)}">Seleccionar</button></div>`).join('')+limitNote)||'<div class="picker-empty"><b>Sin productos para mostrar</b><span>Pruebe otra categoría o revise el texto de búsqueda.</span></div>';
    ...
    $$('[data-pickcat]',modalRoot).forEach(b=>b.onclick=()=>{cat=b.dataset.pickcat;$$('[data-pickcat]',modalRoot).forEach(x=>x.classList.toggle('active',x===b));draw()});
    draw();
  }
  ...
  function giftItemHTML(it,i){
    const qty=Math.max(1,Number(it.qty)||1);
    const p=itemProductRef(it);
    return `<div class="cart-row cart-row-v24 gift-row"><div class="cart-info"><b>🎁 ${escapeHtml(it.name)}</b><span>Regalo · Stock actual ${num(p.stock||0)}</span></div><div class="qtybox"><button data-gift-dec="${i}">−</button><input data-gift-qty="${i}" type="number" value="${qty}"><button data-gift-inc="${i}">+</button></div><button class="btn small danger remove-item" data-gift-rem="${i}">Quitar</button></div>`;
  }
  function addGiftItem(id,isSale){
    const p=productById(id); if(!p)return;
    const doc=isSale?saleDraft:quote; doc.gifts=Array.isArray(doc.gifts)?doc.gifts:[];
    const found=doc.gifts.find(x=>x.id===id);
    if(found) found.qty=Math.max(1,(+found.qty||1)+1);
    else doc.gifts.push({id:p.id,name:p.name,price:0,cost:+p.cost||0,qty:1,image:productImage(p)});
    refreshQuoteUI(isSale);
    toast(`${p.name} agregado como regalo.`);
  }
  function bindGiftPicker(isSale){
    ...
    function draw(){
      ...
      list.innerHTML=items.map(p=>`<div class="picker-item gift-pick-item"><img src="${escapeHtml(productImage(p))}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div><b>${escapeHtml(p.name)}</b><span>Stock ${num(p.stock)} · ${escapeHtml(firstTag(p))}</span></div><button class="btn small add-pick-btn" type="button" data-giftadd="${escapeHtml(p.id)}">Regalar</button></div>`).join('') || '<div class="picker-empty"><b>Sin productos</b><span>Escriba otro nombre o código.</span></div>';
  ...
  function docCard(doc,isSale){
    ...
    const statusText=isSale?'Venta confirmada':'Pendiente';
    ...
    const giftRows=(doc.gifts||[]).map((it,i)=>{
      ...
      const thumb=realImg?`<img class="receipt-item-thumb" src="${escapeHtml(realImg)}" alt="${escapeHtml(it.name)}" loading="eager" crossorigin="anonymous" referrerpolicy="no-referrer" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'receipt-item-thumb receipt-thumb-fallback',textContent:'${initials}'}))">`:`<div class="receipt-item-thumb receipt-thumb-fallback">${initials}</div>`;
      return `<div class="receipt-item-pro has-thumb gift-receipt-row"><div class="receipt-item-index">🎁</div>${thumb}<div class="receipt-item-info"><b>${escapeHtml(it.name)}</b><span>${num(qty)} ${qty===1?'unidad':'unidades'} · Regalo incluido</span></div><strong>Lps. 0</strong></div>`;
    }).join('');
    ...
    return `<div class="doc-wrap compact-doc doc-v21 doc-v23 receipt-pro-v4 receipt-v32 receipt-v36 receipt-v49-tight ${isSale?'receipt-sale-v32':'receipt-quote-v32'}" id="printableDoc">
      ...
      <div class="receipt-inner-pro">
        ...
        <section class="receipt-products-pro">
          <div class="receipt-title-pro"><span>${productTitle}</span><b>${itemCount} ${itemCount===1?'artículo':'artículos'}${giftCount?` · ${giftCount} regalo${giftCount===1?'':'s'}`:''}</b></div>
          ${rows}${giftRows}
        </section>
        ...
        <footer class="receipt-footer-pro receipt-footer-clean">
  ...
  function finishSale(){
    ...
    function addToMap(map,it){ if(!it||!it.id)return; map.set(it.id,(map.get(it.id)||0)+(Math.max(1,+it.qty||1))); }
    (previous?.items||[]).forEach(it=>addToMap(prevQty,it));
    (previous?.gifts||[]).forEach(it=>addToMap(prevQty,it));
    const checkQty=new Map();
    (saleDraft.items||[]).forEach(it=>addToMap(checkQty,it));
    (saleDraft.gifts||[]).forEach(it=>addToMap(checkQty,it));
    for(const [id,qty] of checkQty.entries()){
      const p=productById(id);
      const diff=qty-(prevQty.get(id)||0);
      if(p && diff>Number(p.stock||0)){toast(`Stock insuficiente para ${p.name}. Disponible: ${num(p.stock)}.`); return;}
      const sold=(saleDraft.items||[]).some(it=>it.id===id);
    ...
    (saleDraft.items||[]).forEach(it=>addToMap(newQty,it));
    (saleDraft.gifts||[]).forEach(it=>addToMap(newQty,it));
  ...
  function shortReceiptLinesHTML(doc){
    return (doc.items||[]).map(it=>{const qty=Math.max(1,+it.qty||1); return `<div class="short-line item"><span>${escapeHtml(it.name)} <em>x${num(qty)}</em></span><b>${money(itemTotal(it))}</b></div>`}).join('');
  }
  function shortReceiptCardHTML(doc){
    const c=calc(doc);
    const variant=doc.receiptVariantLabel?`<span class="short-variant-pill">${escapeHtml(doc.receiptVariantLabel)}</span>`:'';
    return `<div class="short-receipt short-receipt-v34 short-receipt-v36 short-receipt-v72" id="shortReceiptCard"><div class="short-brand"><div class="short-logo-box"><span>SD</span><img class="receipt-logo-inline" src="${RECEIPT_LOGO_SRC}" alt="SD" onerror="this.style.display='none';this.parentElement.classList.add('logo-fallback-active')"></div><div><h2>SD COMAYAGUA</h2><p class="short-meta short-meta-v50"><span class="short-client-name">${escapeHtml(doc.client||'Cliente')}</span><em class="short-date-time">${nowHN()}</em></p>${variant}</div></div>${shortReceiptLinesHTML(doc)}<hr><div class="short-line"><span>Productos</span><b>${money(c.products)}</b></div><div class="short-line"><span>${shippingLabel(doc)}</span><b>${money(c.shipping)}</b></div>${c.commission?`<div class="short-line"><span>Comisión</span><b>${money(c.commission)}</b></div>`:''}<div class="grand short-line"><span>Total</span><b>${money(c.total)}</b></div><small>WhatsApp +504 3151-7755</small></div>`;
  }
  ...
  function openShortReceipt(isSale){
    const doc=currentDoc(isSale);
    const current=SDCStore.clone(doc||{});
    current.receiptVariantLabel=isCodDoc(current)?'Pagar al recibir':'Envío actual';
    openModal(`<div class="modal-head short-receipt-head"><h3>Recibo corto</h3><button class="close">×</button></div><div class="modal-body short-receipt-screen short-receipt-screen-v72"><div class="short-receipt-preview-wrap">${shortReceiptCardHTML(current)}</div><div class="short-receipt-options-v72"><div><b>Descargar con envío</b><span>Genera dos versiones listas: normal y pagar al recibir con comisión.</span></div><button class="btn secondary" id="downloadShortNormal">Recibo 1 · Normal</button><button class="btn" id="downloadShortCOD">Recibo 2 · Pagar al recibir</button></div><div class="modal-actions short-receipt-actions" style="position:static"><button class="btn secondary" id="backFromShortReceipt">← Atrás</button><button class="btn secondary" id="copyShortReceipt">Copiar texto</button><button class="btn" id="downloadShortCurrent">Descargar actual</button></div></div>`,true);
    ...
    $('#downloadShortCurrent')&&($('#downloadShortCurrent').onclick=()=>downloadShortReceiptImage(current,isCodDoc(current)?'actual-pagar-al-recibir':'actual-normal'));
    $('#downloadShortNormal')&&($('#downloadShortNormal').onclick=()=>downloadShortReceiptImage(shortReceiptVariant(doc,'Normal'),'recibo-1-envio-normal'));
    $('#downloadShortCOD')&&($('#downloadShortCOD').onclick=()=>downloadShortReceiptImage(shortReceiptVariant(doc,'COD'),'recibo-2-pagar-al-recibir'));
    $('#copyShortReceipt').onclick=()=>{
      navigator.clipboard?.writeText(shortReceiptText(current)); toast('Recibo corto copiado.');
    };
  }
