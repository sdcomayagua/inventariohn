...
(function(){
  ...
  async function saveProductToSheets(product, previousId=''){
    ...
    await sheetPost({
      ...
      product:{...product, previousCodigo:originalCode}
    });
    return true;
  }
  ...
  function imageFileToDataURL(file, maxSize=1400, quality=0.86){
    return new Promise((resolve,reject)=>{
  ...
  function openProductEditor(id){
    ...
    $('#saveProduct').onclick=async()=>{
      ...
      let remoteOk=false;
      try{remoteOk=await saveProductToSheets(np,id||np.id)}catch(err){console.warn('Sheets save failed',err)}
      closeModal(); render(); toast(remoteOk?'✅ Producto guardado en Google Sheets.': getSheetApiUrl()?'Producto guardado. Error al conectar con Sheets.':'Producto guardado localmente. Configura la URL de Apps Script en SDC_CONFIG para sincronizar.');
    };
