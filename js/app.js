...
(function(){
  ...
  function inventoryHeadHTML(count){return `<div class="section-head inventory-head-v31"><div class="inventory-head-title-row"><h2>Productos</h2></div><div class="inventory-head-actions inventory-head-actions-v72"><button class="inventory-new-btn inventory-new-btn-v73" data-action="newProduct" type="button"><span class="add-plus">+</span><span>Producto</span></button>${inventoryControls()}<div class="inventory-head-right">${sheetsButtonHTML()}<span class="count-pill">${count} resultados</span></div></div></div>`}
  ...
  function productCard(p){
    ...
    return `<article class="product-card product-card-v49 ${inventoryLayout()==='one'?'wide':'compact'}" data-id="${id}" data-product-id="${id}">
  ...
  function mainAction(e){
    ...
    if(a==='focusSearch'){ const search=$('#searchInput'); search?.scrollIntoView({behavior:'smooth',block:'center'}); setTimeout(()=>search?.focus({preventScroll:true}),250); return; }
  ...
  function openProductEditor(id){
    ...
    $('#autoDescBtn')&&($('#autoDescBtn').onclick=()=>{const temp={name:$('#pName')?.value||prod.name,id:$('#pId')?.value||prod.id,categories:$('#pCats')?.value||prod.categories,price:+($('#pPrice')?.value||prod.price)||0}; $('#pDesc').value=autoProductDescription(temp); toast('Descripción automática generada.');});
    $('#saveProduct').onclick=async()=>{
      ...
      const np={id:$('#pId').value.trim()||nextCode(),name:$('#pName').value.trim()||'Producto sin nombre',categories:$('#pCats').value.trim(),cost:+$('#pCost').value||0,price:+$('#pPrice').value||0,stock:+$('#pStock').value||0,image:images[0]||'',gallery:images.slice(1).join('\n'),promos,description:$('#pDesc').value.trim(),active:true};
    ...
    return `<div class="modal-head v49-detail-head"><div><span class="sdc-safe-pill detail-pill hn-time-pill" id="hnLiveTime">${nowHNPanel()}</span><h3>${escapeHtml(p.name)}</h3></div><button class="close">×</button></div>
      <div class="modal-body v49-product-detail">
        ...
        <div class="modal-actions v49-detail-actions" style="position:static">
          ...
          <button class="btn v53-modal-sell" type="button" id="v49SellProduct">Vender</button>
