(function(){
  ...
  const SDC_VERSION_LABEL = 'PRO ESTABLE V39';
  quote = emptyQuote();
  ...
  function autoProductSpecs(p={}){
    return {estado:'Nuevo',categoria:firstTag(p)||'General',pais:'Honduras',envios:'Normal Lps.110 · Pagar al Recibir Lps.100 + 6% · Local por definir'};
  }
  ...
  function productClientPhotoHTML(p, qty){
    ...
    const offer = getOfferPrice(p, q);
    const price = offer.price;
    const total = price * q;
    const normal = total + SHIPPING.NORMAL;
    const codBase = total + SHIPPING.COD_FEE;
    const cod = Math.round(codBase + codBase * SHIPPING.COD_RATE);
    const localText = 'Por definir';
    const desc = productDescription(p);
    const image = p.img || LOCAL_PLACEHOLDER;
    return `
      <div class="productPhotoClean" data-export="product-photo-clean">
        <div class="productPhotoHead">
          <img src="${LOGO_SRC}" alt="SD COMAYAGUA" crossorigin="anonymous">
          <div>
            <strong>SD COMAYAGUA</strong>
            <span>Producto para cliente</span>
          </div>
          <b>${stockStatus(p).text}</b>
        </div>
        <div class="productPhotoImageWrap">
          <img src="${image}" alt="${escapeHtml(p.nombre)}" crossorigin="anonymous" onerror="this.src='${LOCAL_PLACEHOLDER}'">
          <strong>${money(p.precio)}</strong>
        </div>
        <div class="productPhotoBody">
          <h2>${escapeHtml(p.nombre)}</h2>
          <p>${escapeHtml(desc)}</p>
          <div class="productPhotoQty">
            <span>Cantidad consultada</span>
            <b>${q} ${q===1?'unidad':'unidades'} · ${money(total)}</b>
          </div>
          <div class="productPhotoPrices">
            <div><span>Envío normal</span><b>${money(normal)}</b><small>Producto + Lps. 110</small></div>
            <div><span>Pagar al recibir</span><b>${money(cod)}</b><small>Producto + Lps. 100 + 6%</small></div>
            <div><span>Envío local</span><b>${localText}</b><small>Costo por definir</small></div>
          </div>
          <div class="productPhotoNote">${offer.applied ? `🎁 Oferta aplicada: ${money(price)} c/u desde ${offer.min} unidades` : 'Precio sujeto a disponibilidad.'}</div>
        </div>
  ...
  async function downloadProductPhotoDirect(id, btn){
    ...
    host.innerHTML = productClientPhotoHTML(p, qty);
    ...
    try{
      ...
      const filename = `producto-${slugFile(p.nombre||p.id||'producto')}-${Date.now()}.png`;
      downloadBlob(blob, filename);
      toast('Foto del producto descargada sin barra del navegador.');
    }catch(err){
      console.error(err);
      toast('No se pudo descargar la foto. Abra VER y use descargar imagen.');
    }finally{
      host.remove();
      if(btn){ btn.disabled = false; btn.textContent = original || 'FOTO'; }
    }
  }
  ...
  function openProductDetails(id){
    ...
    function cardHTML(){
      ...
      return `<div class="product-share-card product-share-card-v15" id="productShareCard"><div class="share-brand"><span class="share-brand-left"><img class="share-brand-logo" src="${LOGO_SRC}" alt="SD"><span>SD COMAYAGUA</span></span><b>Disponible</b></div><div class="product-share-media">${shareMedia}<b class="price-badge share-main-price">${money(p.price)}</b></div><h2>${escapeHtml(p.name)}</h2><p class="share-description">${escapeHtml(productDescription(p))}</p><div class="share-qty-line"><span>Cantidad consultada</span><b id="shareQtyText">${unitsLabel()} · ${money(productTotal)}${promoUsed()?' · promo aplicada':''}</b></div><div class="public-metrics public-metrics-v15"><div><span>Producto</span><b id="metricProduct">${money(productTotal)}</b></div><div><span>Envío normal</span><b id="metricNormal">${money(normalTotal)}</b></div><div><span>Al recibir + 6%</span><b id="metricCod">${money(codTotal)}</b></div><div><span>Envío local</span><b>Por definir</b></div></div><div class="process-row"><div class="process-card"><b>Envío Normal</b><span>Depósito o Tigo Money: producto + Lps. 110.</span></div><div class="process-card"><b>Pagar al Recibir</b><span>Producto + Lps. 100 de envío + 6% de comisión.</span></div><div class="process-card"><b>Envío Local</b><span>Costo por definir según zona o acuerdo.</span></div></div><div class="share-footer">SD COMAYAGUA · WhatsApp +504 3151-7755</div></div>`
    }
  ...
  function renderPicker(isSale){
    ...
    function draw(){
      ...
      const allItems=state.products.filter(p=>{
  ...
  // V44: refuerzo para que el botón VER abra siempre el detalle del producto.
  function bindViewProductFallback(){
    document.addEventListener('click', e=>{
      const btn=e.target.closest && e.target.closest('[data-action="viewProduct"]');
      if(!btn) return;
      ...
      else toast('No se encontró el producto.');
    }, true);
  }
