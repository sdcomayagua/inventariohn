...
(function(){
  ...
  function inventoryHeadHTML(count){
    ...
    return `<div class="section-head inventory-head-v31 inventory-head-v74 inventory-head-pro-v160 inventory-head-v163"><div class="inventory-titlebar-v74"><div><span class="inventory-kicker-v160">Catálogo digital</span><h2>Productos</h2><small class="inventory-subtitle-v163">Foto, precio, stock, colores y ofertas.</small></div><div class="inventory-head-right inventory-title-actions-v74">${sheetsButtonHTML()}<span class="count-pill">${count} resultados</span></div></div><div class="inventory-toolbar-v74 inventory-toolbar-v163 no-print"><button class="inventory-new-btn inventory-new-btn-v73 inventory-new-btn-v74" data-action="newProduct" type="button"><span class="add-plus">+</span><span>Producto</span></button><label class="inventory-inline-search-v74" for="inventorySearchInput"><span aria-hidden="true">⌕</span><input id="inventorySearchInput" data-product-search="1" placeholder="Buscar producto, código o categoría..." value="${escapeHtml(filter.q)}" autocomplete="off" inputmode="search"></label><label class="inventory-cat-select-v74" for="inventoryCategorySelect"><span aria-hidden="true">▾</span><select id="inventoryCategorySelect" aria-label="Filtrar por categoría">${inventoryCategoryOptionsHTML()}</select></label><button class="inventory-client-shot-v163" data-action="categoryCapture" type="button" title="Crear imagen limpia para cliente"><span>📸</span><b>Captura ${escapeHtml(catLabel)}</b></button><button class="inventory-client-print-v163" data-action="categoryPrint" type="button" title="Vista imprimible por categoría"><span>🖨️</span><b>Imprimir</b></button></div></div>`
  }
  ...
  function productCard(p){
    ...
    return `<article class="product-card product-card-v49 product-card-v163 ${inventoryLayout()==='one'?'wide':'compact'} ${isOut?'is-out is-agotado':''}" data-id="${id}" data-product-id="${id}" data-stock-status="${st.cls}">
      <button type="button" class="v49-card-photo v163-card-photo" data-action="viewProduct" data-id="${id}" aria-label="Ver detalle de ${escapeHtml(p.name)}">
      ...
      <div class="v49-card-body v163-card-body">
        <div class="v51-product-meta v163-product-meta"><span>${escapeHtml(cat||'Producto')}</span>${id?`<span>${id}</span>`:''}</div>
        <h3 data-action="viewProduct" data-id="${id}">${escapeHtml(p.name)}</h3>
        <div class="v163-price-stock-row"><strong class="v49-price ${isOut?'is-out':''}">${money(p.price)}</strong><span class="v163-stock-pill ${st.cls}">${isOut?'0 disponibles':`${num(stockQty)} disp.`}</span></div>
        <div class="v163-card-facts">
          ${colors?`<span><i>🎨</i>${escapeHtml(colors)}</span>`:''}
          ${offer?`<span class="offer"><i>🏷️</i>${escapeHtml(offer)}</span>`:''}
        </div>
        <div class="v49-card-actions v49-card-actions-single v53-card-actions v163-card-actions">
          ${adminButton}
          <button type="button" class="v53-view-only" data-action="viewProduct" data-id="${id}">Ver / vender</button>
        </div>
      </div>
