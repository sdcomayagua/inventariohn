(function(){
  ...
  function render(){
    ...
    bindMain();
    // Controles de cantidad en la vista Cliente desde la primera carga del catálogo.
    bindProductCards();
  }
  ...
  function topbar(){
    ...
    return `<header class="topbar topbar-v28"><img class="top-logo" src="assets/logo-sdc.png" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1><p><span class="mini-led ${cloud?'ok':'warn'}"></span>${turbo?'Pro Gamer':'Normal'} · ${cloud?'Nube activa':(cloudHasUrl()?'URL guardada':'Local')}</p></div><div class="spacer"></div><button class="btn small ghost cloud-btn" data-action="cloudSetup">☁</button><button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro Gamer'}</button>
...
WhatsApp SD COMAYAGUA: +504 3151-7755`;
  ...
  /* =========================================================
     Mejoras visuales para cliente, recibo y captura
  ...
  function render(){
    applyAppearance();
    if(!state.unlocked){renderLogin();return}
    app.className='app app-pro';
    app.innerHTML = `${topbar()}${hero()}${smartStatusPanel()}${executivePanel()}${businessPulse()}${qualityRoute()}${quickPanel()}${cardModePanel()}${searchPanel()}${categoryGallery()}${inventoryHTML()}${bottomNav()}`;
    bindMain();
    bindProductCards();
  }
  ...
  function topbar(){
    ...
    return `<header class="topbar topbar-v28 topbar-v35">
      ...
      <div class="top-title"><h1>SD COMAYAGUA</h1><p><span class="mini-led ${cloud?'ok':'warn'}"></span>${turbo?'Pro Gamer':'Normal'} · ${state.settings.compactMode?'Compacto':'Amplio'} · ${cloud?'Nube activa':(cloudHasUrl()?'URL guardada':'Local')}</p></div>
