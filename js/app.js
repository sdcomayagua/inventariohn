(function(){
  ...
  function cardModePanel(){
    ...
    return `<section class="view-mode-panel no-print"><div class="view-mode-copy"><b>Vista privada / cliente</b><span>${captureActive?'Modo captura activo. Toque SALIR para volver a la vista normal.':(mode==='admin'?'Admin: costos, inversión y ganancia solo para ti.':'Cliente: precio, descripción y totales listos para enviar.')}</span></div><div class="view-mode-buttons"><button class="${mode==='admin'&&!captureActive?'active':''}" data-action="cardAdmin">ADMIN</button><button class="${mode==='client'&&!captureActive?'active':''}" data-action="cardClient">CLIENTE</button><button class="${captureActive?'active capture-live':''}" data-action="captureClean">${captureActive?'SALIR':'CAPTURA'}</button></div></section>`
  }
  ...
  function openProductDetails(id){
    ...
    function cardHTML(){
      ...
      return `<div class="product-share-card product-share-card-v15" id="productShareCard"><div class="share-brand"><span class="share-brand-left"><img class="share-brand-logo" src="${LOGO_SRC}" alt="SD"><span>SD COMAYAGUA</span></span><b>Disponible</b></div><div class="product-share-media"><img id="detailMainImage" src="${escapeHtml(safeImgs[0])}" loading="eager" crossorigin="anonymous" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><b class="price-badge share-main-price">${money(p.price)}</b></div><h2>${escapeHtml(p.name)}</h2><p class="share-description">${escapeHtml(p.description||'Producto disponible para entrega. Consulta disponibilidad antes de confirmar tu pedido.')}</p><div class="share-qty-line"><span>Cantidad consultada</span><b id="shareQtyText">${unitsLabel()} · ${money(productTotal)}${promoUsed()?' · promo aplicada':''}</b></div><div class="public-metrics public-metrics-v15"><div><span>Producto</span><b id="metricProduct">${money(productTotal)}</b></div><div><span>Envío normal</span><b id="metricNormal">${money(normalTotal)}</b></div><div><span>Al recibir + 6%</span><b id="metricCod">${money(codTotal)}</b></div></div><div class="process-row"><div class="process-card"><b>Envío Normal</b><span>Deposita, transfiere o paga por Tigo Money: producto + Lps. 110.</span></div><div class="process-card"><b>Pagar al Recibir</b><span>Producto + Lps. 100 de envío + 6% de comisión. Total redondeado en lempiras.</span></div></div><div class="share-footer">SD COMAYAGUA · WhatsApp +504 3151-7755</div></div>`
    }
  ...
  function docCard(doc,isSale){
    ...
    const rows=(doc.items||[]).map((it,i)=>{
      ...
      return `<div class="receipt-item-pro has-thumb">
        ...
        <img class="receipt-item-thumb" src="${escapeHtml(img)}" alt="${escapeHtml(it.name)}" loading="eager" crossorigin="anonymous" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p||{name:it.name}))}'">
  ...
  async function docToBlob(){
    ...
    if(!blob) toast('No se pudo generar la imagen del documento. Verifica que las miniaturas hayan cargado o usa el botón PDF.');
    return blob;
  }
