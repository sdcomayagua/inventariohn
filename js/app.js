(function(){
  ...
  function renderLogin(){
    ...
    app.innerHTML=`<section class="login-card">
      ...
      <div class="version-pill">Sistema privado de ventas</div>
  ...
  function unlock(){ if($('#keyInput').value.trim()===(state.settings.accessKey || window.SDC_CONFIG?.accessKey || '')){state.unlocked=true;save();render();toast('Panel desbloqueado.')} else toast('Clave incorrecta.'); }
  function topbar(){const turbo=currentAppearance()==='turbo'; return `<header class="topbar"><img class="top-logo" src="assets/logo-sdc.png" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1><p>${turbo?'Modo Pro':'Modo normal'} · Sistema privado</p></div><div class="spacer"></div><button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro Gamer'}</button><button class="btn small ghost sync-btn" data-action="sync">Sync</button><button class="btn small secondary" data-action="lock">Salir</button></header>`}
  function hero(){
    const st=stats();
    return `<section class="hero" id="inicio">
      <div class="pill login-pill"><span class="dot"></span> SD Comayagua · Sistema privado</div>
      <h2>CONTROL DE VENTAS</h2><p>Inventario, cotizaciones, ventas, recibos editables, envíos y respaldo para trabajar rápido desde celular.</p><div class="hero-note">Panel optimizado para celular: inventario, cotizaciones, ventas y fotos listas para enviar al cliente.</div>
      <div class="stats">
  ...
  function quickPanel(){
    ...
    return `<section class="quick no-print quick-clean">
  ...
  function searchPanel(){
    return `<section class="search-panel search-stable clean-search no-print" id="searchPanel"><div class="search-title"><b>Buscar producto</b><span>Escribe sin que se suba la pantalla</span></div><div class="searchbar"><span class="icon">⌕</span><input id="searchInput" placeholder="Buscar producto, código, precio, categoría, cliente o teléfono" value="${escapeHtml(filter.q)}" autocomplete="off" inputmode="search"></div></section>`}
  ...
  function openProductDetails(id){
    ...
    function cardHTML(){
      ...
      return `<div class="product-share-card product-share-card-clean product-client-card-v2" id="productShareCard"><div class="share-brand"><span>SD COMAYAGUA</span><b>DISPONIBLE</b></div><div class="product-share-media"><img id="detailMainImage" src="${escapeHtml(safeImgs[0])}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><b class="price-badge share-main-price">${money(p.price)}</b></div><div class="share-info"><h2>${escapeHtml(p.name)}</h2><p class="share-description">${escapeHtml(p.description||'Producto disponible para entrega. Consulta disponibilidad antes de confirmar tu pedido.')}</p></div><div class="share-qty-line"><span>Cantidad consultada</span><b id="shareQtyText">${unitsLabel()} · ${money(productTotal)}${promoUsed()?' · oferta aplicada':''}</b></div><div class="public-metrics public-metrics-clean"><div><span>Producto</span><b id="metricProduct">${money(productTotal)}</b></div><div><span>Envío normal</span><b id="metricNormal">${money(normalTotal)}</b></div><div><span>Al recibir + 6%</span><b id="metricCod">${money(codTotal)}</b></div></div><div class="process-row"><div class="process-card"><b>Envío Normal</b><span>Producto + Lps. 110. Pago por depósito, transferencia o Tigo Money.</span></div><div class="process-card"><b>Pagar al Recibir</b><span>Producto + Lps. 100 + 6% de comisión.</span></div></div><div class="share-footer">WhatsApp +504 3151-7755 · Comayagua, Honduras</div></div>`
    }
...
WhatsApp SD COMAYAGUA: +504 3151-7755`;
  ...
  function cartItemHTML(it,i){
    ...
    return `<div class="cart-row cart-row-offer"><div class="cart-info"><b>${escapeHtml(it.name)}</b><span>${money(unit)} c/u · Total ${money(total)}</span>${promoTxt}</div><div class="qtybox"><button data-dec="${i}">−</button><input data-qty="${i}" type="number" value="${qty}"><button data-inc="${i}">+</button></div><button class="btn small danger remove-item" data-rem="${i}">Quitar</button></div>`;
  }
  ...
  function openClients(kind=null){
    hydrateState(); let q='';
    openModal(`<div class="modal-head"><h3>Clientes guardados</h3><button class="close">×</button></div><div class="modal-body clients-clean"><div class="card-box"><b>Agenda de clientes</b><span>Se llena automáticamente al guardar cotizaciones o ventas.</span><div class="searchbar"><span class="icon">⌕</span><input id="clientSearch" placeholder="Buscar nombre, teléfono, municipio o referencia..."></div></div><div id="clientsList"></div></div>`,true);
    function draw(){const term=q.toLowerCase().trim(); const list=(state.clients||[]).filter(c=>!term || [c.name,c.phone,c.department,c.municipality,c.reference,c.company].join(' ').toLowerCase().includes(term)); $('#clientsList').innerHTML=list.map(c=>`<div class="client-card-clean"><div><b>${escapeHtml(c.name||'Cliente')}</b><span>${escapeHtml(c.phone||'Sin teléfono')} · ${escapeHtml([c.department,c.municipality].filter(Boolean).join(' / ')||'Sin ubicación')}</span><small>${escapeHtml(c.reference||'Sin referencia')} · Último total ${money(c.lastTotal||0)}</small></div><div class="client-actions-list">${kind?`<button class="btn small" data-useclient="${escapeHtml(c.key)}">Usar</button>`:''}<button class="btn small secondary" data-remindclient="${escapeHtml(c.key)}">WhatsApp</button><button class="btn small danger" data-delclient="${escapeHtml(c.key)}">Borrar</button></div></div>`).join('')||'<div class="empty-state">Aún no hay clientes guardados.</div>'; $$('[data-useclient]').forEach(b=>b.onclick=()=>{const c=state.clients.find(x=>x.key===b.dataset.useclient); if(c)applyClientToDoc(c,kind)}); $$('[data-remindclient]').forEach(b=>b.onclick=()=>{const c=state.clients.find(x=>x.key===b.dataset.remindclient); if(c)openWhatsApp(c.phone||'',`Hola ${c.name||''}, le saluda SD COMAYAGUA. ¿Desea que le ayudemos con algún producto o cotización?`)}); $$('[data-delclient]').forEach(b=>b.onclick=()=>{if(!confirm('¿Borrar este cliente guardado?'))return; state.clients=state.clients.filter(x=>x.key!==b.dataset.delclient); save(); draw();});}
  ...
  function toggleMoneyLock(){
    if(state.settings.moneyLocked){
      ...
      if(pin!==(state.settings.accessKey || window.SDC_CONFIG?.accessKey || '')) return toast('Clave incorrecta.');
  ...
  function openQuickSale(){
    ...
    function drawSummary(){const p=productById(selected); const qty=Math.max(1,+($('#quickQty')?.value||1)); if(!p){$('#quickSummary').innerHTML='<div class="empty-state">Elegí un producto.</div>';return;} const products=productItemsTotal(p,qty); const cod=$('#quickType').value==='COD'; const shipping=cod?100:110; const total=cod?Math.round((products+shipping)*(1+((state.settings.codPercent||6)/100))):products+shipping; const offer=promoLabelForQty(p,qty); $('#quickSummary').innerHTML=`<div class="summary-row"><b>Producto</b><b>${money(products)}</b></div><div class="summary-row"><b>Envío</b><b>${money(shipping)}</b></div>${offer?`<div class="promo-applied-clean">🎁 ${escapeHtml(offer)}</div>`:''}<div class="summary-total"><b>Total</b><b>${money(total)}</b></div>`;}
