(function(){
  ...
  function hero(){
    const st=stats();
    return `<section class="hero v51-hero" id="inicio">
      <div class="v51-hero-shell">
        <div class="private-hero-head private-hero-head--no-logo">
          <div>
            <div class="pill v51-kicker"><span class="dot"></span> Panel móvil</div>
            <h2>Vende rápido desde tu celular.</h2>
            <p>Busca productos, cotiza, vende y comparte por WhatsApp sin pantallas complicadas.</p>
          </div>
  ...
  function inventoryHeadHTML(count){return `<div class="section-head inventory-head-v31"><div><h2>Productos</h2><p>Toca VER para cotizar, vender, enviar WhatsApp o editar.</p></div><div class="inventory-head-actions"><button class="inventory-new-btn" data-action="newProduct" type="button">+ Producto</button>${inventoryControls()}<span class="count-pill">${count} resultados</span></div></div>`}
  ...
  function quickPanel(){
    ...
    return `<section class="quick no-print quick-v22 quick-private quick-panel">
      <div class="quick-grid" aria-label="Acciones rápidas del panel">
        <button class="quick-btn" data-action="cardClient"><b>Cliente</b><small>Vista limpia</small></button>
        <button class="quick-btn" data-action="captureClean"><b>Captura</b><small>Foto limpia</small></button>
        <button class="quick-btn" data-action="quote"><b>Cotizar</b><small>Calcular total</small></button>
        <button class="quick-btn" data-action="sell"><b>Vender</b><small>Registrar venta</small></button>
        ...
        <button class="quick-btn" data-action="quickSale"><b>Rápida</b><small>Venta ágil</small></button>
  ...
  function pageFooter(){
    ...
    return `<footer class="sdc-page-footer no-print footer-v4911"><div class="footer-main"><div><b>SD COMAYAGUA</b><span>Inventario actualizado · ${escapeHtml(updated)}</span></div><small>WhatsApp +504 3151-7755 · Comayagua, Honduras</small></div><div class="footer-best"><strong>Más vendidos</strong><div>${bestSellersFooter()}</div></div><div class="footer-services"><span>🚚 Envío normal Lps. 110</span><span>📦 Pago al recibir: Lps. 110 + 6%</span><span>📲 Cotización lista para WhatsApp</span><span>🎁 Regalos descuentan stock</span></div></footer>`
  }
  ...
  function categoryGallery(){
    ...
    return `<section class="category-gallery no-print" id="categoriesBlock"><div class="category-head"><div><h2>Categorías</h2><p>Toca una categoría para filtrar el inventario.</p></div><span>${cats.length-1} categorías</span></div><div class="category-grid">${cats.map(c=>`<button class="category-card ${filter.cat===c?'active':''}" data-catcard="${escapeHtml(c)}"><img src="${escapeHtml(categoryImage(c))}" alt="${escapeHtml(c)}" onerror="this.onerror=null;this.src='assets/categorias/categoria.svg'"><b>${escapeHtml(c)}</b><small>${categoryCount(c)} productos</small></button>`).join('')}</div></section>`
  }
  ...
  function applyCategory(cat){const y=window.scrollY; filter.cat=cat||'Todos'; filter.special=''; renderInventoryOnly(); requestAnimationFrame(()=>window.scrollTo({top:y,left:0,behavior:'auto'}));}
  ...
  function renderPicker(isSale){
    ...
    function draw(){
      ...
      if(!cat && !hasSearch){
        showEmpty('Seleccione una categoría para ver productos','Así no se carga todo el inventario. También puede buscar por nombre o código escribiendo al menos 2 letras.');
        return;
      }
