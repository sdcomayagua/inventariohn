/* SDC V84 Mobile POS Pro: alertas en barra inferior y centro de control. */
(function(){
  ...
  async function openSheetsDoctor(){
    ...
    openModal(`<div class="modal-head"><h3>Diagnóstico Google Sheets</h3><button class="close">×</button></div><div class="modal-body"><div class="card-box"><b>Revisión de conexión</b><span>Comprueba si la app está leyendo la URL /exec, el Sheet ID y la pestaña productos_pos. Porque aparentemente ahora hasta una hoja de cálculo necesita terapia.</span></div><div id="sheetsDoctorBody" class="cart-list"><div class="empty-state">Probando conexión...</div></div><div class="modal-actions" style="position:static"><button class="btn secondary" id="doctorCopyConfig">Copiar configuración</button><button class="btn" id="doctorSyncNow">Sincronizar ahora</button></div></div>`,true);
    ...
    const tab=getProductSheetName();
    add(!!url,'URL /exec detectada',url?maskSheetUrl(url):'No hay webAppUrl en la configuración.');
    add(!!sid,'Sheet ID detectado',sid||'No hay Sheet ID configurado.');
    add(!!tab,'Pestaña de productos',tab||'No hay pestaña configurada.');
    if(!url || !sid){
      draw('<div class="empty-state">Falta configuración básica. Revisa js/data.js o vuelve a subir este paquete.</div>');
      return;
    }
    try{
      const ping=await sheetGet({action:'ping',sheetId:sid,productSheet:tab,_diag:Date.now()});
      add(!!(ping&&ping.ok),'Apps Script responde',ping&&ping.ok?(ping.message||'Endpoint activo.'):(ping&&ping.error)||'Respuesta inesperada.');
    }catch(err){
      add(false,'Apps Script no respondió',String(err&&err.message||err));
      draw('<div class="empty-state">El endpoint no respondió. Revisa implementación: ejecutar como Tú y acceso Cualquier persona con el enlace.</div>');
      return;
    }
    try{
      const data=await sheetGet({action:'products',only:'productos',sheetId:sid,productSheet:tab,_diag:Date.now()});
      const count=(data&&Array.isArray(data.products))?data.products.length:0;
      add(!!(data&&data.ok),'Lectura de productos',data&&data.ok?`${count} productos activos leídos desde ${tab}.`:(data&&data.error)||'No se pudieron leer productos.');
    }catch(err){
      add(false,'No se pudo leer productos',String(err&&err.message||err));
    }
    draw('');
    const copy=$('#doctorCopyConfig',modalRoot);
    if(copy) copy.onclick=()=>{copyTextSafe(`sheetId=${sid}
productSheet=${tab}
webAppUrl=${url}`); toast('Configuración de Sheets copiada.');};
    const sync=$('#doctorSyncNow',modalRoot);
    if(sync) sync.onclick=()=>syncProductsFromSheets({silent:false});
  }
  async function saveProductToSheets(product, previousId=''){
    ...
    const cleanProduct={...product, id:newCode, codigo:newCode, previousCodigo:originalCode};
    const attempts=[
      {action:'upsertProduct',codigo:originalCode,previousCodigo:originalCode,product:cleanProduct},
      {action:'saveProduct',codigo:originalCode,previousCodigo:originalCode,product:cleanProduct},
      {action:'updateProduct',codigo:originalCode,previousCodigo:originalCode,product:cleanProduct}
    ];
    let lastErr=null;
    for(const payload of attempts){
      try{
        const result=await sheetPost(payload,{verifyProductCode:newCode,timeout:26000});
        return result || true;
      }catch(err){
        lastErr=err;
  ...
  function alertMetrics(){
    ...
    const lowStockProducts=products.filter(p=>Number(p.stock)>0 && Number(p.stock)<=lowLimit);
    ...
    const outStockProducts=products.filter(p=>Number(p.stock||0)<=0);
  ...
  function filteredProducts(){
    ...
    return activeProducts().filter(p=>{
      ...
      if(filter.special==='outStock') ok = ok && Number(p.stock||0)<=0;
  ...
  function productForm(p={}){
    ...
    <div class="modal-body product-editor product-editor-v83">
      <div class="card-box product-sync-note-v83">
        ...
        <span>Al tocar “Guardar y sincronizar”, se guarda en este dispositivo y se envía a Google Sheets en la pestaña <strong>${escapeHtml(getProductSheetName())}</strong>. No crea otro archivo nuevo en Drive; actualiza la hoja conectada.</span>
      </div>
      <div class="card-box"><h4>Información básica</h4>
        <div class="modal-grid">
          ...
          <label><span class="label">Costo compra</span><input id="pCost" class="input" type="number" min="0" step="0.01" value="${prod.cost}"></label>
          <label><span class="label">Precio venta</span><input id="pPrice" class="input" type="number" min="0" step="0.01" value="${prod.price}"></label>
          <label><span class="label">Stock</span><input id="pStock" class="input" type="number" min="0" step="1" inputmode="numeric" placeholder="0" value="${prod.stock}"></label>
    ...
    $('#saveProduct').onclick=async()=>{
      ...
      try{
        ...
        if(getSheetApiUrl()){
          ...
          setProductSaveStatus('Enviando a Google Sheets y verificando que aparezca en la hoja...', 'info');
          ...
          setProductSaveStatus('Guardado solo en este dispositivo: falta configurar la URL /exec de Apps Script.', 'error'); toast('Producto guardado localmente. Falta configurar la URL de Apps Script.');
        }
      }catch(err){
        console.warn('Sheets save failed',err);
        setProductSaveStatus('No se confirmó Google Sheets: '+(err.message||err)+'. El producto quedó guardado en este dispositivo. Revisa el Apps Script o toca Guardar y sincronizar otra vez.', 'error');
