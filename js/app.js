...
(function(){
  ...
  function upliftQuoteUnit(value){
    return moneyRoundUpPlus(Number(value||0)*(1+(QUOTE_UPLIFT_PERCENT/100)));
  }
  ...
  function productQuotedUnit(p){
    return quotedUnitPrice(Number(p?.price||0));
  }
  function productQuotedItemsTotal(p,qty=1){
    const q=Math.max(1,Number(qty)||1);
    const baseTotal=productItemsTotal(p,q);
    const baseUnit=baseTotal/q;
    return upliftQuoteUnit(baseUnit)*q;
  }
  function itemTotal(it){
    const qty=Math.max(1,Number(it?.qty)||1);
    return qty*itemQuotedUnit(it);
  }
  function itemEffectiveUnit(it){return itemQuotedUnit(it)}
  ...
  function promoLabelForQty(p,qty){
    ...
    if(!tier) return '';
    const unit=upliftQuoteUnit(tier.price/tier.qty);
  ...
  function productNormalTotalQty(p,qty=1){return productQuotedItemsTotal(p,qty)+SHIPPING.normal.fee}
  function productCodTotalQty(p,qty=1){const base=productQuotedItemsTotal(p,qty)+SHIPPING.cod.fee; return codGrandTotal(base)}
  ...
  function promoWhatsAppLines(p){
    ...
    return rows.map(r=>{
      const quoteProducts=upliftQuoteUnit((Number(r.price||0)/Math.max(1,Number(r.qty)||1)))*Math.max(1,Number(r.qty)||1);
  ...
  function productLocalTotalQty(p,qty=1){return productQuotedItemsTotal(p,qty)}
  ...
  function updateClientCardTotals(id){
    ...
    set('main-label', mode==='local'?'Comayagua · sin envío':'Honduras · envío normal');
  ...
  function promoCompactText(p){
    ...
    if(!rows.length) return '';
    const best=rows[0];
    const offerTotal=upliftQuoteUnit(best.price/Math.max(1,best.qty))*Math.max(1,best.qty);
    return `${num(best.qty)}+ por ${money(offerTotal)}`;
  }
  ...
  function productCard(p){
    ...
    return `<article class="product-card product-card-v178 product-card-v190 product-card-v235 ${isOut?'is-out is-agotado':''}" data-id="${id}" data-product-id="${id}" data-stock-status="${st.cls}" data-product-name="${escapeHtml(p.name||'')}" data-product-price="${escapeHtml(String(productQuotedUnit(p)||0))}" data-product-stock="${escapeHtml(String(stockQty))}">
      ...
      <div class="product-copy-v178 product-copy-v235">
        <div class="product-code-row-v178">${cat?`<button type="button" class="product-cat-chip-v237" data-product-cat="${escapeHtml(cat)}">${escapeHtml(cat)}</button>`:''}${id?`<span>${id}</span>`:''}</div>
        <h3 data-action="viewProduct" data-id="${id}">${escapeHtml(p.name)}</h3>
        <div class="product-price-v178 product-main-price-v245"><strong class="${isOut?'is-out':''}" data-client-total="main">${money(mainTotal)}</strong><em class="${st.cls}" data-client-total="main-label">${deliveryMode==='local'?'Comayagua · sin envío':'Honduras · envío normal'}</em></div>
        <div class="delivery-panel-v245" data-delivery-panel data-mode="${deliveryMode}">
          ...
          <small class="delivery-offer-v245" data-client-total="offer" style="${promoLabelForQty(p,qty)?'':'display:none'}">${promoLabelForQty(p,qty)?`🎁 ${escapeHtml(promoLabelForQty(p,qty))}`:''}</small>
        </div>
        ${outNote}
...
📲 WhatsApp: +504 3151-7755`;
  ...
  function renderPicker(isSale){
    ...
    function draw(){
      ...
      list.innerHTML=(items.map(p=>`<div class="picker-item picker-item-v200"><img src="${escapeHtml(productImage(p))}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div><b>${escapeHtml(p.name)}</b><span>${money(productQuotedUnit(p))} · Stock ${num(productStock(p))}${colorStockSummary(p,2)?` · ${escapeHtml(colorStockSummary(p,2))}`:''} · ${escapeHtml(firstTag(p))}</span></div><button class="btn small add-pick-btn" type="button" data-additem="${escapeHtml(p.id)}">Añadir</button></div>`).join('')+limitNote)||'<div class="picker-empty"><b>Sin productos para mostrar</b><span>Pruebe otra categoría o revise el texto de búsqueda.</span></div>';
...
📲 WhatsApp +504 3151-7755`;
  ...
  function openQuickSale(){
    ...
    function drawList(){
      ...
      $('#quickList').innerHTML=list.map(p=>`<div class="picker-item ${selected===p.id?'active':''}"><img src="${escapeHtml(productImage(p))}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div><b>${escapeHtml(p.name)}</b><span>${money(productQuotedUnit(p))} · Stock ${num(productStock(p))}${colorStockSummary(p,2)?` · ${escapeHtml(colorStockSummary(p,2))}`:''}</span></div><button class="btn small" data-qselect="${escapeHtml(p.id)}">Elegir</button></div>`).join('');
