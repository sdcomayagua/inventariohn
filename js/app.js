...
(function(){
        ...
        <div class="v141-detail-shell v163-detail-shell">
          ...
          <section class="v49-tab active" data-panel="cliente">
            ...
            <div class="v49-price-cards v141-price-cards v163-price-cards v164-sale-buttons v250-price-cards" data-v49-route-wrap="${routeMode}">
              <button type="button" class="v164-price-option ${routeMode==='local'?'is-main is-current':''}" data-v49-card="local" ${routeMode==='local'?'':'hidden aria-hidden="true"'}><span>Comayagua</span><b data-v49-total="local">${money(local)}</b><small>Precio local · según zona</small></button>
              <button type="button" class="v164-price-option ${routeMode==='hn'?'is-main is-current':''}" data-v49-card="normal" ${routeMode==='hn'?'':'hidden aria-hidden="true"'}><span>Honduras / normal</span><b data-v49-total="normal">${money(normal)}</b><small>Depósito / Tigo Money</small></button>
              <button type="button" class="v164-price-option" data-v49-card="cod" ${routeMode==='hn'?'':'hidden aria-hidden="true"'}><span>Pagar al recibir</span><b data-v49-total="cod">${money(cod)}</b><small>Envío + comisión 10%</small></button>
            </div>
  ...
  function bindProductDetails(p){
    ...
    if(detailModalEl && !detailModalEl.dataset.v279Controls){
      detailModalEl.dataset.v279Controls='1';
      detailModalEl.addEventListener('click',ev=>{
        const routeBtn=ev.target.closest?.('[data-v49-route]');
        if(routeBtn){
          ev.preventDefault(); ev.stopPropagation();
          routeMode=routeBtn.dataset.v49Route==='local'?'local':'hn';
