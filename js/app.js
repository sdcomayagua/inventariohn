(function(){
  ...
  function renderLogin(){
    ...
    app.innerHTML=`<section class="login-card">
      ...
      <div class="version-pill">Versión 1 · paquete limpio</div>
  ...
  function openProductDetails(id){
    ...
    function cardHTML(){
      ...
      return `<div class="product-share-card product-share-card-clean" id="productShareCard"><div class="share-brand"><span>SD COMAYAGUA</span><b>Disponible</b></div><div class="product-share-media"><img id="detailMainImage" src="${escapeHtml(safeImgs[0])}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><b class="price-badge share-main-price">${money(p.price)}</b></div><h2>${escapeHtml(p.name)}</h2><p class="share-description">${escapeHtml(p.description||'Producto disponible para entrega. Consulta disponibilidad antes de confirmar tu pedido.')}</p><div class="share-qty-line"><span>Cantidad consultada</span><b id="shareQtyText">${unitsLabel()} · ${money(productTotal)}${promoUsed()?' · promo aplicada':''}</b></div><div class="public-metrics public-metrics-clean"><div><span>Producto</span><b id="metricProduct">${money(productTotal)}</b></div><div><span>Envío normal</span><b id="metricNormal">${money(normalTotal)}</b></div><div><span>Al recibir + 6%</span><b id="metricCod">${money(codTotal)}</b></div></div><div class="process-row"><div class="process-card"><b>Envío Normal</b><span>Deposita, transfiere o paga por Tigo Money: producto + Lps. 110.</span></div><div class="process-card"><b>Pagar al Recibir</b><span>Producto + Lps. 100 de envío + 6% de comisión. Total redondeado en lempiras.</span></div></div><div class="share-footer">SD COMAYAGUA · WhatsApp +504 3151-7755</div></div>`
    }
