(function(){
  ...
  const RECEIPT_LOGO_SRC = LOGO_SRC;
  const SHIPPING = {
    normal: { type:'Normal', label:'Envío Normal', fee:110, cod:false, note:'Producto + Lps. 110 de envío. Pago por depósito o Tigo Money.' },
    cod: { type:'COD', label:'Pagar al recibir', fee:100, cod:true, note:'Producto + Lps. 100 de envío + comisión del 6%. Total redondeado en lempiras.' },
  ...
  function emptyQuote(){return {id:'COT-'+Date.now(),items:[],client:'',phone:'',department:'Comayagua',municipality:'Comayagua',reference:'',shippingType:'Normal',company:'Forza',shipping:SHIPPING.normal.fee,cod:false,discount:0,date:new Date().toISOString(),saved:false}}
      ...
      <div class="receipt-inner-pro">
        <header class="receipt-header-pro">
          <div class="receipt-brand-pro">
            <div class="receipt-logo-box"><span>SD</span><img class="doc-logo receipt-logo-inline" src="${RECEIPT_LOGO_SRC}" alt="Logo SD Comayagua" loading="eager"></div>
            <div>
  ...
  function openQuickSale(){
    ...
    function drawSummary(){const p=productById(selected); const qty=Math.max(1,+($('#quickQty')?.value||1)); if(!p){$('#quickSummary').innerHTML='<div class="empty-state">Elegí un producto.</div>';return;} const products=productItemsTotal(p,qty); const type=$('#quickType').value; const cod=type==='COD'; const shipping=type==='COD'?SHIPPING.cod.fee:type==='Local'?(+$('#quickShip').value||0):SHIPPING.normal.fee; const total=cod?Math.round((products+shipping)*(1+((state.settings.codPercent||6)/100))):products+shipping; const offer=promoLabelForQty(p,qty); const shipName=type==='Local'?'Envío local':type==='COD'?'Pagar al recibir':'Envío normal'; $('#quickSummary').innerHTML=`<div class="summary-row"><b>Producto</b><b>${money(products)}</b></div><div class="summary-row"><b>${shipName}</b><b>${money(shipping)}</b></div>${offer?`<div class="promo-applied-v26">🎁 ${escapeHtml(offer)}</div>`:''}<div class="summary-total"><b>Total</b><b>${money(total)}</b></div>`;}
    $('#quickSearch').oninput=e=>{q=e.target.value;drawList()}; $('#quickQty').oninput=drawSummary; $('#quickShip').oninput=drawSummary; $('#quickType').onchange=()=>{const t=$('#quickType').value; $('#quickShip').value=t==='COD'?SHIPPING.cod.fee:t==='Local'?0:SHIPPING.normal.fee; drawSummary();}; drawList(); drawSummary();
    $('#quickFinish').onclick=()=>{const p=productById(selected); if(!p)return toast('Elegí un producto.'); const qty=Math.max(1,+$('#quickQty').value||1); saleDraft=emptySale(); saleDraft.client=$('#quickClient').value.trim(); saleDraft.phone=$('#quickPhone').value.trim(); saleDraft.shippingType=$('#quickType').value; saleDraft.cod=saleDraft.shippingType==='COD'; saleDraft.shipping=+$('#quickShip').value||0; applyShippingPreset(saleDraft,saleDraft.shippingType,false); saleDraft.shipping=+$('#quickShip').value||0; saleDraft.status=isCodDoc(saleDraft)?'Pagar al recibir':isLocalDoc(saleDraft)?'Entrega local':'Vendido'; saleDraft.items=[{id:p.id,name:p.name,price:+p.price||0,cost:+p.cost||0,qty,image:productImage(p)}]; finishSale(); closeModal(); render();};
  }
  ...
  function openShortReceipt(isSale){
    ...
    openModal(`<div class="modal-head"><h3>Recibo corto</h3><button class="close">×</button></div><div class="modal-body"><div class="short-receipt short-receipt-v34" id="shortReceiptCard"><div class="short-brand"><div class="short-logo-box"><span>SD</span><img class="receipt-logo-inline" src="${RECEIPT_LOGO_SRC}" alt="SD"></div><div><h2>SD COMAYAGUA</h2><p>${escapeHtml(doc.client||'Cliente')} · ${nowHN()}</p></div></div>${lines}<hr><div><span>Productos</span><b>${money(c.products)}</b></div><div><span>${shippingLabel(doc)}</span><b>${money(c.shipping)}</b></div>${c.commission?`<div><span>Comisión</span><b>${money(c.commission)}</b></div>`:''}<div class="grand"><span>Total</span><b>${money(c.total)}</b></div><small>WhatsApp +504 3151-7755</small></div><div class="modal-actions" style="position:static"><button class="btn" id="copyShortReceipt">Copiar texto</button></div></div>`,true);
