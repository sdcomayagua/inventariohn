...
(function(){
  ...
  function normalizeSheetRemoteProduct(row,i=0){
    const p=SDCStore.normalizeProduct({
      ...
      colors: row.colors || row.colores || row.colorStock || row.stock_colores || row.stockColores || row.variantes_color || row.variantesColor || '',
  ...
  async function syncProductsFromSheets(opts={}){
    ...
    try{
      ...
      const products=raw.filter(sheetRowHasProduct).map(normalizeSheetRemoteProduct).filter(isRealProduct).map(p=>{
  ...
  async function uploadLocalProductsToSheets(){
    hydrateState();
    if(!getSheetApiUrl()) return toast('Falta configurar la URL /exec de Apps Script.');
    ...
    toast(`Subiendo ${total} productos a Google Sheets...`);
    for(let i=0;i<products.length;i++){
      const p=products[i];
      try{
        await saveProductToSheets(p,p.id);
        ok.push(p.id);
        if(i===0 || (i+1)%5===0 || i===products.length-1) toast(`Sheets: ${i+1}/${total} productos procesados...`);
      }catch(err){
        console.warn('No se pudo subir producto a Sheets',p,err);
    ...
    if(fail.length){
      ...
      alert(`Se subieron ${ok.length}/${total} productos. Fallaron ${fail.length}.

Primeros errores:
${detail}

Si todos fallan, pega el archivo google-apps-script-sdc-v87.gs en Apps Script y vuelve a implementar.`);
      toast(`Sheets: ${ok.length}/${total} subidos; ${fail.length} con error.`);
    }else{
      render();
      toast(`✅ Inventario completo subido y verificado en Google Sheets (${ok.length}).`);
    }
  }
  ...
  async function openSheetsDoctor(){
    ...
    openModal(`<div class="modal-head"><h3>Diagnóstico Google Sheets</h3><button class="close">×</button></div><div class="modal-body sheets-doctor-v87"><div class="card-box"><b>Revisión de conexión</b><span>Comprueba si la app está leyendo la URL /exec, el Sheet ID y la pestaña productos_pos. También puedes subir el inventario local a la hoja cuando Sheets está vacío.</span></div><div id="sheetsDoctorBody" class="cart-list"><div class="empty-state">Probando conexión...</div></div><div class="modal-actions sheets-doctor-actions-v87" style="position:static"><button class="btn secondary" id="doctorCopyConfig">Copiar configuración</button><button class="btn secondary" id="doctorSyncNow">Bajar desde Sheets</button><button class="btn" id="doctorUploadNow">Subir inventario local</button></div></div>`,true);
...
webAppUrl=${url}`); toast('Configuración de Sheets copiada.');};
    ...
    if(upload) upload.onclick=()=>uploadLocalProductsToSheets();
  }
  async function saveProductToSheets(product, previousId=''){
    ...
    const cleanProduct=normalizeProductColorStock({...product, id:newCode, codigo:newCode, previousCodigo:originalCode});
    cleanProduct.colores=colorRowsText(cleanProduct.colors);
    cleanProduct.stock=productStock(cleanProduct);
    const attempts=[
  ...
  async function syncStockAfterSale(ids){
    ...
    const updates=Array.from(ids||[]).map(id=>{
      ...
      return p?{codigo:p.id,stock:productStock(p),colors:productColorRows(p),colores:colorRowsText(productColorRows(p))}:null;
    }).filter(Boolean);
  ...
  function topbar(){
    ...
    return `<header class="topbar topbar-v87" role="banner"><img class="top-logo" src="${LOGO_SRC}" alt="Logo SD Comayagua"><div class="top-title top-title-v87"><div class="top-title-row-v88"><h1>SD COMAYAGUA</h1><span class="top-badge-v88">POS PRO</span></div><small>Inventario · Ventas · Google Sheets</small><span class="top-meta-v88">Panel activo · ${nowHNPanel()}</span></div><div class="spacer"></div><button class="btn small ghost sync-status sync-icon-only" data-action="sync" title="Sincronizar desde Google Sheets (${lastSync})" aria-label="Sincronizar desde Google Sheets"><span class="sync-dot"></span><span class="sync-icon">↻</span><span class="sync-label">${lastSync}</span></button><button class="btn small secondary logout-btn logout-icon-btn" data-action="lock" title="Salir del panel" aria-label="Salir del panel"><span class="logout-door" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M9 4h7.2c.9 0 1.6.7 1.6 1.6v12.8c0 .9-.7 1.6-1.6 1.6H9"/><path d="M9 3.8 5.4 5.2v13.6L9 20.2V3.8Z"/><path d="M12.2 12h7"/><path d="m17 9.8 2.2 2.2-2.2 2.2"/></svg></span><span class="logout-text">Salir</span></button></header>`
=======
    return `<header class="topbar topbar-v87" role="banner"><img class="top-logo" src="${LOGO_SRC}" alt="Logo SD Comayagua"><div class="top-title top-title-v87"><div class="top-title-row-v88"><h1>SD COMAYAGUA</h1><span class="top-badge-v88">POS PRO</span></div><small>Inventario · Ventas · Google Sheets</small></div><div class="spacer"></div><button class="btn small ghost sync-status sync-icon-only" data-action="sync" title="Sincronizar desde Google Sheets" aria-label="Sincronizar desde Google Sheets"><span class="sync-dot"></span><span class="sync-icon">↻</span><span class="sync-label">${lastSyncLabel()}</span></button><button class="btn small secondary logout-btn logout-icon-btn" data-action="lock" title="Salir del panel" aria-label="Salir del panel"><span class="logout-door" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M9 4h7.2c.9 0 1.6.7 1.6 1.6v12.8c0 .9-.7 1.6-1.6 1.6H9"/><path d="M9 3.8 5.4 5.2v13.6L9 20.2V3.8Z"/><path d="M12.2 12h7"/><path d="m17 9.8 2.2 2.2-2.2 2.2"/></svg></span><span class="logout-text">Salir</span></button></header>`
  }
  function hero(){
    const st=stats();
    return `<section class="hero v51-hero hero-v56-clean home-hero-v87" id="inicio">
      <div class="v51-hero-shell home-hero-shell-v87">
        <div class="home-actions-card-v87 no-print">
          <div class="home-actions-head-v87">
            <div><small>Acciones principales</small><b>¿Qué vas a hacer?</b></div>
            <em>${num(st.stock)} en stock</em>
          </div>
          <div class="v51-hero-actions home-actions-grid-v87">
            <button type="button" data-action="sell" class="home-action-sell-v87"><i aria-hidden="true">⚡</i><span>Vender</span><small>Crear venta rápida</small></button>
            <button type="button" data-action="quote" class="secondary"><i aria-hidden="true">🧾</i><span>Cotizar</span><small>Enviar precio</small></button>
            <button type="button" data-action="newProduct" class="secondary hero-new-product"><i aria-hidden="true">＋</i><span>Producto</span><small>Agregar stock</small></button>
          </div>
        </div>
        <div class="stats home-stats-v87">
          <div class="stat"><b>${num(st.count)}</b><span>Productos</span></div><div class="stat"><b>${num(st.stock)}</b><span>Unidades</span></div>
          <div class="stat"><b>${moneyPrivate(st.value)}</b><span>Venta total</span></div><div class="stat"><b>${moneyPrivate(st.invested)}</b><span>Invertido</span></div>
  ...
  function productCard(p){
    ...
    const stockLine=isOut?'Sin unidades disponibles':`En stock: <b>${num(stockQty)}</b> unidades${colorSummary?`<br><small class="stock-colors-v86">${escapeHtml(colorSummary)}</small>`:''}`;
    ...
    <div class="modal-body product-editor product-editor-v83">
      ...
      <div class="card-box"><h4>Información básica</h4>
        <div class="modal-grid">
          ...
          <label><span class="label">Stock general</span><input id="pStock" class="input" type="number" min="0" step="1" inputmode="numeric" placeholder="0" value="${prod.stock}"></label>
  ...
  function productDetailHTML(p){
    ...
    const gainTotal=Math.max(0,gainUnit*stockQty);
    const invested=Number(p.cost||0)*stockQty;
    const sellAll=Number(p.price||0)*stockQty;
    ...
    return `<div class="modal-head v49-detail-head"><div><span class="sdc-safe-pill detail-pill hn-time-pill" id="hnLiveTime">${nowHNPanel()}</span><h3>${escapeHtml(p.name)}</h3></div><button class="close">×</button></div>
      <div class="modal-body v49-product-detail ${st.cls==='out'?'is-out':''}">
        ...
        <section class="v49-tab active" data-panel="cliente">
          ...
          <div class="v49-benefits"><b>Beneficios para cliente</b><p>${escapeHtml(productDescription(p))}</p></div>${colorsHTML?`<div class="v86-color-client"><b>Colores disponibles</b>${colorsHTML}<small>Confirme color antes de cerrar la venta.</small></div>`:''}
        </section>
        <section class="v49-tab" data-panel="admin">
          <div class="v49-admin-grid">
            <div><span>Stock</span><b>${num(stockQty)}</b></div>
            ...
            <div class="full"><span>Si vende todo el stock</span><b>${money(sellAll)}</b></div>${colorsHTML?`<div class="full"><span>Stock por color</span>${colorsHTML}</div>`:''}
          </div>
        </section>
  ...
  function docCard(doc,isSale){
    ...
    const rows=(doc.items||[]).map((it,i)=>{
      ...
      return `<div class="receipt-item-pro has-thumb">
        ...
        <div class="receipt-item-info">
          ...
          <span>${num(qty)} ${qty===1?'unidad':'unidades'} · ${money(unit)} c/u${itemColorLine(it)}${promo?' · Oferta aplicada':''}</span>
        </div>
    ...
    const giftRows=(doc.gifts||[]).map((it,i)=>{
      ...
      const thumb=realImg?`<img class="receipt-item-thumb" src="${escapeHtml(realImg)}" alt="${escapeHtml(it.name)}" loading="eager" crossorigin="anonymous" referrerpolicy="no-referrer" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'receipt-item-thumb receipt-thumb-fallback',textContent:'${initials}'}))">`:`<div class="receipt-item-thumb receipt-thumb-fallback">${initials}</div>`;
      return `<div class="receipt-item-pro has-thumb gift-receipt-row"><div class="receipt-item-index">🎁</div>${thumb}<div class="receipt-item-info"><b>${escapeHtml(it.name)}</b><span>${num(qty)} ${qty===1?'unidad':'unidades'}${itemColorLine(it)} · Regalo incluido</span></div><strong>Lps. 0</strong></div>`;
    }).join('');
  ...
  function openProfit(){
    const rows=state.products.map(p=>({p,stock:productStock(p),profit:(+p.price||0)-(+p.cost||0),total:((+p.price||0)-(+p.cost||0))*productStock(p)}));
    openModal(`<div class="modal-head"><h3>Ganancias</h3><button class="close">×</button></div><div class="modal-body"><div class="empty-state">Alerta: productos con ganancia muy baja aparecen marcados para revisar precio o costo.</div><table class="profit-table"><thead><tr><th>Producto</th><th>C/U</th><th>Stock</th><th>Total</th></tr></thead><tbody>${rows.map(r=>`<tr class="${r.profit>0&&r.profit<10?'low-profit-row':''}"><td>${escapeHtml(r.p.name)}</td><td>${money(r.profit)}</td><td>${num(r.stock)}</td><td>${money(r.total)}</td></tr>`).join('')}</tbody></table></div>`,true)
  }
  ...
  function shortReceiptLinesHTML(doc){
    return (doc.items||[]).map(it=>{const qty=Math.max(1,+it.qty||1); const color=selectedColorLabel(it); return `<div class="short-line item"><span>${escapeHtml(it.name)} <em>x${num(qty)}</em>${color?`<small>Color: ${escapeHtml(color)}</small>`:''}</span><b>${money(itemTotal(it))}</b></div>`}).join('');
  }
  ...
  window.uploadLocalProductsToSheets = uploadLocalProductsToSheets;
