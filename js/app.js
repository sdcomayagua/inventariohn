...
(function(){
  ...
  function inventoryHeadHTML(count){return `<div class="section-head inventory-head-v31"><div><h2>Productos</h2></div><div class="inventory-head-actions"><button class="inventory-new-btn" data-action="newProduct" type="button">+ Producto</button>${inventoryControls()}<span class="count-pill">${count} resultados</span></div></div>`}
  ...
  function categoryGallery(){
    ...
    return `<section class="category-gallery no-print" id="categoriesBlock"><div class="category-head"><div><h2>Categorías</h2></div><span>${cats.length-1} categorías</span></div><div class="category-grid">${cats.map(c=>`<button class="category-card ${filter.cat===c?'active':''}" data-catcard="${escapeHtml(c)}"><img src="${escapeHtml(categoryImage(c))}" alt="${escapeHtml(c)}" onerror="this.onerror=null;this.src='assets/categorias/categoria.svg'"><b>${escapeHtml(c)}</b><small>${categoryCount(c)} productos</small></button>`).join('')}</div></section>`
  }
