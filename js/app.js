...
(function(){
  ...
  const SHIPPING = {
    ...
    cod: { type:'COD', label:'Pagar al recibir', fee:110, cod:true, note:'Producto + Lps. 110 de envío + comisión 7%. Si sale con centavos, se redondea hacia arriba y se suma Lps. 1.' },
  ...
  function autoProductSpecs(p={}){
    return {estado:'Nuevo',categoria:firstTag(p)||'General',pais:'Honduras',envios:'Normal Lps.110 · Pagar al Recibir Lps.110 + 7% · Local por definir'};
  }
  ...
  function categoryIconFor(p){
    ...
    if(/aud[ií]fono|auricular|audio|qkz|bluetooth|tipo c/.test(hay)) return '🎧';
    if(/dedal|gatillo|trigger|gamer|gaming|free fire|pubg|call of duty/.test(hay)) return '🎮';
    if(/enfriador|cooler|ventilador|radiador/.test(hay)) return '❄️';
    if(/cable|cargador|adaptador|usb|micro sd|microsd|memoria|tipo c/.test(hay)) return '🔌';
    if(/termo|cocina|hogar|limpieza|zapato|secador|organizador/.test(hay)) return '🏠';
    if(/belleza|cosm[eé]tico|cuidado/.test(hay)) return '✨';
    return '📦';
  }
  function categoryFallbackSVG(p){
    const cat=firstTag(p)||'Producto';
    const label=String(cat).replace(/\s+/g,' ').trim().slice(0,24)||'Producto';
    const icon=categoryIconFor(p);
    const code=String(p?.id||'SDC').replace(/\s+/g,' ').trim().slice(0,16)||'SDC';
    const name=String(p?.name||'Sin imagen').replace(/\s+/g,' ').trim().slice(0,34)||'Sin imagen';
    ...
    const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="900" height="900" viewBox="0 0 900 900">
      <defs>
        ...
        <radialGradient id="r" cx="72%" cy="18%" r="70%"><stop offset="0" stop-color="#ffffff" stop-opacity=".72"/><stop offset=".55" stop-color="#ffffff" stop-opacity=".14"/><stop offset="1" stop-color="#ffffff" stop-opacity="0"/></radialGradient>
        <filter id="s" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="22" stdDeviation="20" flood-color="#03224a" flood-opacity=".22"/></filter>
      </defs>
      <rect width="900" height="900" rx="118" fill="url(#g)"/>
      <rect width="900" height="900" rx="118" fill="url(#r)"/>
      <circle cx="735" cy="142" r="174" fill="#fff" opacity=".15"/>
      <circle cx="160" cy="765" r="226" fill="#002c66" opacity=".13"/>
      <g filter="url(#s)">
        <rect x="120" y="126" width="660" height="594" rx="82" fill="#ffffff" opacity=".94"/>
        <rect x="158" y="164" width="584" height="518" rx="62" fill="#f5fbff"/>
        <circle cx="450" cy="338" r="122" fill="#e6f4ff"/>
        <text x="450" y="385" text-anchor="middle" font-size="132" font-family="Apple Color Emoji, Segoe UI Emoji, Arial, sans-serif">${icon}</text>
        <text x="450" y="528" text-anchor="middle" font-size="54" font-weight="900" fill="#07192f" font-family="Barlow, Arial, sans-serif">${escapeHtml(label)}</text>
        <text x="450" y="586" text-anchor="middle" font-size="28" font-weight="800" fill="#607894" font-family="Barlow, Arial, sans-serif">Imagen automática por categoría</text>
        <text x="450" y="635" text-anchor="middle" font-size="24" font-weight="900" fill="#0b73f6" font-family="Barlow, Arial, sans-serif">${escapeHtml(code)}</text>
      </g>
      <text x="450" y="804" text-anchor="middle" font-size="30" font-weight="900" fill="#ffffff" opacity=".94" font-family="Barlow, Arial, sans-serif">${escapeHtml(name)}</text>
      <text x="450" y="850" text-anchor="middle" font-size="24" font-weight="800" fill="#dff3ff" font-family="Barlow, Arial, sans-serif">SD COMAYAGUA</text>
    </svg>`;
  ...
  function promoWhatsAppLines(p){
    ...
    return rows.map(r=>`• ${num(r.qty)} ${r.qty===1?'unidad':'unidades'}: Producto ${money(r.price)} | Depósito ${money(r.price+SHIPPING.normal.fee)} | Pagar al Recibir ${money(codGrandTotal(r.price+SHIPPING.cod.fee))}`).join('\n');
  }
  ...
  async function syncProductsFromFirebase(opts={}){
    ...
    try{
      ...
      const repairImages=[];
      const products=dedupeProducts((Array.isArray(raw)?raw:[]).map(normalizeFirebaseRemoteProduct).filter(isRealProduct).map(p=>{
  ...
  function topbar(){
    ...
    return `<header class="sdc-top-v178 no-print" role="banner">
      <section class="sdc-hero-v178" aria-label="SD Comayagua">
        <div class="sdc-hero-tools-v178">
        ...
        <div class="sdc-brand-v178">
          <div class="sdc-logo-v178"><img src="${LOGO_SRC}" alt="SD Comayagua"></div>
          <div class="sdc-brand-text-v178">
            <small>Panel privado</small>
            <h1>SD COMAYAGUA</h1>
            <p>Catálogo · Ventas · Cotizaciones · Inventario</p>
          </div>
        </div>
        <div class="sdc-status-v178"><span>Activo</span><b>${nowHNPanel()}</b></div>
      </section>
    </header>`
  }
    function hero(){
    ...
    return `<section class="hero v51-hero hero-v56-clean home-hero-v87 home-hero-v94" id="inicio">
      ...
      <div class="v51-hero-shell home-hero-shell-v87 home-hero-shell-v94 home-clean-v187">
        <div class="home-actions-card-v87 home-actions-card-v94 no-print home-actions-only-v187">
        ...
        <div class="stats home-stats-v87 home-stats-v94 home-stats-clean-v187">
  ...
  function categoryQuickRailHTML(){
    const current=filter.cat||'Todos';
    const cats=allCategories();
    const label=current==='Todos'?'Todas':current;
    const activeCount=categoryCount(current);
    const chips=cats.map(c=>{
      const isActive=String(current).toLowerCase()===String(c).toLowerCase();
      const shortLabel=c==='Todos'?'Todas':c;
      return `<button type="button" class="category-chip-v191 ${isActive?'active':''}" data-action="categoryQuick" data-cat="${escapeHtml(c)}" aria-pressed="${isActive?'true':'false'}"><span>${escapeHtml(shortLabel)}</span><em>${num(categoryCount(c))}</em></button>`;
    }).join('');
    return `<section class="category-rail-v191 no-print" aria-label="Categorías del catálogo">
      <div class="category-rail-head-v191">
        <button type="button" class="category-main-v191" data-action="categoriesSheet"><i aria-hidden="true">▦</i><span>Categorías</span></button>
        <div><b>${escapeHtml(label)}</b><small>${num(activeCount)} productos</small></div>
      </div>
      <div class="category-strip-v191" role="list">${chips}</div>
    </section>`;
  }
  function inventoryHeadHTML(count,list=null){
    ...
    return `<div class="products-screen-v178 products-screen-v189 products-screen-v190">
      <section class="catalog-cover-v178 catalog-cover-v189" aria-label="Catálogo digital">
        ...
        <div class="catalog-metrics-v178 catalog-metrics-v189" aria-label="Resumen de catálogo">
          <article class="metric-results-v189"><span>Resultados</span><b class="count-pill">${count}</b><small>productos visibles</small></article>
          <article class="metric-units-v189"><span>Unidades</span><b>${num(visibleUnits)}</b><small>en vista actual</small></article>
          <article class="metric-cloud-v189"><span>Nube</span><button type="button" class="cloud-pill-v189" data-action="sync" title="${escapeHtml(sync)}"><i aria-hidden="true"></i><b>Firebase</b><small>${escapeHtml(sync)}</small></button></article>
        </div>
      ...
      <section class="catalog-control-v178 catalog-control-v189 no-print" aria-label="Herramientas de productos">
        ...
        <div class="catalog-filter-row-v178 catalog-filter-row-v189">
  ...
  function productCard(p){
    ...
    return `<article class="product-card product-card-v178 product-card-v190 ${isOut?'is-out is-agotado':''}" data-id="${id}" data-product-id="${id}" data-stock-status="${st.cls}" data-product-name="${escapeHtml(p.name||'')}" data-product-price="${escapeHtml(String(p.price||0))}" data-product-stock="${escapeHtml(String(stockQty))}">
      ${outRibbon}
      <button type="button" class="product-photo-v178" data-action="viewProduct" data-id="${id}" aria-label="Ver detalle de ${escapeHtml(p.name)}">
      ...
      <div class="product-copy-v178">
        <div class="product-code-row-v178"><span>${escapeHtml(cat||'Producto')}</span>${id?`<span>${id}</span>`:''}</div>
        <h3 data-action="viewProduct" data-id="${id}">${escapeHtml(p.name)}</h3>
        <div class="product-price-v178"><strong class="${isOut?'is-out':''}">${money(p.price)}</strong><em class="${st.cls}">${isOut?'Sin stock':`${num(stockQty)} disp.`}</em></div>
        ${outNote}
        <div class="product-pills-v178">
          ${colors?`<span><i>◌</i>${escapeHtml(colors)}</span>`:''}
          ${offer?`<span class="offer"><i>◆</i>${escapeHtml(offer)}</span>`:''}
        </div>
      </div>
      <div class="product-actions-v178 product-actions-v190">
  ...
  function openCategoriesSheet(){
    ...
    const cards=cats.map(c=>{
      ...
      return `<button type="button" class="category-sheet-card-v191 ${isActive?'active':''}" data-catpick-v191="${escapeHtml(c)}"><span>${escapeHtml(label)}</span><b>${num(categoryCount(c))}</b><small>${c==='Todos'?'Ver todo el catálogo':'Filtrar esta categoría'}</small></button>`;
    }).join('');
    openModal(`<div class="modal-head category-sheet-head-v191"><div><small>Filtro rápido</small><h3>Categorías</h3></div><button class="close">×</button></div><div class="modal-body category-sheet-v191"><p class="category-sheet-copy-v191">Elige una categoría y el catálogo baja directo a los productos correctos.</p><div class="category-sheet-grid-v191">${cards}</div></div>`,true);
  ...
  function openModal(html,wide=false){
    ...
    const m=$('.modal',modalRoot); if(m){ if(String(html||'').includes('quote-body-v176')) m.classList.add('quote-modal-v176'); m.scrollLeft=0; m.scrollTop=0;}
  ...
  function openProductEditor(id){
    ...
    $('#saveProduct').onclick=async()=>{
      ...
      const colors=mergeColorRows(colorRows).filter(r=>String(r.name||'').trim());
  ...
  function productDetailHTML(p){
    ...
    const cod=codGrandTotal(productTotal+SHIPPING.cod.fee);
      ...
      <div class="modal-body v49-product-detail v141-product-detail v163-product-detail ${st.cls==='out'?'is-out':''}">
        <div class="v141-detail-shell v163-detail-shell">
          ...
          <section class="v49-tab active" data-panel="cliente">
            ...
            <div class="v49-price-cards v141-price-cards v163-price-cards v164-sale-buttons">
              <button type="button" class="v164-price-option is-main"><span>Producto</span><b data-v49-total="product">${money(productTotal)}</b><small>${num(q)} unidad${q===1?'':'es'}</small></button>
              <button type="button" class="v164-price-option"><span>Depósito</span><b data-v49-total="normal">${money(normal)}</b><small>Tigo Money</small></button>
              <button type="button" class="v164-price-option"><span>Al recibir</span><b data-v49-total="cod">${money(cod)}</b><small>Contra entrega</small></button>
              <button type="button" class="v164-price-option"><span>Local</span><b>${local}</b><small>Según zona</small></button>
            </div>
      ...
      <div class="productPhotoClean productPhotoClean-v49 productPhotoClean-v498 productPhotoLandscape-v147 productPhotoClean-v163" data-export="product-photo-clean">
        ...
        <div class="productPhotoMain-v147 productPhotoMain-v163">
          ...
          <div class="productPhotoBody productPhotoBody-v147 productPhotoBody-v163">
            ...
            <strong class="productPhotoPrice-v163">${money(p.price)}</strong>
...
🧾 *Producto:* ${money(total)}
📦 *Disponibles:* ${num(stockQty)}${colors?`
🎨 *Colores:* ${colors}`:''}${offer?`
🏷️ *Oferta:* ${offer.replace('Oferta aplicada: ','')}`:''}
...
📲 WhatsApp: +504 3151-7755`;
  ...
  function categorySnapshotHTML(list,title){
    ...
    const rows=visible.map(p=>{
      ...
      const offer=promoCompactText(p);
      const st=status(p);
      const outRibbon=st.cls==='out'?'<span class="stockout-ribbon-v181 stockout-ribbon-share-v181" aria-hidden="true">AGOTADO</span>':'';
      return `<article class="categoryShareItem-v163 ${st.cls==='out'?'is-out':''}">
        ${outRibbon}
        ...
        <div><h3>${escapeHtml(p.name)}</h3><strong>${money(p.price)}</strong><p><b>${num(stockQty)}</b> ${st.cls==='out'?'disponibles':'disponibles'}</p>${colors?`<small>🎨 ${escapeHtml(colors)}</small>`:''}${offer?`<small class="offer">🏷️ ${escapeHtml(offer)}</small>`:''}</div>
      </article>`;
    }).join('') || `<div class="categoryShareEmpty-v163">Sin productos para mostrar.</div>`;
    return `<section class="categoryShareClean-v163" data-export="category-share-clean">
      <header class="categoryShareHead-v163">
        <img src="${exportLogoSrc()}" alt="SD Comayagua" loading="eager" decoding="sync">
        <div><span>SD COMAYAGUA</span><h2>${escapeHtml(title)}</h2><p>${num((list||[]).length)} producto${(list||[]).length===1?'':'s'} · ${nowHNPanel()}</p></div>
      </header>
      <div class="categoryShareGrid-v163">${rows}</div>
      ${hidden?`<div class="categoryShareMore-v163">+ ${num(hidden)} productos más en la página</div>`:''}
      <footer class="categoryShareFooter-v163">Foto · nombre · precio · stock · colores · WhatsApp +504 3151-7755</footer>
    </section>`;
  }
  async function exportCategorySnapshot(mode='download'){
    const list=categorySnapshotList();
    if(!list.length) return toast('No hay productos en esta categoría para capturar.');
    const title=categorySnapshotTitle();
    const host=document.createElement('div');
    host.className='categoryShareExportHost-v163';
    ...
    try{
      ...
      if(mode==='share' && navigator.share && navigator.canShare && navigator.canShare({files:[file]})){
        ...
        toast('Captura de categoría descargada en PNG.');
      }
    }catch(err){
      console.error(err);
  ...
  function openCategoryPrintPreview(){
    const list=categorySnapshotList();
    if(!list.length) return toast('No hay productos en esta categoría para imprimir.');
    const title=categorySnapshotTitle();
    openModal(`<div class="modal-head"><h3>Catálogo para cliente</h3><button class="close">×</button></div><div class="modal-body categoryPrintModal-v163"><div class="categorySharePrint-v163">${categorySnapshotHTML(list,title)}</div><div class="modal-actions categoryPrintActions-v163" style="position:static"><button class="btn" id="printCategoryV163">Imprimir</button><button class="btn secondary" id="downloadCategoryV163">Descargar PNG</button></div></div>`,true);
  ...
  function shortReceiptCardHTML(doc){
    ...
    return `<div class="short-receipt short-receipt-v34 short-receipt-v36 short-receipt-v72 short-receipt-v147 short-receipt-v156 short-receipt-v157 short-receipt-v158" id="shortReceiptCard"><div class="short-brand short-brand-v147 short-brand-v156 short-brand-v157 short-brand-v158"><div class="short-logo-box"><span>SD</span><img class="receipt-logo-inline" src="${exportLogoSrc()}" alt="SD Comayagua" loading="eager" decoding="sync"></div><div class="short-copy-v153"><div class="short-kicker-v157 short-kicker-v158">Recibo visual para cliente</div><h2>SD COMAYAGUA</h2><p class="short-meta short-meta-v50"><span class="short-client-name">${escapeHtml(doc.client||'Cliente')}</span><em class="short-date-time">${nowHN()}</em></p>${variant}</div></div><div class="short-highlight-v158"><span>Resumen claro</span><b>Total listo para enviar por WhatsApp.</b></div><div class="short-section-title-v158">Detalle del pedido</div><div class="short-items-v156 short-items-v157 short-items-v158">${shortReceiptLinesHTML(doc)}</div><div class="short-summary-v157 short-summary-v158"><div class="short-line summary-row-v156 summary-row-v157 summary-row-v158"><span>Subtotal de productos</span><b>${money(c.products)}</b></div><div class="short-line summary-row-v156 summary-row-v157 summary-row-v158"><span>${shippingLabel(doc)}</span><b>${money(c.shipping)}</b></div>${c.commission?`<div class="short-line summary-row-v156 summary-row-v157 summary-row-v158"><span>Comisión</span><b>${money(c.commission)}</b></div>`:''}<div class="grand short-line grand-v156 grand-v157 grand-v158"><span>Total a pagar</span><b>${money(c.total)}</b></div></div><div class="short-note-v157 short-note-v158">Confirme disponibilidad, entrega y pago antes de cerrar el pedido.</div><small class="short-wa-v158">WhatsApp +504 3151-7755</small></div>`;
  }
