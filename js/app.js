(function(){
    ...
    return `<section class="hero v51-hero" id="inicio">
      <div class="v51-hero-shell">
        <div class="private-hero-head private-hero-head--no-logo">
          <div>
            <div class="pill v51-kicker"><span class="dot"></span> Panel administrativo</div>
            <h2>Ventas más rápidas, inventario más claro.</h2>
            <p>Gestiona productos, cotiza, registra ventas y prepara contenido listo para WhatsApp desde una sola pantalla.</p>
          </div>
          <div class="v51-hero-actions no-print">
            <button type="button" data-action="sell">Nueva venta</button>
  ...
  function inventoryHeadHTML(count){return `<div class="section-head inventory-head-v31"><div><h2>Inventario</h2><p>Busca, filtra y abre productos para cotizar, vender o compartir por WhatsApp.</p></div><div class="inventory-head-actions"><button class="inventory-new-btn" data-action="newProduct" type="button">+ Nuevo producto</button>${inventoryControls()}<span class="count-pill">${count} resultados</span></div></div>`}
  ...
  function searchPanel(){
    return `<section class="search-panel v10-search clean-search no-print" id="searchPanel"><div class="search-title"><b>Buscar rápido</b><span>Usa / o Ctrl+K para encontrar productos</span></div><div class="searchbar"><span class="icon">⌕</span><input id="searchInput" placeholder="Producto, código, categoría o detalle" value="${escapeHtml(filter.q)}" autocomplete="off" inputmode="search"></div></section>`}

  function categoryGallery(){
    const cats=allCategories();
    return `<section class="category-gallery no-print" id="categoriesBlock"><div class="category-head"><div><h2>Categorías</h2><p>Filtros rápidos para preparar ventas y publicaciones sin perder tiempo.</p></div><span>${cats.length-1} categorías</span></div><div class="category-grid">${cats.map(c=>`<button class="category-card ${filter.cat===c?'active':''}" data-catcard="${escapeHtml(c)}"><img src="${escapeHtml(categoryImage(c))}" alt="${escapeHtml(c)}" onerror="this.onerror=null;this.src='assets/categorias/categoria.svg'"><b>${escapeHtml(c)}</b><small>${categoryCount(c)} productos</small></button>`).join('')}</div></section>`
  }
    ...
    return `<article class="product-card product-card-v49 ${inventoryLayout()==='one'?'wide':'compact'}" data-id="${id}" data-product-id="${id}">
      ...
      <div class="v49-card-body">
        ...
        <div class="v49-card-actions v49-card-actions-single">
          ...
          <button type="button" data-action="waProduct" data-id="${id}">WhatsApp</button>
        </div>
      </div>
    </article>`;
  }
