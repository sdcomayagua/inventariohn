...
📲 WhatsApp: +504 3151-7755`;
  ...
  function shortReceiptLinesHTML(doc){
    return (doc.items||[]).map(it=>{
      ...
      return `<div class="short-line item item-v156 short-item-row-v197"><span class="short-item-copy-v197"><strong>${escapeHtml(it.name)}</strong><small>${qtyText} · ${money(unit)} c/u</small>${color?`<small class="short-color-v197">Color: ${escapeHtml(color)}</small>`:''}</span><b class="short-item-total-v197">${money(total)}</b></div>`;
    }).join('');
  }
