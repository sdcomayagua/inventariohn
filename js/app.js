...
(function(){
  ...
  function pageTabsV150(){
    ...
    return `<nav class="sdc-tabs-v150 sdc-tabs-v177 no-print" aria-label="Secciones principales">
  ...
  function topbar(){
    ...
    return `<header class="sdc-shell-top-v177 no-print" role="banner">
      <div class="sdc-action-row-v177">
        <button class="sdc-menu-main-v177" type="button" data-sdc127="open" aria-label="Abrir menú"><span class="sdc-burger-v177" aria-hidden="true"></span><b>MENÚ</b></button>
        <button class="sdc-sync-main-v177" type="button" data-action="sync" title="Sincronizar desde Firebase (${escapeHtml(lastSync)})" aria-label="Actualizar Firebase"><span aria-hidden="true">↻</span><b>ACTUALIZAR FIREBASE</b></button>
      </div>
      <section class="sdc-brand-card-v177" aria-label="SD Comayagua">
        <div class="sdc-brand-logo-v177"><img src="${LOGO_SRC}" alt="SD Comayagua"></div>
        <div class="sdc-brand-copy-v177">
          <h1>SD COMAYAGUA</h1>
          <p>CATÁLOGO · VENTAS · COTIZACIONES · INVENTARIO</p>
          <span><i aria-hidden="true">▣</i> Panel activo · ${nowHNPanel()}</span>
        </div>
      </section>
    </header>`
  }
  ...
  function inventoryHeadHTML(count){
    ...
    return `<div class="products-board-v177">
      <section class="products-hero-v177">
        <div class="products-hero-copy-v177">
          <span class="products-kicker-v177">Catálogo digital</span>
          <h2>Productos</h2>
          <p>Foto, precio, stock, colores y ofertas.</p>
        </div>
        <div class="products-status-v177">
          ${sheetsButtonHTML()}
          <span class="count-pill">${count} resultados</span>
        </div>
      </section>
      <section class="products-tools-v177 no-print" aria-label="Herramientas de productos">
        <label class="products-search-v177" for="inventorySearchInput"><i aria-hidden="true">⌕</i><input id="inventorySearchInput" data-product-search="1" placeholder="Buscar producto, código o categoría..." value="${escapeHtml(filter.q)}" autocomplete="off" inputmode="search"></label>
        <label class="products-category-v177" for="inventoryCategorySelect"><i aria-hidden="true">▦</i><select id="inventoryCategorySelect" aria-label="Filtrar por categoría">${inventoryCategoryOptionsHTML()}</select><b aria-hidden="true">⌄</b></label>
        <div class="products-actions-v177">
          <button class="products-utility-v177" data-action="categoryCapture" type="button" title="Crear imagen limpia para cliente"><span>▣</span><b>Captura ${escapeHtml(catLabel)}</b></button>
          <button class="products-utility-v177" data-action="categoryPrint" type="button" title="Vista imprimible por categoría"><span>▥</span><b>Imprimir</b></button>
        </div>
        <button class="products-new-v177" data-action="newProduct" type="button"><span>+</span><b>Producto</b></button>
      </section>
    </div>`
  }
  ...
  function productCard(p){
    ...
    const cat=firstTag(p);
    const adminButton=cardView()==='admin'?`<button type="button" class="product-admin-v177" data-action="adminProduct" data-id="${id}">Admin</button>`:'';
    return `<article class="product-card product-card-v177 ${isOut?'is-out is-agotado':''}" data-id="${id}" data-product-id="${id}" data-stock-status="${st.cls}">
      <div class="product-main-row-v177">
        <button type="button" class="product-media-v177" data-action="viewProduct" data-id="${id}" aria-label="Ver detalle de ${escapeHtml(p.name)}">
          ...
          <span class="product-status-v177 ${st.cls}"><i></i>${st.text}</span>
        </button>
        <div class="product-info-v177">
          <div class="product-tags-v177"><span>${escapeHtml(cat||'Producto')}</span>${id?`<span>${id}</span>`:''}</div>
          <h3 data-action="viewProduct" data-id="${id}">${escapeHtml(p.name)}</h3>
          <div class="product-price-line-v177"><strong class="${isOut?'is-out':''}">${money(p.price)}</strong><em class="${st.cls}">${isOut?'0 disponibles':`${num(stockQty)} disp.`}</em></div>
        </div>
      </div>
      <div class="product-bottom-row-v177">
        <div class="product-facts-v177">
          ${colors?`<span><i>◌</i>${escapeHtml(colors)}</span>`:''}
          ${offer?`<span class="offer"><i>◇</i>${escapeHtml(offer)}</span>`:''}
        </div>
        <div class="product-card-actions-v177">
          ${adminButton}
          <button type="button" data-action="viewProduct" data-id="${id}">Ver / vender</button>
        </div>
      </div>
    </article>`;
  }
  ...
  function categorySnapshotHTML(list,title){
    ...
    const rows=visible.map(p=>{
      ...
      return `<article class="categoryShareItem-v163 ${st.cls==='out'?'is-out':''}">
        ...
        <div><h3>${escapeHtml(p.name)}</h3><strong>${money(p.price)}</strong><p><b>${num(stockQty)}</b> disponibles</p>${colors?`<small>🎨 ${escapeHtml(colors)}</small>`:''}${offer?`<small class="offer">🏷️ ${escapeHtml(offer)}</small>`:''}</div>
      </article>`;
