(function(){
  ...
  function topbar(){const turbo=currentAppearance()==='turbo'; return `<header class="topbar topbar-private"><img class="top-logo" src="${LOGO_SRC}" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1><p>${turbo?'Privado · foto, texto y venta':'Privado · cliente no entra aquí'}</p></div><div class="spacer"></div><button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro'}</button><button class="btn small ghost sync-btn" data-action="sync">Sync</button><button class="btn small secondary" data-action="lock">Salir</button></header>`}
  function hero(){
    const st=stats();
    return `<section class="hero" id="inicio">
      <div class="private-hero-head private-hero-head--no-logo"><div><div class="pill"><span class="dot"></span> Panel privado</div><h2>CONTROL SDC</h2><p>Uso interno: aquí preparas inventario, venta, captura y texto. El cliente solo recibe lo que tú le compartes.</p><div class="hero-mini-actions no-print"><button data-action="cardClient">Vista cliente</button><button data-action="captureClean">Captura limpia</button><button data-action="quote">Cotizar</button></div></div></div>
      <div class="stats">
  ...
  function quickPanel(){
    ...
    return `<section class="quick no-print quick-v22 quick-private">
      <button data-action="cardClient" class="priority-action"><b>Cliente</b><span>Vista para foto/texto</span></button>
      <button data-action="captureClean" class="priority-action"><b>Captura</b><span>Oculta lo privado</span></button>
      <button data-action="quote" class="priority-action"><b>Cotizar</b><span>Total para WhatsApp</span></button>
      <button data-action="sell"><b>Vender</b><span>Registrar venta</span></button>
  ...
  function cardModePanel(){
    const mode=cardView();
    return `<section class="view-mode-panel no-print"><div class="view-mode-copy"><b>Modo de trabajo</b><span>${mode==='admin'?'Admin: costos, inversión y ganancia solo para ti.':'Cliente: precio, descripción y totales listos para enviar.'}</span></div><div class="view-mode-buttons"><button class="${mode==='admin'?'active':''}" data-action="cardAdmin">ADMIN</button><button class="${mode==='client'?'active':''}" data-action="cardClient">CLIENTE</button><button class="${state.settings.captureClean?'active':''}" data-action="captureClean">CAPTURA</button></div></section>`
  }
  function searchPanel(){
    return `<section class="search-panel v10-search clean-search no-print" id="searchPanel"><div class="search-title"><b>Buscar rápido</b><span>Producto, código o categoría antes de compartir</span></div><div class="searchbar"><span class="icon">⌕</span><input id="searchInput" placeholder="Buscar producto para cliente..." value="${escapeHtml(filter.q)}" autocomplete="off" inputmode="search"></div></section>`}
  function categoryGallery(){
    const cats=allCategories();
    return `<section class="category-gallery no-print" id="categoriesBlock"><div class="category-head"><div><h2>CATEGORÍAS</h2><p>Desliza y toca para filtrar rápido.</p></div><span>${cats.length-1} categorías</span></div><div class="category-grid">${cats.map(c=>`<button class="category-card ${filter.cat===c?'active':''}" data-catcard="${escapeHtml(c)}"><img src="${escapeHtml(categoryImage(c))}" alt="${escapeHtml(c)}" onerror="this.onerror=null;this.src='assets/categorias/categoria.svg'"><b>${escapeHtml(c)}</b><small>${categoryCount(c)} productos</small></button>`).join('')}</div></section>`
  }
