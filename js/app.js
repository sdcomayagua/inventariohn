(() => {
  ...
  function categoriesOf(product){ return parseCategories(product?.categories ?? product?.categorias ?? product?.category ?? product?.categoria).filter(Boolean); }
  ...
  function calcQuote(q){
    const products = (q.items || []).reduce((s,it)=>s + toNumber(it.price) * toNumber(it.qty || 1), 0);
    const type = q.deliveryType || 'normal';
    let shipping = 110;
    let commission = 0;
    if(type === 'domicilio') shipping = toNumber(q.manualShipping);
    if(type === 'normal') shipping = 110;
    if(type === 'cod'){
      shipping = 100;
      commission = Math.round((products + shipping) * 0.06);
    }
    const totalShipping = shipping + commission;
    const discount = toNumber(q.discount);
    const total = Math.max(0, products + totalShipping - discount);
    return { products, shipping, commission, totalShipping, discount, total };
  }
  ...
  async function shareReceiptImage(){
    ...
    else { await captureReceipt('cotizacion-sdc.png'); openWhatsAppToClient(quoteText(state.quote), state.quote.phone); }
  }
  ...
  function openWhatsAppToClient(text, suggestedPhone=''){
    const suggested = normalizePhone(suggestedPhone || '');
    const phone = prompt('Número del cliente para WhatsApp.\nDéjalo vacío para elegir el chat manualmente en WhatsApp.', suggested || '');
  ...
  function handleAction(action, el){
    ...
    if(action === 'send-quote-text') { updateAllQuoteInputs(); openWhatsAppToClient(quoteText(state.quote), state.quote.phone); }
    ...
    if(action === 'send-receipt-text') { const q = state.data.sales.find(x=>x.id===el.dataset.id); if(q) openWhatsAppToClient(quoteText(q, q.totals || calcQuote(q)), q.phone); }
  }
