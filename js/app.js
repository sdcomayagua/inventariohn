(function(){
  ...
  function openProductDetails(id){
    ...
    openModal(`<div class="modal-head"><h3>Producto</h3><button class="close">×</button></div><div class="modal-body"><div class="product-share-card" id="productShareCard"><div class="product-share-media"><img id="detailMainImage" src="${escapeHtml(safeImgs[0])}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><b class="price-badge">${money(p.price)}</b></div><h2>${escapeHtml(p.name)}</h2><p>${escapeHtml(p.description||'Sin descripción registrada.')}</p><div class="public-metrics"><div><span>Precio</span><b>${money(p.price)}</b></div><div><span>Stock</span><b>${Number(p.stock)>0?num(p.stock)+' disponible':'Consultar'}</b></div><div><span>Categoría</span><b>${escapeHtml(parseTags(p.categories).join(', ')||'General')}</b></div></div>${promos?`<div class="promo-public"><span>Promociones por cantidad</span><pre>${escapeHtml(promos)}</pre></div>`:''}<div class="share-footer">SD COMAYAGUA · WhatsApp +504 3151-7755</div></div><div class="thumb-row">${safeImgs.map((img,i)=>`<button class="thumb ${i===0?'active':''}" data-product-img="${escapeHtml(img)}"><img src="${escapeHtml(img)}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"></button>`).join('')}</div><div class="modal-actions product-actions"><button class="btn secondary" id="downloadProductPhoto">↓ Descargar imagen</button><button class="btn" id="shareProductPhoto">Compartir foto</button><button class="btn secondary" id="waProductText">WhatsApp texto</button><button class="btn" data-action="sellProduct" data-id="${escapeHtml(id)}">Vender</button><button class="btn secondary" data-action="quoteProduct" data-id="${escapeHtml(id)}">Cotizar</button><button class="btn ghost" data-action="editProduct" data-id="${escapeHtml(id)}">Editar</button></div></div>`);
