(function(){
  ...
  function cloudReady(){return !!(window.SDCCloud && SDCCloud.configured(state));}
  ...
  function cloudStatusHTML(){
    const ok=cloudReady();
    const status=state.settings.lastCloudStatus || (ok?'Nube conectada':'Configurar nube');
    return `<div class="cloud-status ${ok?'ok':'warn'}"><b>${ok?'☁ Sincronización':'☁ Local'}</b><span>${escapeHtml(ok?`${status} · ${cloudTimeLabel()}`:'Aún no está conectado a Google Sheets')}</span></div>`;
  }
  async function cloudBackup(label='manual', silent=false){
    if(!cloudReady()){ if(!silent) openCloudSetup(); return false; }
    ...
    try{
      ...
      state.settings.lastCloudStatus = 'Guardado';
      SDCStore.save(state);
      if(!silent) toast('Respaldo completo guardado en Google Sheets.');
      return true;
    }catch(err){
      console.error(err);
      state.settings.lastCloudStatus = 'Error de nube';
      SDCStore.save(state);
      if(!silent) toast('No se pudo guardar en nube. Revisa URL, permisos o PIN.');
      return false;
    }finally{cloudBusy=false;}
  }
  async function syncCloud(opts={}){
    if(!cloudReady()){openCloudSetup(); return;}
    if(cloudBusy) return;
    try{
      cloudBusy=true;
      if(!opts.silent) toast('Sincronizando con Google Sheets...');
      ...
      if(!opts.silent) toast('Listo: celular y computadora quedan sincronizados.');
    }catch(err){
      console.error(err);
      state.settings.lastCloudStatus='Error de sincronización';
      SDCStore.save(state);
      if(!opts.silent) toast('No se pudo sincronizar. Revisa Apps Script, permisos o PIN.');
    }finally{cloudBusy=false;}
  }
  function openCloudSetup(){
    openModal(`<div class="modal-head"><h3>Sincronización en nube</h3><button class="close">×</button></div><div class="modal-body cloud-setup"><div class="card-box cloud-intro"><h4>Conectar Google Sheets + Apps Script</h4><p>Esto permite que lo que hagas en celular se vea en computadora: inventario, ventas, cotizaciones, clientes, gastos, cierres y respaldos completos.</p>${cloudStatusHTML()}</div><label><span class="label">URL Web App /exec de Apps Script</span><input id="syncUrlInput" class="input" placeholder="https://script.google.com/macros/s/.../exec" value="${escapeHtml(state.settings.syncUrl||'')}"></label><label><span class="label">PIN de nube</span><input id="syncPinInput" class="input" type="password" inputmode="numeric" placeholder="PIN configurado en Apps Script" value="${escapeHtml(state.settings.syncPin||'')}"></label><label class="switch-row"><span><b>Guardar automáticamente</b><small>Cada cambio importante se respalda en Google Sheets sin tocar otro botón.</small></span><input id="syncAutoInput" type="checkbox" ${state.settings.syncAuto?'checked':''}></label><div class="modal-actions import-actions" style="position:static"><button class="btn full" id="saveCloudConfig">Guardar y probar conexión</button><button class="btn secondary full" id="syncNowCloud">Sincronizar ahora</button><button class="btn ghost full" id="backupNowCloud">Respaldar todo ahora</button></div><div class="empty-state">Primero copia el archivo <b>apps-script/Code.gs</b> en Apps Script, ejecuta <b>setup()</b>, publica como Web App y pega aquí la URL /exec.</div></div>`,true);
    $('#saveCloudConfig').onclick=async()=>{
      state.settings.syncUrl=$('#syncUrlInput').value.trim();
      state.settings.syncPin=$('#syncPinInput').value.trim();
      state.settings.syncAuto=$('#syncAutoInput').checked;
      save();
      try{await SDCCloud.ping(state); state.settings.lastCloudStatus='Conectado'; state.settings.lastCloudSyncAt=new Date().toISOString(); save(); toast('Conexión correcta con Google Sheets.'); closeModal(); render();}
      catch(err){console.error(err); toast('No conectó. Revisa URL /exec, permisos y PIN.');}
    };
    $('#syncNowCloud').onclick=()=>{state.settings.syncUrl=$('#syncUrlInput').value.trim(); state.settings.syncPin=$('#syncPinInput').value.trim(); state.settings.syncAuto=$('#syncAutoInput').checked; save(); closeModal(); syncCloud();};
    $('#backupNowCloud').onclick=()=>{state.settings.syncUrl=$('#syncUrlInput').value.trim(); state.settings.syncPin=$('#syncPinInput').value.trim(); state.settings.syncAuto=$('#syncAutoInput').checked; save(); cloudBackup('manual');};
  }
  ...
  function topbar(){
    ...
    return `<header class="topbar topbar-v28"><img class="top-logo" src="assets/logo-sdc.png" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1><p><span class="mini-led ${cloud?'ok':'warn'}"></span>${turbo?'Modo Pro Gamer':'Modo Gamer normal'} · ${cloud?'Nube activa':'Local'} · V28</p></div><div class="spacer"></div><button class="btn small ghost cloud-btn" data-action="cloudSetup">☁</button><button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro Gamer'}</button><button class="btn small ghost sync-btn" data-action="sync">Sync</button><button class="btn small secondary exit-btn" data-action="lock">Salir</button></header>`
  }
  function hero(){
    const st=stats();
    ...
    return `<section class="hero hero-v27 hero-v28" id="inicio">
  ...
  function quickPanel(){
    ...
    return `<section class="quick no-print quick-v22">
      ...
      <button data-action="cloudSetup"><b>Nube</b><span>Sheets / App Script</span></button>
    </section>
  ...
  function mainAction(e){
    ...
    if(a==='sync') return cloudReady()?syncCloud():syncLocal();
