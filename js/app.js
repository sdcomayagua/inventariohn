...
WhatsApp SD COMAYAGUA: +504 3151-7755`;
  ...
  function docCard(doc,isSale){
    ...
    const rows=(doc.items||[]).map((it,i)=>`<div class="doc-item-row"><span class="doc-idx">${i+1}</span><span class="doc-item-name"><b>${escapeHtml(it.name)}</b><small>${escapeHtml(it.id||'')}</small></span><span class="doc-num">${num(it.qty)}</span><span class="doc-num">${money(it.price)}</span><span class="doc-num total">${money(itemTotal(it))}</span></div>`).join('')||'<div class="doc-item-empty">Sin productos agregados</div>';
    return `<div class="doc-wrap compact-doc doc-v21" id="printableDoc"><div class="doc-topline">${isSale?'RECIBO / FACTURA DE COMPRA':'COTIZACIÓN PARA CLIENTE'}</div><div class="doc-head compact-head"><div><span class="doc-pill">${isSale?'Venta registrada':'Preventa informativa'}</span><h2>SD COMAYAGUA</h2><p>${date} · <b>${escapeHtml(code)}</b></p></div><img class="doc-logo" src="assets/logo-sdc.png" alt="Logo"></div><div class="doc-fields compact-fields"><div class="doc-field"><span>Cliente</span><b>${escapeHtml(doc.client||'Cliente')}</b></div><div class="doc-field"><span>Teléfono</span><b>${escapeHtml(doc.phone||'No registrado')}</b></div><div class="doc-field"><span>Ubicación</span><b>${escapeHtml([doc.department,doc.municipality].filter(Boolean).join(' / ')||'No seleccionada')}</b></div><div class="doc-field"><span>Entrega</span><b>${escapeHtml(doc.company||'No seleccionada')}</b></div>${doc.reference?`<div class="doc-field wide"><span>Referencia</span><b>${escapeHtml(doc.reference)}</b></div>`:''}</div><div class="doc-process"><b>Proceso de pago:</b> ${process}</div><div class="doc-items-v21"><div class="doc-items-head"><span>#</span><span>Producto</span><span>Cant.</span><span>Precio</span><span>Total</span></div>${rows}</div><div class="doc-mini-summary doc-summary-v21"><div><span>Productos</span><b>${money(c.products)}</b></div><div><span>Envío</span><b>${money(c.shipping)}</b></div><div><span>Comisión</span><b>${money(c.commission)}</b></div><div><span>Descuento</span><b>${money(c.discount)}</b></div><div class="grand"><span>Total a pagar</span><b>${money(c.total)}</b></div></div><p class="doc-note compact-note">${isSale?'Gracias por comprar en SD Comayagua.':'Cotización informativa. Revise producto, cantidad, envío y total antes de confirmar.'} WhatsApp: +504 3151-7755</p></div>`
  }
  ...
  async function shareDocPhoto(isSale){const doc=currentDoc(isSale); if(!doc.items.length)return toast('Agrega productos primero.'); const c=calc(doc); if(c.products<=0||c.total<=0)return toast('El total está en cero. Revisa producto, precio y envío antes de enviar.'); save(); const blob=await docToBlob(); const text=whatsappText(doc,isSale); const filename=`${isSale?'recibo':'cotizacion'}-${clientLabel(doc)}-${fileStamp()}-${slugFile(doc.id||'sdc')}.png`; if(blob && navigator.canShare){const file=new File([blob],filename,{type:'image/png'}); if(navigator.canShare({files:[file]})){try{await navigator.share({files:[file],text,title:isSale?'Recibo SD Comayagua':'Cotización SD Comayagua'}); toast('Selecciona WhatsApp y el chat del cliente. La foto y el texto van juntos.'); return;}catch(e){if(e && e.name==='AbortError')return;}}} if(blob){const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=filename; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),1000);} const phone=chooseWaPhone(doc); if(phone!==null) openWhatsApp(phone,text); toast('No se pudo compartir directo con foto. Descargué la imagen y abrí WhatsApp con el texto.');}
  function finishSale(){
  ...
  function saleProfit(s){return (s.items||[]).reduce((a,it)=>a+((Number(it.price||0)-Number(it.cost||0))*Number(it.qty||0)),0)}
  ...
  function openReceipts(){
    ...
    const total=today.reduce((a,s)=>a+(s.total||calc(s).total),0); const profit=today.reduce((a,s)=>a+saleProfit(s),0); const pending=sales.filter(s=>/pendiente|recibir/i.test(s.status||s.paymentStatus||'')).reduce((a,s)=>a+(s.total||calc(s).total),0);
    openModal(`<div class="modal-head"><h3>Caja / recibos</h3><button class="close">×</button></div><div class="modal-body receipts-v22"><div class="cash-stats"><div><span>Vendido hoy</span><b>${money(total)}</b></div><div><span>Ganancia hoy</span><b>${money(profit)}</b></div><div><span>Ventas hoy</span><b>${num(today.length)}</b></div><div><span>Pendiente</span><b>${money(pending)}</b></div></div><button class="btn full" data-action="dailyClose">Cierre del día</button><div class="cart-list">${sales.map(s=>`<div class="cart-row"><div><b>${escapeHtml(s.client||'Cliente')}</b><br><span>${escapeHtml(s.id)} · ${money(s.total||calc(s).total)} · ${escapeHtml(s.status||s.paymentStatus||'Venta')}</span></div><button class="btn small secondary" data-openreceipt="${s.id}">Editar</button></div>`).join('')||'<div class="empty-state">Todavía no hay ventas registradas.</div>'}</div></div>`,true);
  ...
  function openDailyClose(){
    const today=(state.sales||[]).filter(s=>isTodayISO(s.date)); const total=today.reduce((a,s)=>a+(s.total||calc(s).total),0); const products=today.reduce((a,s)=>a+calc(s).products,0); const delivery=today.reduce((a,s)=>a+calc(s).delivery,0); const profit=today.reduce((a,s)=>a+saleProfit(s),0);
    const txt=`CIERRE DEL DÍA - SD COMAYAGUA\nFecha: ${nowHN()}\nVentas: ${today.length}\nProductos: ${money(products)}\nEnvío/comisión: ${money(delivery)}\nTotal vendido: ${money(total)}\nGanancia estimada: ${money(profit)}`;
    openModal(`<div class="modal-head"><h3>Cierre del día</h3><button class="close">×</button></div><div class="modal-body"><div class="cash-stats"><div><span>Ventas</span><b>${num(today.length)}</b></div><div><span>Total</span><b>${money(total)}</b></div><div><span>Ganancia</span><b>${money(profit)}</b></div><div><span>Envío/comisión</span><b>${money(delivery)}</b></div></div><textarea class="textarea" id="closeText">${escapeHtml(txt)}</textarea><div class="modal-actions" style="position:static"><button class="btn" id="saveClose">Guardar cierre</button><button class="btn secondary" id="copyClose">Copiar resumen</button></div></div>`,true);
    $('#copyClose').onclick=()=>{navigator.clipboard?.writeText($('#closeText').value); toast('Resumen copiado.');};
    $('#saveClose').onclick=()=>{state.closings.unshift({id:'CIERRE-'+Date.now(),date:new Date().toISOString(),total,profit,count:today.length,text:$('#closeText').value}); save(); SDCStore.saveBackup(state,'Cierre del día'); toast('Cierre guardado.');};
  }
