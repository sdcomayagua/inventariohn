...
(function(){
  ...
  function inventoryHeadHTML(count){
    const catLabel=filter.cat&&filter.cat!=='Todos'?filter.cat:'catálogo';
    const st=stats();
    return `<div class="products-screen-v178">
      <section class="catalog-cover-v178" aria-label="Catálogo digital">
        <div class="catalog-cover-title-v178">
          <span class="catalog-chip-v178">Catálogo digital</span>
          <h2>Productos</h2>
          <p>Foto, precio, stock, colores y ofertas.</p>
        </div>
        <div class="catalog-metrics-v178" aria-label="Resumen de catálogo">
          <article><span>Resultados</span><b class="count-pill">${count} resultados</b></article>
          <article><span>Unidades</span><b>${num(st.stock)}</b></article>
          <article><span>Nube</span><b><i></i>Firebase</b></article>
        </div>
      </section>
      <section class="catalog-control-v178 no-print" aria-label="Herramientas de productos">
        <label class="catalog-search-v178" for="inventorySearchInput"><i aria-hidden="true">⌕</i><input id="inventorySearchInput" data-product-search="1" placeholder="Buscar producto, código o categoría..." value="${escapeHtml(filter.q)}" autocomplete="off" inputmode="search"></label>
        <div class="catalog-filter-row-v178">
          <label class="catalog-category-v178" for="inventoryCategorySelect"><span aria-hidden="true">▦</span><select id="inventoryCategorySelect" aria-label="Filtrar por categoría">${inventoryCategoryOptionsHTML()}</select><b aria-hidden="true">⌄</b></label>
          <button class="catalog-add-v178" data-action="newProduct" type="button"><span>+</span><b>Producto</b></button>
        </div>
        <div class="catalog-utility-row-v178">
          <button class="catalog-tool-v178" data-action="categoryCapture" type="button" title="Crear imagen limpia para cliente"><span aria-hidden="true">▣</span><b>Captura ${escapeHtml(catLabel)}</b></button>
          <button class="catalog-tool-v178" data-action="categoryPrint" type="button" title="Vista imprimible por categoría"><span aria-hidden="true">▤</span><b>Imprimir</b></button>
        </div>
      </section>
    </div>`
  }
  ...
  function productCard(p){
    ...
    const cat=firstTag(p);
    const adminButton=cardView()==='admin'?`<button type="button" class="product-admin-v178" data-action="adminProduct" data-id="${id}">Admin</button>`:'';
    const actionLabel=isOut?'Solo ver':'Ver / vender';
    ...
    return `<article class="product-card product-card-v178 ${isOut?'is-out is-agotado':''}" data-id="${id}" data-product-id="${id}" data-stock-status="${st.cls}">
      ${outRibbon}
      ...
      <div class="product-actions-v178">
        ${adminButton}
        <button type="button" data-action="viewProduct" data-id="${id}" class="${isOut?'out-view':''}">${actionLabel}</button>
      </div>
    </article>`;
  }
