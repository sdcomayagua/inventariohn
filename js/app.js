...
(function(){
  ...
  function categoryQuickRailHTML(){
    ...
    return `<section class="category-selector-v195 no-print" aria-label="Categorías del catálogo">
      ...
      <div class="category-current-v195"><b>${escapeHtml(label)}</b><small>${num(activeCount)} productos · ${num(totalCats)} categorías</small><div class="category-mini-actions-v198"><button type="button" class="category-mini-btn-v198" data-action="categoryPrint">Imprimir</button><button type="button" class="category-mini-btn-v198 secondary" data-action="categoryCapture">Vista cliente</button></div></div>
    </section>`;
  }
...
📲 WhatsApp: +504 3151-7755`;
  ...
  function categorySnapshotHTML(list,title){
    const source=list||[];
    const visible=source.slice(0,64);
    const hidden=Math.max(0,source.length-visible.length);
    const sum=categorySnapshotSummary(source);
    const priceRange=sum.min&&sum.max?(sum.min===sum.max?money(sum.min):`${money(sum.min)} - ${money(sum.max)}`):'Consultar';
    const rows=visible.map(p=>{
      ...
      return `<article class="categoryShareItem-v199 ${st.cls==='out'?'is-out':''}">
        ...
        <div class="categoryShareInfo-v199">
          ...
          <div class="categorySharePriceRow-v199"><strong>${money(p.price)}</strong><em>${num(stockQty)} disp.</em></div>
    ...
    return `<section class="categoryShareClean-v199" data-export="category-share-clean">
      <header class="categoryShareHead-v199">
        <div class="categoryShareBrand-v199">
          ...
          <div><span>SD COMAYAGUA</span><h2>${escapeHtml(title)}</h2><p>Vista rápida para cliente · ${nowHNPanel()}</p></div>
        </div>
      ...
      <div class="categoryShareGrid-v199">${rows}</div>
  ...
  function renderPicker(isSale){
    ...
    function draw(){
      ...
      list.innerHTML=(items.map(p=>`<div class="picker-item picker-item-v200"><img src="${escapeHtml(productImage(p))}" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><div><b>${escapeHtml(p.name)}</b><span>${money(p.price)} · Stock ${num(productStock(p))}${colorStockSummary(p,2)?` · ${escapeHtml(colorStockSummary(p,2))}`:''} · ${escapeHtml(firstTag(p))}</span></div><button class="btn small add-pick-btn" type="button" data-additem="${escapeHtml(p.id)}">Añadir</button></div>`).join('')+limitNote)||'<div class="picker-empty"><b>Sin productos para mostrar</b><span>Pruebe otra categoría o revise el texto de búsqueda.</span></div>';
  ...
  function shortReceiptExportCSS(){return `
    .shortReceiptExportHost{position:fixed;left:-10000px;top:0;width:860px;padding:28px;background:#eaf3fb;z-index:-1}
    .shortReceiptExportHost *{box-sizing:border-box}
    .shortReceiptExportHost .sdc208-ticket{width:804px;max-width:804px;margin:0 auto;padding:22px;border-radius:34px;background:#ffffff;color:#07192f;border:1px solid #d8e7f6;box-shadow:0 20px 52px rgba(7,26,53,.10);overflow:hidden;font-family:Barlow,Arial,sans-serif}
    .shortReceiptExportHost .sdc208-head{display:grid;grid-template-columns:76px minmax(0,1fr);grid-template-areas:"logo copy" "mode mode";gap:16px;align-items:center;padding:20px;border-radius:28px;background:linear-gradient(135deg,#071a35 0%,#0b63ce 72%,#0a7cf2 100%);color:#fff}
    .shortReceiptExportHost .sdc208-logo{grid-area:logo;width:76px;height:76px;border-radius:22px;background:#fff;display:grid;place-items:center;border:1px solid rgba(255,255,255,.7);box-shadow:0 16px 32px rgba(0,0,0,.16)}
    .shortReceiptExportHost .sdc208-logo img{width:58px;height:58px;object-fit:contain}
    ...
    .shortReceiptExportHost .sdc208-head-copy h2{margin:8px 0 5px;color:#fff;font-size:38px;line-height:.92;letter-spacing:-.05em}
    ...
    .shortReceiptExportHost .sdc208-meta{display:grid;grid-template-columns:minmax(0,1fr) 140px;gap:12px;margin:16px 0}
    ...
    .shortReceiptExportHost .sdc208-line{display:grid;grid-template-columns:40px minmax(0,1fr) auto;gap:12px;align-items:center;padding:14px;border-radius:22px;background:#fff;border:1px solid #dbe8f6;box-shadow:0 8px 20px rgba(7,26,53,.035)}
    .shortReceiptExportHost .sdc208-line-index{width:40px;height:40px;display:grid;place-items:center;border-radius:14px;background:#eef6ff;color:#0b63ce;font-size:15px;font-weight:950}
    ...
    .shortReceiptExportHost .sdc208-line strong{display:block;color:#d61c3b;font-size:23px;line-height:1;font-weight:950;white-space:nowrap;padding-left:8px}
    ...
    .shortReceiptExportHost .sdc208-grand b{color:#fff;font-size:43px;line-height:.92;letter-spacing:-.05em;white-space:nowrap}
  ...
  function shortReceiptText(doc){
    ...
    return [
      ...
      ...(doc.items||[]).map(it=>{
        ...
        const total=itemTotal(it);
        const unit=qty?total/qty:total;
        return `${it.name}${selectedColorLabel(it)?` (${selectedColorLabel(it)})`:''} · ${num(qty)} ${qty===1?'unidad':'unidades'} · ${money(unit)} c/u · Total ${money(total)}`;
      }),
