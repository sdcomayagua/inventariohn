    ...
    return `<section class="hero v51-hero hero-v56-clean home-hero-v87 home-hero-v94" id="inicio">
      ...
      <div class="v51-hero-shell home-hero-shell-v87 home-hero-shell-v94">
        <div class="home-brand-card-v94">
          <div class="home-kicker-v94"><span></span> Panel conectado a Firebase</div>
          <h2>Ventas, cotizaciones e inventario sin enredos.</h2>
          <p>Catálogo conectado a Firebase, acciones grandes para móvil y sincronización en la nube.</p>
  ...
  function openProductEditor(id){
    ...
    $('#saveProduct').onclick=async()=>{
      ...
      try{
        ...
        setProductSaveStatus('Enviando producto a Firebase...', 'info');
        await saveProductToFirebase(np,id||np.id);
        state.settings.lastFirebaseSync=new Date().toISOString();
        state.settings.lastFirebaseSyncError='';
        save();
        setProductSaveStatus('Producto guardado y confirmado en Firebase.', 'ok');
        closeModal(); render(); toast('✅ Producto guardado y sincronizado con Firebase.');
      }catch(err){
        console.warn('Firebase save failed',err);
        state.settings.lastFirebaseSyncError=String(err && err.message ? err.message : err || 'Error de Firebase');
        save();
        setProductSaveStatus('No se confirmó Firebase: '+(err.message||err)+'. El producto sí quedó guardado en este dispositivo.', 'error');
        closeModal(); render();
        toast('⚠️ Producto guardado localmente. Firebase no respondió; puedes seguir editando y luego sincronizar.');
      }finally{
        productSaving=false;
