(function(){
  ...
  function miniToast(title,msg){
    ...
    let el=$('.sdc-toast');
    if(!el){el=document.createElement('div');el.className='sdc-toast';document.body.appendChild(el)}
  ...
  function enhance(){
    document.body.classList.add('sdc-ready');
    const top=$('.topbar');
    if(top && !$('.quality-strip')){
      const strip=document.createElement('section');
      strip.className='quality-strip no-print';
      strip.innerHTML='<div class="online"><span>Estado del sistema</span><b>'+(navigator.onLine?'Conectado':'Sin internet')+'</b></div><div><span>Modo</span><b>Calidad Elite</b></div><div><span>Última revisión</span><b>'+formatTime()+'</b></div>';
  ...
  function updateNetwork(){
    const el=$('.quality-strip .online b');
  ...
  document.addEventListener('click',e=>{
    ...
    if(/guardar|sincronizar|respaldo|backup|nube|finalizar/.test(text)){
      btn.classList.add('sdc-pressed'); setTimeout(()=>btn.classList.remove('sdc-pressed'),260);
