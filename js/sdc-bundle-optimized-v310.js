...
;/* ==== js/sdc-v309-low-lag-engine.js ==== */
(function(){
  'use strict';
  if(window.__sdcV309LowLagEngine) return;
  window.__sdcV309LowLagEngine = true;
  ...
  function modalOpen(){ var r=root(); return !!(isMobile() && r && r.querySelector('.modal')); }
  function quoteOpen(){ var r=root(); return !!(isMobile() && r && r.querySelector('.quote-body-v176')); }
  function detailOpen(){ var r=root(); return !!(isMobile() && r && r.querySelector('.v49-product-detail,.v141-product-detail,.v163-product-detail,.product-detail-modal-v221')); }

  var scrolling=false, scrollTimer=0;
  function markScroll(){ scrolling=true; clearTimeout(scrollTimer); scrollTimer=setTimeout(function(){ scrolling=false; }, 260); }
  var nativeAdd = EventTarget.prototype.addEventListener;
  ['scroll','touchmove','wheel'].forEach(function(type){
    try{ nativeAdd.call(window,type,markScroll,{passive:true,capture:true}); }catch(e){}
  });

  /* Los parches viejos tenían muchos listeners de scroll/move. En modal móvil los dejamos descansar. */
  var noisyEvents={scroll:1,wheel:1,pointermove:1,mousemove:1,touchmove:1};
  EventTarget.prototype.addEventListener = function(type, listener, options){
    if(listener && noisyEvents[String(type)] && !listener.__sdcV309Wrapped){
      var original=listener;
      var wrapped=function(ev){
        if(modalOpen() && (scrolling || type==='scroll' || type==='wheel' || type==='pointermove' || type==='mousemove')) return;
        if(typeof original==='function') return original.call(this,ev);
        if(original && typeof original.handleEvent==='function') return original.handleEvent(ev);
      };
      try{ Object.defineProperty(wrapped,'__sdcV309Wrapped',{value:true}); }catch(e){ wrapped.__sdcV309Wrapped=true; }
  ...
  if(NativeMO){
    window.MutationObserver = function(callback){
      var raf=0, timer=0, pending=null, obsRef=null, last=0;
      function run(){
        raf=0; timer=0; last=Date.now();
        try{ callback(pending || [], obsRef); }catch(e){ setTimeout(function(){ throw e; },0); }
        pending=null;
      }
      var wrapped=function(mutations, observer){
        pending=mutations; obsRef=observer;
        if(modalOpen()){
          if(scrolling) { clearTimeout(timer); timer=setTimeout(run,280); return; }
          var now=Date.now();
          if(now-last<180){ if(!timer) timer=setTimeout(run,190); return; }
        }
        if(raf || timer) return;
  ...
  function lightImages(scope){
    (scope||document).querySelectorAll('#modalRoot img').forEach(function(img){
  ...
  function mark(){
    var q=quoteOpen(), d=detailOpen(), m=modalOpen();
    document.body.classList.toggle('sdc-v309-modal-fast', m);
    document.body.classList.toggle('sdc-v309-quote-fast', q);
    document.body.classList.toggle('sdc-v309-detail-fast', d);
    if(m) lightImages(root());
    var time=document.getElementById('hnLiveTime');
    if(time && time._timer){ try{ clearInterval(time._timer); }catch(e){} time._timer=null; }
  }
  window.__sdcV309MarkFastMode = mark;
  document.addEventListener('click',function(){ setTimeout(mark,60); setTimeout(mark,260); },true);
  window.addEventListener('resize',function(){ setTimeout(mark,120); },{passive:true});
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',mark,{once:true}); else mark();
  try{ new (NativeMO||window.MutationObserver)(function(){ if(!scrolling) setTimeout(mark,60); }).observe(document.getElementById('modalRoot')||document.body,{childList:true,subtree:false}); }catch(e){}
})();
...
(function(){
  ...
  function runAction(action){
    ...
    if(action==='tabInicio') window.scrollTo({top:0,behavior:'auto'});
  }
...
(function(){
  ...
  function apply(){
    ...
    if(isQuote){
      modal.querySelectorAll('.cart-list,.gift-list-v176,#docPreview').forEach(el=>{
        setImp(el,'max-height','none');
        setImp(el,'height','auto');
        setImp(el,'overflow','visible');
        setImp(el,'touch-action','pan-y pinch-zoom');
      });
      modal.querySelectorAll('.picker-list,.picker-list-v200,.gift-picker-list').forEach(el=>{
        setImp(el,'max-height','260px');
        setImp(el,'height','auto');
        setImp(el,'overflow-y','auto');
        setImp(el,'overflow-x','hidden');
        setImp(el,'-webkit-overflow-scrolling','touch');
        setImp(el,'overscroll-behavior','contain');
        setImp(el,'touch-action','pan-y pinch-zoom');
      });
      modal.querySelectorAll('.quote-category-list-v201').forEach(el=>{
        if(el.classList.contains('is-open')){
          setImp(el,'display','grid');
          setImp(el,'max-height','280px');
          setImp(el,'overflow-y','auto');
          setImp(el,'overflow-x','hidden');
          setImp(el,'-webkit-overflow-scrolling','touch');
        }else{
  ...
  function scrollBodyTop(){
    ...
    if(body){
      try{body.scrollTo({top:0,behavior:'auto'});}catch(e){body.scrollTop=0;}
    }
  }
  ...
  async function exportBoth(modal){
    ...
    try{
      ...
      const canvas=await window.html2canvas(target, {
        ...
        scale:(/Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent||'')?1.05:2),
        useCORS:true,
        allowTaint:true,
        logging:false,
        width:safeWidth,
...
;window.SDC_V307_LIGHT=true;console.info('SD Comayagua v307 ligero cargado');
...
(function(){
  ...
  document.addEventListener('touchend',function(ev){
    const now=Date.now();
    /* v307: no bloquear scroll/taps en móvil */
    last=now;
...
;/* ==== js/sdc-v307-light-mobile-fixes.js ==== */
(function(){
  'use strict';
  if(window.__sdcV307LightFixes) return;
  window.__sdcV307LightFixes = true;

  // Firebase queda diferido para acelerar el primer uso, pero si el usuario toca sincronizar/guardar se carga al instante.
  document.addEventListener('click', function(ev){
    const btn = ev.target && ev.target.closest && ev.target.closest('[data-action="sync"],#doctorSyncNow,#doctorUploadNow,#saveProduct,#duplicateProduct,#deleteProduct');
    if(btn && typeof window.sdcLoadFirebaseNow === 'function') window.sdcLoadFirebaseNow();
  }, true);

  // Quita scroll suave de la modal de cotización: en celular se siente como lag.
  document.addEventListener('click', function(ev){
    const jump = ev.target && ev.target.closest && ev.target.closest('#modalRoot [data-jump]');
    if(!jump) return;
    const root = document.getElementById('modalRoot');
    const target = root && root.querySelector('#' + CSS.escape(jump.dataset.jump || ''));
    if(target){
      try{ target.scrollIntoView({behavior:'auto', block:'start'}); }catch(e){ target.scrollIntoView(); }
    }
  }, true);

  // Etiqueta para aplicar CSS liviano mientras la cotización está abierta.
  function mark(){
    const open = !!document.querySelector('#modalRoot .quote-body-v176');
    document.body.classList.toggle('sdc-v307-quote-fast', open);
  }
  new MutationObserver(mark).observe(document.getElementById('modalRoot') || document.body, {childList:true, subtree:true});
  document.addEventListener('click', function(){ setTimeout(mark, 60); }, true);
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', mark, {once:true}); else mark();
})();


;/* ==== js/sdc-v308-real-mobile-fast.js ==== */
(function(){
  'use strict';
  if(window.__sdcV308RealMobileFast) return;
  window.__sdcV308RealMobileFast = true;

  function isMobile(){ return (window.matchMedia && window.matchMedia('(max-width: 760px)').matches) || /Android|iPhone|iPad|Mobi/i.test(navigator.userAgent); }
  function mark(){
    var root=document.getElementById('modalRoot');
    var open=!!(root && root.querySelector('.quote-body-v176'));
    document.body.classList.toggle('sdc-v308-quote-ultra-fast', open && isMobile());
    if(open){
      root.querySelectorAll('img').forEach(function(img){
        try{ img.loading='lazy'; img.decoding='async'; img.removeAttribute('crossorigin'); }catch(e){}
      });
      var modal=root.querySelector('.modal.quote-modal-v176,.modal:has(.quote-body-v176)');
      if(modal){
  ...
  function onClick(ev){
    var btn=ev.target && ev.target.closest && ev.target.closest('[data-sdc210-step],[data-sdc210-next],[data-sdc210-prev],[data-jump],#downloadDoc,#saveQuote,#toSale,#waText');
    if(btn) setTimeout(mark,40);
  }
  document.addEventListener('click', onClick, true);
  window.addEventListener('resize', function(){ setTimeout(mark,80); }, {passive:true});
  window.addEventListener('orientationchange', function(){ setTimeout(mark,220); }, {passive:true});
  var root=document.getElementById('modalRoot') || document.body;
  try{ new MutationObserver(mark).observe(root,{childList:true, subtree:false}); }catch(e){}
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',mark,{once:true}); else mark();
})();


;/* ==== js/sdc-v309-final-mobile-overrides.js ==== */
(function(){
  'use strict';
  if(window.__sdcV309FinalOverrides) return;
  window.__sdcV309FinalOverrides=true;
  document.addEventListener('click',function(ev){
    var btn=ev.target && ev.target.closest && ev.target.closest('#v49DownloadProductPhoto,#v49ShareProductPhotoTab,[data-tab="captura"]');
    if(!btn) return;
    var card=document.querySelector('#modalRoot #productShareCard');
    if(card && card.dataset.needsRender==='1'){
      card.innerHTML='<div class="product-share-placeholder-v309"><b>Preparando imagen...</b><span>Un momento.</span></div>';
      card.dataset.needsRender='0';
    }
    if(typeof window.__sdcV309MarkFastMode==='function') setTimeout(window.__sdcV309MarkFastMode,80);
  },true);
})();
