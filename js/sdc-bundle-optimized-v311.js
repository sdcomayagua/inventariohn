...
(function(){
  ...
  async function downloadShortBoth(){
    ...
    try{
      ...
      host.className='sdc-v311-both-host';
      host.innerHTML='<div class="sdc-v311-both-title">SD COMAYAGUA · Comparativo</div><div class="sdc-v311-both-grid"></div>';
      var grid=host.querySelector('.sdc-v311-both-grid');
      qsa('.short-receipt-preview-wrap',root).slice(0,2).forEach(function(x){ grid.appendChild(x.cloneNode(true)); });
      document.body.appendChild(host);
      await new Promise(function(r){setTimeout(r,80);});
      var canvas=await h2c(host,{backgroundColor:'#eaf3fb',scale:isMobile()?1.35:1.8,useCORS:true,allowTaint:false,scrollX:0,scrollY:0});
