...
(function(){
  ...
  function setView(v){currentView=v; render(); window.scrollTo({top:0,behavior:'auto'});}
  ...
  function setPageV150(page){
    ...
    requestAnimationFrame(()=>{
      document.querySelector('.sdc-tabs-v150')?.scrollIntoView({behavior:'auto',block:'start'});
    });
  }
  ...
  function mainAction(e){
    ...
    if(a==='lowStock'){filter.cat='Todos'; filter.q=''; filter.special='lowStock'; render(); setTimeout(()=>{document.getElementById('inventario')?.scrollIntoView({behavior:'auto',block:'start'}); const n=filteredProducts().length; toast(n?`${n} productos con bajo stock filtrados.`:'No hay productos en bajo stock.');},50); return;}
    if(a==='outStock'){filter.cat='Todos'; filter.q=''; filter.special='outStock'; render(); setTimeout(()=>{document.getElementById('inventario')?.scrollIntoView({behavior:'auto',block:'start'}); const n=filteredProducts().length; toast(n?`${n} productos agotados filtrados.`:'No hay productos agotados.');},50); return;}
    if(a==='noCost'){filter.cat='Todos'; filter.q=''; filter.special='noCost'; render(); setTimeout(()=>{document.getElementById('inventario')?.scrollIntoView({behavior:'auto',block:'start'}); toast(`${filteredProducts().length} productos sin costo filtrados.`);},50); openNoCost(); return;}
    if(a==='lowProfit'){filter.cat='Todos'; filter.q=''; filter.special='lowProfit'; render(); setTimeout(()=>{document.getElementById('inventario')?.scrollIntoView({behavior:'auto',block:'start'}); toast(`${filteredProducts().length} productos con ganancia baja filtrados.`);},50); openLowProfit(); return;}
    if(a==='noImage'){filter.cat='Todos'; filter.q=''; filter.special='noImage'; render(); setTimeout(()=>{document.getElementById('inventario')?.scrollIntoView({behavior:'auto',block:'start'}); const n=filteredProducts().length; toast(n?`${n} productos sin imagen filtrados.`:'Todos los productos tienen imagen.');},50); return;}
  }
      ...
      <div class="modal-body v49-product-detail v141-product-detail v163-product-detail ${st.cls==='out'?'is-out':''}">
        <div class="v141-detail-shell v163-detail-shell">
          <section class="v49-detail-hero v141-detail-hero v163-detail-hero ${st.cls==='out'?'is-out':''}">
            <div class="v49-detail-image v141-detail-image v163-detail-image"><img src="${escapeHtml(productImage(p))}" alt="${escapeHtml(p.name)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='${escapeHtml(placeholderFor(p))}'"><span class="stock-badge ${st.cls}"><i></i>${st.text}</span></div>
          ...
          <section class="v49-tab" data-panel="captura">
            ...
            <div id="productShareCard" class="product-share-placeholder-v309" data-needs-render="1"><b>Imagen para cliente pausada</b><span>Para que el producto abra rápido en celular, esta imagen se prepara solo al tocar la pestaña IMAGEN, Generar PNG o Compartir foto.</span></div>
  ...
  function bindProductDetails(p){
    ...
    const redrawTotals=(forcedQty=null)=>{
      ...
      syncRouteUI();
      const card=$('#productShareCard',modalRoot); if(card && card.dataset.rendered==='1') card.innerHTML=productClientPhotoHTML(p,q);
    };
    ...
    $$('.v49-tabbar [data-tab]',modalRoot).forEach(btn=>btn.onclick=()=>{
      ...
      if(btn.dataset.tab==='captura'){
        const card=$('#productShareCard',modalRoot);
        if(card && card.dataset.needsRender==='1'){
          card.dataset.needsRender='0';
          card.dataset.rendered='1';
          card.innerHTML=productClientPhotoHTML(p,qtyValue());
          card.querySelectorAll('img').forEach(img=>{try{img.loading='lazy';img.decoding='async';img.removeAttribute('crossorigin');}catch(e){}});
        }
      }
    });
    redrawTotals();
    ...
    if(timeEl){
      timeEl.textContent=nowHNPanel();
      if(timeEl._timer){ try{clearInterval(timeEl._timer);}catch(e){} timeEl._timer=null; }
    }
...
📲 WhatsApp: +504 3151-7755`;
  ...
  async function captureNodeAsPngBlob(node, scale=3){
    ...
    const safeScale=isMobileDevice()?Math.min(Number(scale)||2,1.15):(Number(scale)||2);
  ...
  function renderPicker(isSale){
    ...
    function draw(){
      ...
      const pickerLimit=isMobileDevice()?5:48;
    ...
    if(pickSearchEl){
      pickSearchEl.oninput=e=>{
        ...
        pickSearchEl._sdcPickTimer=setTimeout(draw,isMobileDevice()?180:40);
      };
    }
  ...
  function bindGiftPicker(isSale){
    ...
    function draw(){
      ...
      const items=activeProducts().filter(p=>!term || [p.name,p.id,categoryText(p)].join(' ').toLowerCase().includes(term)).slice(0,isMobileDevice()?8:30);
  ...
  function scheduleDocPreview(doc,isSale,delay=260){
    ...
    if(isMobileDevice() && target.dataset.previewRequested!=='1'){
      ...
      if(!target.querySelector('[data-show-doc-preview]')){
        target.innerHTML='<div class="doc-preview-loading doc-preview-light-v307"><b>Vista previa pausada</b><span>Para que Cotizar sea más rápido en celular, la factura visual se carga solo cuando la necesites.</span><button type="button" class="btn secondary" data-show-doc-preview="1">Ver vista previa</button></div>';
      }
      return;
    }
  ...
  function bindQuoteCommon(isSale){
    ...
    $$('[data-jump]',modalRoot).forEach(b=>b.onclick=()=>{if(b.dataset.jump==='docPreview') renderDocPreviewNow(isSale); $('#'+b.dataset.jump,modalRoot)?.scrollIntoView({behavior:'auto',block:'start'});});
    const preview=$('#docPreview',modalRoot);
    if(preview) preview.onclick=(ev)=>{if(ev.target.closest('[data-show-doc-preview]')){ev.preventDefault(); renderDocPreviewNow(isSale);}};
    bindAction('backToQuote',()=>{$('#currentDocTitle',modalRoot)?.scrollIntoView({behavior:'auto',block:'start'}); toast('La cotización sigue abierta.');});
    bindAction('downloadDoc',()=>{ renderDocPreviewNow(isSale); setTimeout(()=>downloadDocImage(isSale?'recibo':'cotizacion'), isMobileDevice()?80:20); });
...
📲 WhatsApp: +504 3151-7755`;
  ...
  async function captureNodeToBlob(el,backgroundColor){
    ...
    try{
      const exportScale=isMobileDevice()?1.05:2.1;
...
📲 WhatsApp +504 3151-7755`;
  ...
  function shortReceiptCardHTML(doc){
    ...
    const isCodVariant=isCodDoc(doc);
    const normalRef=shortReceiptVariant(doc,'Normal');
    const normalCalc=calc(normalRef);
    const savings=Math.max(0,Math.round((c.total-normalCalc.total)*100)/100);
    const noteHTML=isCodVariant && savings?`<section class="sdc208-variant-note sdc208-variant-note-cod"><span>Servicio pagar al recibir</span><b>Incluye ${money(savings)} adicionales</b><small>Si usas envío normal, te ahorras ${money(savings)}.</small></section>`:'';
    return `<div class="short-receipt sdc208-ticket ${isCodVariant?'sdc208-cod':'sdc208-normal'}" data-variant="${isCodVariant?'cod':'normal'}" id="shortReceiptCard">
  ...
  function shortReceiptExportCSS(){return `
    ...
    .shortReceiptExportHost .sdc208-variant-note{display:grid;gap:5px;margin:12px 0 14px;padding:14px 16px;border-radius:20px;border:1px solid #dbe8f6;background:#f8fbff}
    .shortReceiptExportHost .sdc208-variant-note span{display:block;color:#8f5d00;font-size:11px;line-height:1;font-weight:950;letter-spacing:.11em;text-transform:uppercase;margin:0}
    .shortReceiptExportHost .sdc208-variant-note b{display:block;color:#8a4f00;font-size:24px;line-height:1.05;font-weight:950}
    .shortReceiptExportHost .sdc208-variant-note small{display:block;color:#9a6a12;font-size:13px;line-height:1.25;font-weight:850}
    .shortReceiptExportHost .sdc208-cod .sdc208-head{background:linear-gradient(135deg,#8a4e00 0%,#d98a00 55%,#f0b429 100%)}
    .shortReceiptExportHost .sdc208-cod .sdc208-head-copy span{color:#fff0b8}
    .shortReceiptExportHost .sdc208-cod .sdc208-head-copy p{color:#fff8da}
    .shortReceiptExportHost .sdc208-cod .sdc208-mode{background:#fff7df;color:#9a5d00}
    .shortReceiptExportHost .sdc208-cod .sdc208-summary{background:#fff8ea;border-color:#f3deb0}
    .shortReceiptExportHost .sdc208-cod .sdc208-variant-note{background:#fff3d8;border-color:#f2d58d}
    .shortReceiptExportHost .sdc208-cod .sdc208-grand{background:linear-gradient(135deg,#8a4e00 0%,#d98a00 60%,#f0b429 100%)}
  `}
  ...
  async function downloadShortReceiptImage(doc,label='actual'){
    ...
    try{
      ...
      const blob=await captureNodeAsPngBlob(node,isMobileDevice()?1.15:3);
  ...
  function openAlertsV196(){
    ...
    $$('[data-alert-filter-v196]',modalRoot).forEach(btn=>btn.onclick=()=>{
      ...
      render();
      setTimeout(()=>{document.getElementById('inventario')?.scrollIntoView({behavior:'auto',block:'start'}); toast(`Filtro aplicado: ${btn.querySelector('span')?.textContent||'Alertas'}.`);},80);
    });
  ...
  $('#goTop').onclick=()=>window.scrollTo({top:0,behavior:'auto'});
...
;/* ==== js/sdc-v311-no-flicker.js ==== */
(function(){
  'use strict';
  if(window.__sdcV311NoFlicker) return;
  window.__sdcV311NoFlicker = true;

  var markTimer = 0;
  var raf = 0;
  function qs(s,r){ return (r||document).querySelector(s); }
  function qsa(s,r){ return Array.prototype.slice.call((r||document).querySelectorAll(s)); }
  function isMobile(){ return (window.matchMedia && window.matchMedia('(max-width: 780px)').matches) || /Android|iPhone|iPad|iPod|Mobi/i.test(navigator.userAgent||''); }
  function api(){ return window.SDCApp || window.SDCAppV196 || window.SDCAppV287 || {}; }
  function toast(msg){
    var el=document.getElementById('toast');
    if(!el) return;
    el.textContent=msg;
    el.classList.add('show');
    clearTimeout(el._sdcV311);
    el._sdcV311=setTimeout(function(){ el.classList.remove('show'); },2200);
  }
  function toggle(cls,on){
    if(!document.body) return;
    if(document.body.classList.contains(cls)!==!!on) document.body.classList.toggle(cls,!!on);
  }
  function lightModalImages(){
    var root=document.getElementById('modalRoot');
    if(!root) return;
    qsa('img',root).forEach(function(img){
      try{ img.loading='lazy'; img.decoding='async'; img.removeAttribute('crossorigin'); img.setAttribute('referrerpolicy','no-referrer'); }catch(e){}
    });
  }
  function mark(){
    raf=0;
    var root=document.getElementById('modalRoot');
    var hasModal=!!(root && root.querySelector('.modal'));
    var hasQuote=!!(root && root.querySelector('.quote-body-v176'));
    var hasDetail=!!(root && root.querySelector('.product-detail-modal-v221,.v49-product-detail,.v141-product-detail,.v163-product-detail'));
    toggle('sdc-v311-no-flicker',true);
    toggle('sdc-v311-modal-open',hasModal);
    toggle('sdc-v306-quote-open',hasQuote);
    toggle('sdc-v307-quote-fast',hasQuote);
    toggle('sdc-v308-quote-ultra-fast',hasQuote && isMobile());
    toggle('sdc-v309-modal-fast',hasModal && isMobile());
    toggle('sdc-v309-quote-fast',hasQuote && isMobile());
    toggle('sdc-v309-detail-fast',hasDetail && isMobile());
    if(hasModal) lightModalImages();
    var t=document.getElementById('hnLiveTime');
    if(t && t._timer){ try{ clearInterval(t._timer); }catch(e){} t._timer=null; }
  }
  function scheduleMark(delay){
    clearTimeout(markTimer);
    markTimer=setTimeout(function(){ if(!raf) raf=requestAnimationFrame(mark); }, delay==null?50:delay);
  }

  function ensureMenu(){
    if(document.querySelector('.sdc-menu-drawer-v116')) return;
    var html = ''+
      ...
      '<aside class="sdc-menu-drawer-v116 sdc-menu-drawer-v179 no-print" aria-label="Menú principal">'+
        '<header class="sdc-menu-head-v116 sdc-menu-head-v179">'+
          '<div class="sdc-menu-brand-v116 sdc-menu-brand-v179"><img src="assets/logo-sdc.png" alt="SD Comayagua"><div><b>SD COMAYAGUA</b><span>Panel rápido</span></div></div>'+
          '<button class="sdc-menu-close-v116 sdc-menu-close-v179" type="button" data-sdc127="close" aria-label="Cerrar menú">×</button>'+
        '</header>'+
        '<button class="sdc-menu-feature-v179" type="button" data-sdc127="vender"><i>⚡</i><span><b>Nueva venta</b><small>Factura rápida</small></span><em>›</em></button>'+
        '<nav class="sdc-menu-list-v116 sdc-menu-list-v179 sdc-menu-grid-v116 sdc-menu-grid-v179">'+
          menuItem('inicio','⌂','Inicio','Resumen principal')+
          menuItem('panel','▦','Panel','Costos y ganancias')+
          menuItem('productos','▣','Productos','Catálogo e inventario')+
          menuItem('cotizar','🧾','Cotizar','Enviar a cliente')+
          menuItem('vender','⚡','Vender','Registrar factura')+
          menuItem('recibos','▤','Recibos','Caja e historial')+
          menuItem('cotizaciones','☷','Cotizaciones','Guardadas')+
          menuItem('ganancias','↗','Ganancias','Utilidad')+
          menuItem('alertas','🔔','Alertas','Revisión')+
          menuItem('nuevo','＋','Nuevo producto','Agregar inventario')+
        '</nav>'+
      '</aside>';
    document.body.insertAdjacentHTML('beforeend', html);
  }
  function menuItem(action,icon,title,sub){
    return '<button class="sdc-menu-item-v116 sdc-menu-item-v179" type="button" data-sdc127="'+action+'"><i>'+icon+'</i><span><b>'+title+'</b><small>'+sub+'</small></span></button>';
  }
  function openMenu(){ ensureMenu(); document.body.classList.add('sdc-menu-open-v116'); }
  function closeMenu(){ document.body.classList.remove('sdc-menu-open-v116'); }
  function runAction(action){
    var a=api();
    closeMenu();
    switch(action){
      case 'open': openMenu(); return true;
      case 'close': closeMenu(); return true;
      case 'inicio': if(a.setPage){ a.setPage('inicio'); return true; } if(window.SDCSetPageV150){ window.SDCSetPageV150('inicio'); return true; } return clickData('tabInicio');
      case 'panel': if(a.setPage){ a.setPage('panel'); return true; } if(window.SDCSetPageV150){ window.SDCSetPageV150('panel'); return true; } return clickData('tabPanel');
      case 'productos': if(a.setPage){ a.setPage('productos'); return true; } if(window.SDCSetPageV150){ window.SDCSetPageV150('productos'); return true; } return clickData('tabProductos');
      case 'cotizar': if(a.openQuote){ a.openQuote(); return true; } return clickData('quote');
      case 'vender': if(a.openSale){ a.openSale(); return true; } return clickData('sell');
      case 'recibos': if(a.openReceipts){ a.openReceipts(); return true; } return clickData('receipts');
      case 'cotizaciones': if(a.openSavedQuotes){ a.openSavedQuotes(); return true; } return clickData('quotes');
      case 'ganancias': if(a.openProfit){ a.openProfit(); return true; } return clickData('profit');
      case 'alertas': if(a.openNotifications){ a.openNotifications(); return true; } return clickData('notifications');
      case 'nuevo': if(a.openProductEditor){ a.openProductEditor(); return true; } return clickData('newProduct');
    }
    return false;
  }
  function clickData(action){
    var b=document.querySelector('[data-action="'+action+'"]');
    if(b){ b.click(); return true; }
    return false;
  }

  function addShortBothButton(){
    var modal=document.querySelector('#modalRoot .short-receipt-screen');
    if(!modal || document.getElementById('downloadShortBothV311')) return;
    var actions=modal.querySelector('.short-receipt-actions') || modal;
    var btn=document.createElement('button');
    btn.type='button';
    btn.id='downloadShortBothV311';
    btn.className='btn sdc230-short-both';
    btn.textContent='Descargar ambas';
    actions.appendChild(btn);
    btn.addEventListener('click',downloadShortBoth,false);
  }
  function ensureHtml2Canvas(){
    if(window.html2canvas) return Promise.resolve(window.html2canvas);
    return new Promise(function(resolve,reject){
      var old=[].slice.call(document.scripts).find(function(s){return String(s.src||'').indexOf('html2canvas')>-1;});
      var start=Date.now();
      if(!old){
        old=document.createElement('script');
        old.src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';
        old.async=true;
        document.head.appendChild(old);
      }
      (function check(){
        if(window.html2canvas) return resolve(window.html2canvas);
        if(Date.now()-start>12000) return reject(new Error('html2canvas no cargó'));
        setTimeout(check,80);
      })();
    });
  }
  function shortBothExportCSS(){return shortReceiptExportCSS()+`
    .sdc-v312-both-host.shortReceiptExportHost{width:1240px!important;padding:30px!important;background:#eaf3fb!important;z-index:-1!important;font-family:Arial,sans-serif!important}
    .sdc-v312-both-host .sdc-v312-both-title{margin:0 0 22px;text-align:center;color:#07192f;font-size:34px;line-height:1.05;font-weight:950;letter-spacing:.08em;text-transform:uppercase}
    .sdc-v312-both-host .sdc-v312-both-grid{display:grid!important;grid-template-columns:540px 540px!important;gap:32px!important;align-items:start!important;justify-content:center!important}
    .sdc-v312-both-host .short-receipt-preview-wrap{padding:0!important;margin:0!important;border:0!important;background:transparent!important;box-shadow:none!important;overflow:visible!important;border-radius:0!important}
    .sdc-v312-both-host .sdc208-ticket{margin:0!important;min-height:760px!important}
    .sdc-v312-both-host .sdc208-line{grid-template-columns:38px minmax(0,1fr) auto!important}
    .sdc-v312-both-host .sdc208-line strong{grid-column:3!important;grid-row:1 / span 2!important;align-self:center!important;justify-self:end!important;margin:0!important}
    .sdc-v312-both-host .sdc208-summary div{display:grid!important;grid-template-columns:minmax(0,1fr) auto!important}
    .sdc-v312-both-host .sdc208-summary span{margin:0!important;align-self:center!important}
    .sdc-v312-both-host .sdc208-summary b{align-self:center!important}
    .sdc-v312-both-host .sdc208-foot span{font-size:14px!important}
    @media(max-width:900px){.sdc-v312-both-host.shortReceiptExportHost{width:1120px!important;padding:24px!important}.sdc-v312-both-host .sdc-v312-both-grid{grid-template-columns:520px 520px!important;gap:24px!important}.sdc-v312-both-host .sdc208-ticket{width:520px!important;max-width:520px!important}.sdc-v312-both-host .sdc-v312-both-title{font-size:28px!important}}
  `}
  async function downloadShortBoth(){
    var root=document.querySelector('#modalRoot .short-receipt-grid-v147');
    if(!root) return toast('No encontré los recibos.');
    try{
      toast('Preparando imagen...');
      var h2c=await ensureHtml2Canvas();
      var source=qsa('.short-receipt-preview-wrap',root).slice(0,2);
      if(source.length<2) return toast('No encontré los dos recibos.');
      var host=document.createElement('div');
      host.className='shortReceiptExportHost sdc-v312-both-host';
      host.innerHTML='<style>'+shortBothExportCSS()+'</style><div class="sdc-v312-both-title">SD COMAYAGUA · Comparativo</div><div class="sdc-v312-both-grid"></div>';
      var grid=host.querySelector('.sdc-v312-both-grid');
      source.forEach(function(x){
        var wrap=document.createElement('div');
        wrap.className='short-receipt-preview-wrap sdc-v312-both-card-wrap';
        var card=x.querySelector('.short-receipt');
        wrap.appendChild((card||x).cloneNode(true));
        grid.appendChild(wrap);
      });
      document.body.appendChild(host);
      await waitForImages(host);
      await new Promise(function(r){setTimeout(r,120);});
      var canvas=await h2c(host,{backgroundColor:'#eaf3fb',scale:isMobile()?1.25:1.65,useCORS:true,allowTaint:false,scrollX:0,scrollY:0,windowWidth:1300});
      var a=document.createElement('a');
      a.download='SDC-recibo-comparativo.png';
      a.href=canvas.toDataURL('image/png');
      a.click();
      host.remove();
      toast('Imagen comparativa descargada.');
    }catch(err){ console.error(err); toast('No se pudo descargar ambas.'); }
  }

  document.addEventListener('click',function(ev){
    var node=ev.target && ev.target.closest && ev.target.closest('[data-sdc127]');
    if(node){
      var action=(node.getAttribute('data-sdc127')||'').trim();
      if(runAction(action)){ ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation(); }
    }
    var fb=ev.target && ev.target.closest && ev.target.closest('[data-action="sync"],#doctorSyncNow,#doctorUploadNow,#saveProduct,#duplicateProduct,#deleteProduct,#finishSale,#saveQuote,#toSale');
    if(fb && typeof window.sdcLoadFirebaseNow==='function') window.sdcLoadFirebaseNow();
    scheduleMark(60);
    setTimeout(addShortBothButton,140);
  },true);

  document.addEventListener('input',function(){ scheduleMark(100); },true);
  window.addEventListener('resize',function(){ scheduleMark(120); },{passive:true});
  window.addEventListener('orientationchange',function(){ scheduleMark(240); },{passive:true});

  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',function(){ ensureMenu(); mark(); },{once:true});
  }else{
    ensureMenu(); mark();
  }
})();
...
;/* ==== SDC v313: menú móvil corregido + descargar ambas robusto ==== */
(function(){
  'use strict';
  if(window.__sdcV313FinalFix) return;
  window.__sdcV313FinalFix = true;

  function qs(sel,root){return (root||document).querySelector(sel)}
  function qsa(sel,root){return Array.prototype.slice.call((root||document).querySelectorAll(sel))}
  function isMobile(){return /Android|iPhone|iPad|iPod|Mobi/i.test(navigator.userAgent||'') || (window.matchMedia && window.matchMedia('(max-width:780px)').matches)}
  function toast(msg){
    var el=document.getElementById('toast');
    if(!el) return alert(msg);
    el.textContent=msg;
    el.classList.add('show');
    clearTimeout(el._sdcV313Toast);
    el._sdcV313Toast=setTimeout(function(){el.classList.remove('show')},2600);
  }
  function rebuildMenu(){
    var drawer=qs('.sdc-menu-drawer-v116');
    if(!drawer) return;
    drawer.classList.add('sdc-menu-v313');
    var head=qs('.sdc-menu-head-v116',drawer);
    if(head && !qs('.sdc-menu-brand-v116',head)){
      var img=qs(':scope > img',head) || qs('img',head);
      var copy=qs(':scope > div',head) || (img && img.nextElementSibling);
      if(img && copy){
        var brand=document.createElement('div');
        brand.className='sdc-menu-brand-v116 sdc-menu-brand-v179';
        brand.appendChild(img);
        brand.appendChild(copy);
        head.insertBefore(brand, head.firstChild);
      }
    }
    var nav=qs('.sdc-menu-list-v116',drawer) || qs('nav',drawer);
    if(nav){ nav.classList.add('sdc-menu-grid-v116','sdc-menu-grid-v179','sdc-menu-list-v313'); }
    var feature=qs('.sdc-menu-feature-v179',drawer);
    if(feature && !qs('em',feature)){
      var em=document.createElement('em'); em.textContent='›'; feature.appendChild(em);
    }
    qsa('.sdc-menu-item-v116,.sdc-menu-item-v179',drawer).forEach(function(b){b.classList.add('sdc-menu-item-v313')});
  }

  function loadHtml2Canvas(){
    if(window.html2canvas) return Promise.resolve(window.html2canvas);
    return new Promise(function(resolve,reject){
      var existing=qsa('script').find(function(s){return String(s.src||'').indexOf('html2canvas')>-1});
      if(!existing){
        existing=document.createElement('script');
        existing.src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';
        existing.async=true;
      ...
      (function wait(){
        if(window.html2canvas) return resolve(window.html2canvas);
        if(Date.now()-start>15000) return reject(new Error('html2canvas no cargó'));
        setTimeout(wait,80);
      })();
    });
  }
  function waitImages(root){
    var imgs=qsa('img',root);
    return Promise.all(imgs.map(function(img){
      try{ img.loading='eager'; img.decoding='sync'; }catch(e){}
      if(img.complete) return Promise.resolve();
      return new Promise(function(resolve){ img.onload=img.onerror=resolve; setTimeout(resolve,4000); });
    }));
  }
  function exportCss(){return `
    .sdc-v313-both-export{position:fixed!important;left:0!important;top:0!important;width:1240px!important;padding:28px!important;background:#eaf3fb!important;color:#07192f!important;box-sizing:border-box!important;pointer-events:none!important;z-index:-9999!important;font-family:Arial,sans-serif!important;}
    .sdc-v313-both-export *{box-sizing:border-box!important;}
    .sdc-v313-both-title{margin:0 0 22px!important;text-align:center!important;color:#07192f!important;font-size:34px!important;line-height:1.05!important;font-weight:950!important;letter-spacing:.06em!important;text-transform:uppercase!important;}
    .sdc-v313-both-grid{display:grid!important;grid-template-columns:1fr 1fr!important;gap:28px!important;align-items:start!important;}
    .sdc-v313-both-card{width:100%!important;margin:0!important;padding:0!important;background:transparent!important;border:0!important;box-shadow:none!important;overflow:visible!important;}
    .sdc-v313-both-export .sdc208-ticket,.sdc-v313-both-export .short-receipt{width:100%!important;max-width:100%!important;margin:0!important;display:block!important;background:#fff!important;border:1px solid #d8e7f6!important;border-radius:30px!important;padding:20px!important;color:#07192f!important;box-shadow:none!important;overflow:hidden!important;font-family:Arial,sans-serif!important;}
    .sdc-v313-both-export .sdc208-head{display:grid!important;grid-template-columns:68px minmax(0,1fr)!important;grid-template-areas:'logo copy' 'mode mode'!important;gap:14px!important;align-items:center!important;padding:18px!important;border-radius:26px!important;background:linear-gradient(135deg,#071a35 0%,#0b63ce 78%)!important;color:#fff!important;}
    .sdc-v313-both-export .sdc208-logo{grid-area:logo!important;width:68px!important;height:68px!important;border-radius:20px!important;background:#fff!important;display:grid!important;place-items:center!important;overflow:hidden!important;}
    .sdc-v313-both-export .sdc208-logo img{width:54px!important;height:54px!important;object-fit:contain!important;}
    .sdc-v313-both-export .sdc208-head-copy{grid-area:copy!important;min-width:0!important;}
    .sdc-v313-both-export .sdc208-head-copy span{display:block!important;color:#cfe8ff!important;font-size:13px!important;font-weight:950!important;letter-spacing:.14em!important;text-transform:uppercase!important;line-height:1!important;}
    .sdc-v313-both-export .sdc208-head-copy h2{margin:7px 0 4px!important;color:#fff!important;font-size:32px!important;line-height:1!important;font-weight:950!important;}
    .sdc-v313-both-export .sdc208-head-copy p{margin:0!important;color:#e8f4ff!important;font-size:14px!important;line-height:1.2!important;font-weight:800!important;}
    .sdc-v313-both-export .sdc208-mode{grid-area:mode!important;justify-self:start!important;display:inline-flex!important;align-items:center!important;padding:10px 15px!important;border-radius:999px!important;background:#fff!important;color:#0b63ce!important;font-size:13px!important;font-weight:950!important;}
    .sdc-v313-both-export .sdc208-meta{display:grid!important;grid-template-columns:minmax(0,1fr) 112px!important;gap:10px!important;margin:14px 0!important;}
    .sdc-v313-both-export .sdc208-meta div,.sdc-v313-both-export .sdc208-summary div{background:#fff!important;border:1px solid #dbe8f6!important;border-radius:18px!important;padding:13px!important;}
    .sdc-v313-both-export .sdc208-meta div{background:#f7fbff!important;}
    .sdc-v313-both-export .sdc208-meta span,.sdc-v313-both-export .sdc208-summary span,.sdc-v313-both-export .sdc208-grand span{display:block!important;color:#617993!important;font-size:10.5px!important;line-height:1!important;font-weight:950!important;letter-spacing:.1em!important;text-transform:uppercase!important;margin:0 0 7px!important;}
    .sdc-v313-both-export .sdc208-meta b{display:block!important;color:#07192f!important;font-size:22px!important;line-height:1.08!important;word-break:break-word!important;}
    .sdc-v313-both-export .sdc208-section{display:flex!important;align-items:center!important;gap:10px!important;margin:10px 0!important;}
    .sdc-v313-both-export .sdc208-section:after{content:''!important;flex:1!important;height:1px!important;background:#dbe8f6!important;}
    .sdc-v313-both-export .sdc208-section span{font-size:13px!important;font-weight:950!important;letter-spacing:.1em!important;text-transform:uppercase!important;}
    .sdc-v313-both-export .sdc208-lines{display:grid!important;gap:9px!important;margin-bottom:12px!important;}
    .sdc-v313-both-export .sdc208-line{display:grid!important;grid-template-columns:34px minmax(0,1fr) auto!important;gap:10px!important;align-items:start!important;padding:12px!important;border-radius:18px!important;background:#fff!important;border:1px solid #dbe8f6!important;}
    .sdc-v313-both-export .sdc208-line-index{width:34px!important;height:34px!important;border-radius:12px!important;background:#eef6ff!important;color:#0b63ce!important;display:grid!important;place-items:center!important;font-size:13px!important;font-weight:950!important;}
    .sdc-v313-both-export .sdc208-line-copy{display:grid!important;gap:3px!important;min-width:0!important;}
    .sdc-v313-both-export .sdc208-line-copy b{display:block!important;font-size:16px!important;line-height:1.12!important;color:#07192f!important;word-break:break-word!important;}
    .sdc-v313-both-export .sdc208-line-copy span,.sdc-v313-both-export .sdc208-line-copy em{display:block!important;color:#617993!important;font-size:12px!important;line-height:1.2!important;font-style:normal!important;font-weight:800!important;}
    .sdc-v313-both-export .sdc208-line strong{grid-column:3!important;grid-row:1 / span 2!important;align-self:center!important;justify-self:end!important;color:#d61c3b!important;font-size:18px!important;white-space:nowrap!important;margin:0!important;}
    .sdc-v313-both-export .sdc208-summary{display:grid!important;gap:8px!important;margin:0 0 12px!important;padding:9px!important;border-radius:20px!important;background:#f3f8ff!important;border:1px solid #dbe8f6!important;}
    .sdc-v313-both-export .sdc208-summary div{display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;align-items:center!important;gap:10px!important;}
    .sdc-v313-both-export .sdc208-summary span{margin:0!important;}
    .sdc-v313-both-export .sdc208-summary b{font-size:18px!important;white-space:nowrap!important;color:#07192f!important;}
    .sdc-v313-both-export .sdc208-grand{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;padding:16px!important;border-radius:22px!important;background:linear-gradient(135deg,#071a35,#0b63ce)!important;color:#fff!important;margin-top:4px!important;}
    .sdc-v313-both-export .sdc208-grand span{margin:0!important;color:#dbeafe!important;}
    .sdc-v313-both-export .sdc208-grand b{color:#fff!important;font-size:31px!important;line-height:1!important;white-space:nowrap!important;}
    .sdc-v313-both-export .sdc208-foot{display:grid!important;gap:8px!important;margin-top:12px!important;}
    .sdc-v313-both-export .sdc208-foot span{display:block!important;padding:12px!important;border-radius:16px!important;background:#fbfdff!important;border:1px dashed #cfe3fb!important;color:#617993!important;text-align:center!important;font-size:12px!important;font-weight:850!important;line-height:1.3!important;}
    .sdc-v313-both-export .sdc208-foot b{display:block!important;padding:12px!important;border-radius:16px!important;background:#071a35!important;color:#fff!important;text-align:center!important;font-size:15px!important;font-weight:950!important;}
    .sdc-v313-both-export .sdc208-variant-note{display:grid!important;gap:4px!important;margin:12px 0 14px!important;padding:12px 14px!important;border-radius:18px!important;border:1px solid #dbe8f6!important;background:#f8fbff!important;}
    .sdc-v313-both-export .sdc208-variant-note span{display:block!important;color:#8f5d00!important;font-size:10.5px!important;line-height:1!important;font-weight:950!important;letter-spacing:.11em!important;text-transform:uppercase!important;margin:0!important;}
    .sdc-v313-both-export .sdc208-variant-note b{display:block!important;color:#8a4f00!important;font-size:22px!important;line-height:1.05!important;font-weight:950!important;}
    .sdc-v313-both-export .sdc208-variant-note small{display:block!important;color:#9a6a12!important;font-size:12px!important;line-height:1.25!important;font-weight:850!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-head{background:linear-gradient(135deg,#8a4e00 0%,#d98a00 55%,#f0b429 100%)!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-head-copy span{color:#fff0b8!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-head-copy p{color:#fff8da!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-mode{background:#fff7df!important;color:#9a5d00!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-summary{background:#fff8ea!important;border-color:#f3deb0!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-variant-note{background:#fff3d8!important;border-color:#f2d58d!important;}
    .sdc-v313-both-export .sdc208-cod .sdc208-grand{background:linear-gradient(135deg,#8a4e00 0%,#d98a00 60%,#f0b429 100%)!important;}
  `}
  function buildCardFromVisible(wrap){
    var card=(wrap && (qs('.short-receipt',wrap) || qs('.sdc208-ticket',wrap))) || wrap;
    if(!card) return null;
    var clone=card.cloneNode(true);
    clone.removeAttribute('id');
    qsa('[id]',clone).forEach(function(n){n.removeAttribute('id')});
    return clone;
  }
  async function downloadBoth(){
    var modal=qs('#modalRoot .short-receipt-screen');
    if(!modal) return toast('Abra primero Recibo corto.');
    var wraps=qsa('.short-receipt-grid-v147 .short-receipt-preview-wrap',modal);
    if(wraps.length<2) wraps=qsa('.short-receipt-preview-wrap',modal);
    var cards=wraps.map(buildCardFromVisible).filter(Boolean).slice(0,2);
    if(cards.length<2) return toast('No encontré los dos recibos para comparar.');
    var host=document.createElement('div');
    host.className='sdc-v313-both-export';
    host.innerHTML='<style>'+exportCss()+'</style><div class="sdc-v313-both-title">SD COMAYAGUA · Comparativo</div><div class="sdc-v313-both-grid"><div class="sdc-v313-both-card"></div><div class="sdc-v313-both-card"></div></div>';
    qs('.sdc-v313-both-card:nth-child(1)',host).appendChild(cards[0]);
    qs('.sdc-v313-both-card:nth-child(2)',host).appendChild(cards[1]);
    document.body.appendChild(host);
    try{
      toast('Preparando imagen comparativa...');
      await waitImages(host);
      var h2c=await loadHtml2Canvas();
      await new Promise(function(resolve){setTimeout(resolve,160)});
      var canvas=await h2c(host,{backgroundColor:'#eaf3fb',scale:isMobile()?1.2:1.55,useCORS:true,allowTaint:false,imageTimeout:15000,removeContainer:true,scrollX:0,scrollY:0,windowWidth:1240,windowHeight:host.scrollHeight+40});
      ...
      var fileName='SDC-recibo-comparativo.png';
      ...
      console.error('SDC v313 Descargar ambas',err);
      toast('No se pudo descargar ambas. Probá Descargar normal y Pagar al recibir por separado.');
    }finally{
      setTimeout(function(){ if(host && host.parentNode) host.parentNode.removeChild(host); },250);
    }
  }
  function ensureBothButton(){
    var modal=qs('#modalRoot .short-receipt-screen');
    if(!modal) return;
    var actions=qs('.short-receipt-actions',modal) || modal;
    var btn=qs('#downloadShortBothV311',modal) || qs('[data-sdc313-both]',modal);
    if(!btn){
      btn=document.createElement('button');
      btn.type='button';
      btn.id='downloadShortBothV311';
      btn.setAttribute('data-sdc313-both','1');
      btn.className='btn sdc230-short-both sdc-v313-both-btn';
      btn.textContent='Descargar ambas';
      actions.appendChild(btn);
    }else{
      btn.setAttribute('data-sdc313-both','1');
      btn.classList.add('sdc-v313-both-btn');
    }
  }
  document.addEventListener('click',function(ev){
    var both=ev.target && ev.target.closest && ev.target.closest('#downloadShortBothV311,[data-sdc313-both]');
    if(both){
      ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation();
      downloadBoth();
      return false;
    }
    var menuBtn=ev.target && ev.target.closest && ev.target.closest('[data-sdc127="open"],.sdc-menu-drawer-v116');
    if(menuBtn) setTimeout(rebuildMenu,0);
    setTimeout(ensureBothButton,120);
  },true);
  function init(){
    rebuildMenu();
    ensureBothButton();
    var root=document.getElementById('modalRoot');
    if(root && !root.__sdcV313MO){
      root.__sdcV313MO=true;
      new MutationObserver(function(){setTimeout(ensureBothButton,60)}).observe(root,{childList:true,subtree:true});
    }
  }
