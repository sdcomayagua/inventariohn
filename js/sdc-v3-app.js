...
(function(){
  ...
  function inject(){
    ...
    // V31: la app principal ya trae su propia barra inferior (Catálogo/Vender/Caja/Producto/Cotizar).
    // Evitamos inyectar una segunda barra porque en móvil se montaban una sobre otra y parecía desplazada a la izquierda.
    const mainBottom = document.querySelector('.bottom-nav');
    const v3Bottom = document.querySelector('[data-sdc-v3="bottom"]');
    if(mainBottom && v3Bottom) v3Bottom.remove();
    if(!mainBottom && !v3Bottom){
