(function(){
  'use strict';
  const KEY='sdc_v60_theme';
  const THEMES=['red','blue'];
  const LABEL={red:'Rojo',blue:'Azul'};
  let busy=false;

  function getTheme(){
    const saved=localStorage.getItem(KEY);
    return THEMES.includes(saved)?saved:'red';
  }
  function setTheme(theme, silent){
    const clean=THEMES.includes(theme)?theme:'red';
    ...
    if(meta) meta.setAttribute('content','#061522');
    updateThemePanel();
  ...
  function panelHTML(){
    ...
    return `<section class="sdc-v60-theme-panel no-print" data-v60-theme-panel="1">
      <div class="sdc-v60-theme-title">Modo visual</div>
      <div class="sdc-v60-theme-buttons" role="group" aria-label="Modo visual">
        <button type="button" data-v60-theme="red" aria-pressed="${t==='red'}">${LABEL.red}</button>
        <button type="button" data-v60-theme="blue" aria-pressed="${t==='blue'}">${LABEL.blue}</button>
      </div>
    </section>`;
  }
