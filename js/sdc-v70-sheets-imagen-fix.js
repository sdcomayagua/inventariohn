...
(function(){
  ...
  const ENABLE_EXTRA_SHEETS_BUTTONS = false; // V80: se deja un solo guardado oficial desde app.js.
  ...
  function addButtons(){
    if(ENABLE_EXTRA_SHEETS_BUTTONS){
      addButtonInOpenProductEditors();
      addButtonsInProductCards();
    }else{
      removeExtraSheetsButtons();
    }
    fixStretchedImages();
  }

  function removeExtraSheetsButtons(){
    document.querySelectorAll('.'+MODAL_ROW_CLASS+',.'+CARD_SLOT_CLASS).forEach(el=>el.remove());
  }
  ...
  document.addEventListener('pointerdown', function(ev){
    if(!ENABLE_EXTRA_SHEETS_BUTTONS) return;
  ...
  document.addEventListener('click', function(ev){
    ...
    window.setTimeout(async ()=>{
      ...
      await syncSheets(extractProductFromContext(context), {manual:false});
      if(restored){
        showToast('Se guardó el producto, se conservó la imagen y se actualizó Sheets.', 'ok');
      }else{
        showToast('Producto guardado. Intenté actualizar Sheets automáticamente.', 'ok');
      }
      scheduleAddButtons();
    }, 850);
  }, true);
  ...
  async function tryKnownSyncFunctions(product){
    const names = [
      'SDC_saveProductToSheets','SDC_syncProductToSheets','updateSheets','actualizarSheets','syncSheets','syncToSheets','syncProductsToSheets',
      'syncProductToSheets','saveProductToSheets','updateProductInSheets','googleSheetsSync',
