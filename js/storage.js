(function(){
  const KEY = 'sdc_control_ventas_v1';
  const BACKUP_KEY = 'sdc_backups_v1';
  ...
  function defaultState(){return {version:1,unlocked:false,products:clone(window.SDC_DEFAULT_PRODUCTS||[]),sales:[],quotes:[],clients:[],closings:[],expenses:[],lastReceipt:null,lastQuote:null,settings:clone(window.SDC_CONFIG||{})}}
  ...
  function findLegacyState(){
    try{
      ...
      for(let i=0;i<localStorage.length;i++){
        ...
        if(k && /^sdc_control_ventas_v\d+$/i.test(k) && k!==KEY) keys.push(k);
      }
      keys.sort((a,b)=>(Number((b.match(/v(\d+)/i)||[])[1])||0)-(Number((a.match(/v(\d+)/i)||[])[1])||0));
      for(const k of keys){
        const parsed=safeJSON(localStorage.getItem(k), null);
