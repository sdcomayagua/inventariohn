(function(){
  ...
  function defaultState(){return {version:93,unlocked:false,products:clone(window.SDC_DEFAULT_PRODUCTS||[]),sales:[],quotes:[],clients:[],closings:[],expenses:[],chats:[],catalogs:[],lastReceipt:null,lastQuote:null,settings:clone(window.SDC_CONFIG||{})}}
  ...
  function normalizeState(s){
    ...
    out.catalogs = Array.isArray(out.catalogs)?out.catalogs:[];
    if(out.settings.syncAuto===undefined) out.settings.syncAuto=true;
    if(out.settings.syncPin===undefined) out.settings.syncPin='';
    if(out.settings.syncUrl===undefined) out.settings.syncUrl='';
    out.settings = Object.assign({}, window.SDC_CONFIG||{}, out.settings||{});
    const defaultSyncUrl = String((window.SDC_CONFIG && window.SDC_CONFIG.syncUrl) || '').trim();
    if(!String(out.settings.syncUrl || '').trim() && defaultSyncUrl) out.settings.syncUrl = defaultSyncUrl;
    return out;
  }
